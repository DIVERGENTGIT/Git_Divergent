...
//call_striker_longcode_api($from,$to,$sms_txt,$on_time,$smsinfo);	
	...
	if($to=="9246190909")
	{
		$sms_url="http://ramojifoundation.org/yuvabharat/getsms.php?from='$from'&to='$to'&date='$on_time'&mesg='$sms_txt'";  
		$h=fopen($sms_url,"r");
		fclose($h);
	...
	if($to=="9246010011") 
	{
		...
		$sms_url="http://felr.org/cms-sms/getsms.php?from='$from'&to='$to'&date='$on_time'&mesg='$sms_txt'";  
		$h=fopen($sms_url,"r");
		fclose($h);
		//http://ramojifoundation.org/yuvabharat/getsms.php?from=&to=&date=&mesg=
	}
	if($user_id=="3152")
	{
		...
		// $sms_url="http://www.carehospitals.com/online_reports/sms_test.php?to_no=$to&from_no=$from&sms_txt=$sms_txt_t";
...
LIMIT 0 , 1");
	...
	$pwdcare=$res['PWD'];
mysql_query("update CARE_PWDS SET PWDFLAG='1' where PWD='$pwdcare'");

		$v="Your Password is ".$pwdcare;
		$final_msg=urlencode($v);
		$sms_url="http://www.smsstriker.com/API/sms.php?username=strikertrans&password=123456&from=STRIKR&to=$from&msg=$final_msg&type=1"; 
		$h=fopen($sms_url,"r");
		fclose($h);
		endif;
		*/
