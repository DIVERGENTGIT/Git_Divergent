...
if(isset($_GET['payment'])) {
	...
	if(mysqli_num_rows($result)>0)
	{
			...
			// check number available
				...
				/*
				$updateServiceDetails = "update  longcode_tmp set status = 2 
				WHERE  user_id IN($registeruser_id) and longcode_number IN ($longcode_numbers)
				and service_type='shared' and status = 3"; 
				mysqli_query($link,$updateServiceDetails);*/
				
				
					
			$checknumstatussql = "SELECT * FROM longcode_tmp
			WHERE user_id IN($registeruser_id) and  longcode_number IN ($longcode_numbers)  and service_type='shared' and status=2
		...
		/*	
		echo mysqli_num_rows($checknumstatusresult);
		
		$data=mysqli_fetch_assoc($checknumstatusresult);
		...
		exit;*/
			...
			//print_r(mysql_fetch_assoc($checknumstatusresult));
			//exit;
			/*
	...
	$getServiceDetails = "SELECT * FROM longcode_tmp WHERE  user_id IN($registeruser_id) and 
	longcode_number IN ($longcode_numbers) and status = 2 and service_type='shared'
	group by longcode_number limit 3"; 
//exit;
	...
	WHERE  user_id IN($registeruser_id) and longcode_number IN ($longcode_numbers)
	and service_type='shared' and status = 2"; 
	mysqli_query($link,$updateServiceDetails);
	// after invoice create update number status start
	...
	$data_array[] = $data;	
	$no_of_keywords=$no_of_keywords+$data['no_of_keywords'];  
	}
 ...
 //echo $invoice_id;exit;
 	...
 	$array = array();
  	...
  	for($i=0;$i<count($serviceData);$i++){
		...
		$array[$i]['service_type'] = $serviceData[$i]['service_type'];
...
global $invoice_url;
              ...
              $url = $invoice_url."invoice_code/index_shared.php"; 
...
<div class="otp-padding">
					<br> 
						<p>No of Keywords     : '.$no_of_keywords.' </p>
