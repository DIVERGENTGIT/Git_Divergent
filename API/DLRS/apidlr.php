...
mysql_select_db("sms");
 ...
 $c4=$c[5];
  ...
  //Custom CampaignID for click2share
   $custom_campaign_id = '';      
  $custom_campaign_id = trim(str_replace("custom_campaign_id=","",$c9));
...
mysql_query("insert into $backup_table set dlr_status='$dlr',to_mobileno='$to',message_id='$campaign_id',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts),port_no='$port_no'"); 
  ...
  mysql_query("update sms_api_messages set re_campaign_dlr_status=1 where message_id='$campaign_id' and to_mobileno='$to' AND re_campaign_status = 1");
		...
		mysql_query("insert into sms_api_messages_dlr_1st_reprocess set dlr_status='$dlr',to_mobileno='$to',message_id='$campaign_id',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts),port_no='$port_no',noofmessages = (SELECT noofmessages FROM sms_api_messages WHERE message_id=$campaign_id and to_mobileno=$to)"); 
  ...
  mysql_query("update sms_api_messages set re_campaign_dlr_status=2 where message_id='$campaign_id' and to_mobileno='$to' AND re_campaign_status = 2");
   	...
   	if(mysql_affected_rows() > 0) {      
		...
		mysql_query("insert into sms_api_messages_dlr_2nd_reprocess set dlr_status='$dlr',to_mobileno='$to',message_id='$campaign_id',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts),port_no='$port_no',noofmessages = (SELECT noofmessages FROM sms_api_messages WHERE message_id=$campaign_id and to_mobileno=$to)"); 
...
if(mysql_num_rows($getUserData) > 0) {
	...
	$date = date('Y-m-d');
		...
		// Request FROM Click2Share
		if($custom_campaign_id > 0) {
			$jobID = $custom_campaign_id;
		}
