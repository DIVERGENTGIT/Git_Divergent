...
if($dlr == 2 && $error_code == '000' ) {
	...
	error_log("\n".date('Y-m-d H:i:s')."| International Data |".$requestBody."\n",3,"/var/www/html/strikerapp/api_log/dlrs_log/dlrs_2_".$cd.".log"); 
	$error_txt = 'EXPIRED';
}

	
	
	 $dlrstatus="update sms_api_messages set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where message_id='$campaign_id' and to_mobileno='$to'";
		
		$date=date("y-m-d");
error_log($dlrstatus,3,"/var/www/html/strikerapp/api_log/dlrs_log/intdlr_".$date.".log");
		
        mysql_query($dlrstatus);
        
       $getJobID = mysql_query("SELECT job_id FROM sms_api_messages WHERE message_id='$campaign_id'");
       $getJobIDRes = mysql_fetch_array($getJobID);
       $jobID = $getJobIDRes['job_id'];
        
        
        
        
        $intDLRStatus="update international_api_messages set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where job_id ='$jobID' and to_mobileno='$to'";  
        mysql_query($intDLRStatus);
