<?php
//exec("/usr/sbin/logrotate -f /etc/logrotate.d/lighttpd"); this is a linux command, not required
// /api_reseller_dlr.php?campaign_id=13614859&smscID=vfirst_home&dlr=1&answer=id%3A4b621bc7-30bb-4621-87ab-7c440fc43b82+sub%3A001+dlvrd%3A001+submit+date%3A140825161513+done+date%3A140825161526+stat%3ADELIVRD+err%3A000+text%3ADefault&to=918806662605&from=SATCOP&ts=1408963856
// above is the URL get values
...
'23' =>'Unknown Subscriber',
'5' =>'Unmarked Subscriber',
'6'=>'VMSC:SM subscriber absent',
'9'=>'VMSC:illegal subscriber',
'11' =>'teleservice not supported',
'26' =>'teleservice not supported',
'12'=>'illegal device',
'13'  =>'call forbidden',
'24'=>'call forbidden',
'21' =>'the network does not support SMS',
'25' =>'the network does not support SMS',
'27'=>'VMSC:subscriber absent',
'31'=>'failure because the subscriber is busy',
'32'=>'Deliver fail',
'33' =>'HLR:Message Waiting List is Full',
'34'=>'GMSC:unknown reason',
'35'=>'VMSC:data lacked',
'36'=>'VMSC:unexpected data',
'90'=>'MS error',
'91' =>'SMS not supported by MS',
'92'=>'MS store is full',
'93'=>'SMSC is congestion',
'94'=>'subscriber is not mobile',
'95'=>'Illegal address',
'100'=>'no response from the called subscriber',
'101'=>'VMSC:the called MS poweroff',
'102'=>'VMSC:limited due to MS roaming',
...
'253'=>'VMSC:no response','254'=>'VMSC:unexpected response received',
'9' => 'Illegal users',
'20' => 'Terminal receives an error',
'22' => 'Phone memory full',
'29' => 'Absent user',
'37' => 'GMSC application resource failed',
'38' => 'GMSC and HLR dialogue fails',
'39' => 'GMSC and HLR session timeout',
'40' => 'HLR Abort failure',
'41' => 'Routing confirmed data errors',
'42' => 'Inform indication parameter error',
'43' => 'GMSC and VMSC dialogue fails',
'44' => 'VMSC Abort failed VMSC Abort',
'45' => 'VMSC system error VMSC System Failure',
'46' => 'UI length error',
'47' => 'Error Code Error Code Version inconsistent versions',
'50' => 'Call unresponsive',
'51' => 'Users shutdown',
'52' => 'Roaming restrictions',
'53' => 'User is not identified',
'54' => 'Users are cleared',
'55' => 'User Busy',
'56' => 'Response Timeout',
'57' => 'Had valid',
'58' => 'Message deletion',
'59' => 'HLR unregistered users',
'60' => 'SMMC monitoring center is rejected',
'61' => 'VMSC fault isolation',
'62' => 'HLR fault isolation',
'70' => 'HLR return illegal purposes SCCP address',
'71' => 'HLR returns the illegal source SCCP address',
'72' => 'HLR returned an unknown error',
'73' => 'Potential returns do not match the version HLR',
'74' => 'HLR returns the version is not compatible with the distal',
'75' => 'HLR returns the remote address unreachable',
'76' => 'HLR return duplicate Invoke ID',
'77' => 'HLR returns the peer does not support this business',
'78' => 'HLR returns unrecognized parameter',
'79' => 'HLR returns the congestion caused by the resource unavailable',
'80' => 'HLR returns the dialogue has begun to release',
'81' => 'HLR returns received from peer unpredictable operation response',
'82' => 'HLR returns the service is not completed',
'83' => 'HLR returns a response is not received',
'84' => 'HLR returns did not receive the expected response',
'85' => 'vmsc return illegal purposes SCCP address',
'86' => 'vmsc return illegal source SCCP address',
'87' => 'vmsc returned an unknown error',
'88' => 'Potential returns do not match the version vmsc',
'89' => 'vmsc return is not compatible with versions of the distal',
'96' => 'vmsc returns received from peer unpredictable operation response',
'97' => 'vmsc return service is not completed',
'98' => 'vmsc returns not received a response',
'99' => 'vmsc return receive unexpected response',
'100' => 'vmsc traffic overload' 
);*/
...
//$link=mysql_pconnect("localhost","root",'ReK1ArEtA6');
...
mysql_select_db("sms");
  ...
  $c=explode("&",$ss2);
 // $cd = date('Y-m-d');
            	 	//error_log("\n".date('Y-m-d H:i:s')."| API Data |".$ss2."\n",3,"/var/www/html/strikerapp/api_log/dlrs_log/port_dlrs_".$cd.".log");  
  $c1=$c[0];  
  $c2=$c[2];  
  $ans = $c[3];
   $c3=$c[4];
...
//error_log($req,3,"/var/www/html/strikerapp/api_log/dlrs_log/dlr_errorcode_log".$date.".log");
		if($dlr == 1){
  ...
  $dlrstatus= " Port: ".$port_no. "Campaign ID: ".$campaign_id. " to: ".$to." DLR: ".$dlr." smscID: ".$smscID." Error code: ".$error_code." Error Text: ".$error_txt."\n";
...
$req = json_encode($_REQUEST,true);
error_log($dlrstatus,3,"/var/www/html/strikerapp/api_log/dlrs_log/dlr_".$today.".log");
...
if($dlr == 2 && $error_code == '000' ) {
	...
	$cd = date('Y-m-d');
	$requestBody = " dlr_status=$dlr,error_code=$error_code,error_text=$error_txt, campaign_id=$campaign_id , to_mobile_no=$to , Route = $smscID";
	error_log("\n".date('Y-m-d H:i:s')."| Campaign Data |".$requestBody."\n",3,"/var/www/html/strikerapp/api_log/dlrs_log/dlrs_2_".$cd.".log");  
  ...
  mysql_query("update campaigns_to set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where campaign_id='$campaign_id' and to_mobile_no='$to'");
    


  mysql_query("insert into $backup_table set campaign_id=$campaign_id,to_mobile_no=$to, dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts),port_no = '$port_no'");
  ...
  /** Changed by Saisandeepthi 2018-07-10 Starts **/
	  ...
	  mysql_query("update campaigns_to set re_campaign_dlr_status=1 where campaign_id='$campaign_id' and to_mobile_no='$to' AND re_campaign_status = 1");
	  
	  if(mysql_affected_rows() > 0) {
	   	 // Inserting first process dlrs
	 		mysql_query("insert into campaigns_to_dlr_1st_reprocess set campaign_id=$campaign_id,to_mobile_no=$to, dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts),port_no = '$port_no' ,sms_count = (SELECT sms_count FROM campaigns_to WHERE campaign_id=$campaign_id and to_mobile_no=$to) ");
	  }   
	        
	  // Means knowing that second reprocess dlr's updated for that changing flag 're_campaign_dlr_status' as 2
	  mysql_query("update campaigns_to set re_campaign_dlr_status=2 where campaign_id='$campaign_id' and to_mobile_no='$to' AND re_campaign_status = 2");
	    
	  if(mysql_affected_rows() > 0) {
		  // Inserting second process dlrs
		  mysql_query("insert into campaigns_to_dlr_2nd_reprocess set campaign_id=$campaign_id,to_mobile_no=$to, dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts),port_no = '$port_no',sms_count = (SELECT sms_count FROM campaigns_to WHERE campaign_id=$campaign_id and to_mobile_no=$to)");
	  }
