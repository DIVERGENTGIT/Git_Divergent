...
//{
 ...
 // $b=explode("?",$ss);
  //$ss0=$b[0];
  //$prestring = explode("/",$ss0);
 // $file_name = $prestring[count($prestring)-1];
// /api_reseller_dlr.php?campaign_id=13614859&smscID=vfirst_home&dlr=1&answer=id%3A4b621bc7-30bb-4621-87ab-7c440fc43b82+sub%3A001+dlvrd%3A001+submit+date%3A140825161513+done+date%3A140825161526+stat%3ADELIVRD+err%3A000+text%3ADefault&to=918806662605&from=SATCOP&ts=1408963856$file_name=
  //$file_name = $_SERVER['PHP_SELF'];
  //$ss1=$b[1];
  //$query_string = explode(" ", $ss1);
...
if(empty($error_txt)) {
  ...
  mysql_query("update ftp_campaigns_to set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where campaign_id='$campaign_id' and to_mobile_no='$to'");
  $dlrstatus= $file_name. "Campaign ID: ".$campaign_id. " to: ".$to." DLR: ".$dlr." smscID: ".$smscID." Error code: ".$error_code." Error Text: ".$error_txt."\n";
   
mysql_query("insert into $backup_table set dlr_status='$dlr',to_mobile_no='$to',campaign_id='$campaign_id',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts)");

$date=date("y-m-d");
error_log($dlrstatus,3,"/var/www/vhosts/www.smsstriker.com/htdocs/api_log/ftp/ftpdlr_".$date.".log");			
  }  
//}	
mysql_close($link);
?>
