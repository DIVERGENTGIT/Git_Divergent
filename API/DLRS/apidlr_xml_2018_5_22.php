...
if($dlr == 2 && $error_code == '000' ) {
	...
	$requestBody = "update sms_api_messages set dlr_status=$dlr,error_code=$error_code,error_text=$error_txt,delivered_on=FROM_UNIXTIME($ts) where message_id=$campaign_id and to_mobileno=$to , Route = $smscID ";
	error_log($requestBody.' DateTime '.$cdTime."\r\n",3,"/var/www/html/strikerapp/api_log/dlrs_log/dlrs_2_".$cd.".log");  
	$error_txt = 'EXPIRED';
}
