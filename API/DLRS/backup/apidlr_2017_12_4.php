...
mysql_query("insert into $backup_table set dlr_status='$dlr',to_mobileno='$to',message_id='$campaign_id',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts)");
...
function returnDLRs($retrunURL,$campaign_id,$to,$dlr,$error_code,$error_txt,$ts) {
	$date = date('Y-m-d');
	$request = array("CampaignID" => $campaign_id,"ToMobile" => $to,"DLR" => $dlr,"ErrorCode" => $error_code,"ErrorText" => $error_txt,"DeliveredON" => ($ts));
	 $requestL = json_encode($request,true);
	 //error_log($requestL."\r\n",3,"/var/www/html/logs/office24by7_log/dlrs_2_".$date.".log");   
	 $ch = curl_init();
