...
$ServiceTax=$rec2['value'];
   $query="SELECT pe.registeruser_id,pe.name,pe.mobile,pe.email,pe.epg_txnID,pe.created_on,pe.smstype,th.payment_id,th.shorturl_credits,th.sms_price as smsprice,th.amount,th.tax_amount, th.total_amount,pe.pgresponse,th.epg_txnID FROM transaction_history th INNER JOIN price_enquery pe on pe.epg_txnID=th.epg_txnID
...
/*
						 $query1="INSERT INTO `user_payments` ( `user_id`, `shorturl_credits`, `price`,
						  `amount`, `service_tax`,
						 `service_tax_percent`, `total_amount`, `on_date`, `payment_type`, `transaction_id`)
						 VALUES ('$registeruser_id', '$shorturl_credits', '$smsprice', '$amount', '$tax_amount', '$ServiceTax', '$total_amount','$on_date',1,$trn_id)";*/

$query1="INSERT INTO `user_payments` ( `user_id`, `shorturl_credits`, `price`, `service_type`,`amount`, `service_tax`,`service_tax_percent`, `total_amount`, `on_date`, `payment_type`, `transaction_id`) VALUES ('$registeruser_id', '$shorturl_credits', '$smsprice', 'shorturl', '$amount', '$tax_amount', '$ServiceTax', '$total_amount','$on_date',1,$trn_id)";
						  ...
						  $query2="update users set shorturl_credits=shorturl_credits+$shorturl_credits where user_id=$registeruser_id";
...
send_invoice_generator($invoice_id,$epg_txnID,$created_on,$registeruser_id,$smstype,$payment_id,$organization,$name,$username,$on_date,$shorturl_credits,$smsprice,$amount,$ServiceTax,$service_tax,$total_amount);

send_email_invoice($epg_txnID,$created_on,$registeruser_id,$smstype,$payment_id,$organization,$name,$username,$on_date,$shorturl_credits,$smsprice,$amount,$ServiceTax,$service_tax,$total_amount,$pgresponse,$mobile,$email);
...
function send_email_invoice($epg_txnID,$created_on,$registeruser_id,$smstype,$payment_id,$organization,$name,$username,$on_date,$shorturl_credits,$price_per_sms,$amount,$ServiceTax,$service_tax,$total_amount,$pgresponse,$mobile,$email)
{
$username=ucfirst($name);
$sms_text = "Your payment ".$pgresponse."!..";
$sms_text .= " No of Shorturl Credits ".$shorturl_credits;
...
<div class="otp-padding">
					<br> 
						<p>Shorturl Credits  : '.$shorturl_credits.' </p>
...
function send_invoice_generator($invoice_id,$epg_txnID,$created_on,$registeruser_id,$smstype,$payment_id,$organization,$name,$username,$on_date,$shorturl_credits,$price_per_sms,$amount,$ServiceTax,$service_tax,$total_amount)
{
  
  global $invoice_url;
 $url = $invoice_url."invoice_code/addshorturl_credits.php";
 
  $parameters="tn=$epg_txnID&invoice_id=$invoice_id&cd=$created_on&rid=$registeruser_id&smstype=$smstype
  &payment_id=$payment_id&od=$on_date&org=$organization&name=$name&st=$ServiceTax&amt=$amount&shorturl_credits=$shorturl_credits&total_amount=$total_amount&prices=$price_per_sms&username=$username&tax_amt=$service_tax";
	$ch = curl_init();
