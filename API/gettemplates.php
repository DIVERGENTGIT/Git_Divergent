...
$sql.="order by campaign_id desc limit ".$limit;
	...
	while($res= $rs1->fetch_array()){
		...
		$sql="select template,template_name,template_header,template_footer from templates where user_id='$user_id'";
if(isset($template)){
			if(!empty($template))
			{	
...
$sql.="order by template_id desc limit ".$limit;
	...
	while($res= $rs1->fetch_array()){
		 ...
		 $temp["tmp_title"]=$res['template_name'];
		 $temp["tmp_header"]=$res['template_header'];
		 $temp["tmp_footer"]=$res['template_footer'];
		array_push($template,$temp);
			}
		echo	json_encode($template);
