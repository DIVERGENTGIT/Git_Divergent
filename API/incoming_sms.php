...
$message_from=$mysqli->query("select f_n,d_n,s_n, message_from,coupn_id from sms_messages where message_from=$sender and message like '%$separtenum%' ");
$res= $message_from->fetch_array();
$count=  $message_from->num_rows;
...
{
	$isavail=$mysqli->query("select f_n,d_n,s_n, message_from,message from sms_messages where  message like '%$separtenum%'");
$codevail=  $isavail->num_rows;
	if($codevail>=1)
	{
		$s_n=1;
	}else{
...
$mysqli->query($insert_query,$link);	
$mysqli->close();
...
$data=$insert_query;
error_log($data." Response: ".$res."\r\n",3,"/var/www/html/Reseller_User/idea/error_log/tsrtc.log");
...
/*
 function  call_tsrtc_api($sender,$body){
                 ...
                 $api .= "query=$body&";
                 $api .= "from=$sender";  
              	error_log($api."\r\n",3,"/var/www/html/Reseller_User/idea/error_log/tsrtc_urlcall.log");
                 return file_get_contents($api);

        }*/
         function  call_tsrtc_api_test($sender,$body){
               $api = "http://182.18.165.185/idea/check.php?";
                 $api .= "query=".urlencode($body);
                 $api .= "&from=".urlencode($sender);  
error_log($api."\r\n",3,"/var/www/html/Reseller_User/idea/error_log/tsrtc_urlcall_test.log");
                 return file_get_contents($api);
