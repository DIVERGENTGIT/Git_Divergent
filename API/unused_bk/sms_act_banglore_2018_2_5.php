...
$query = $mysqli->query("SELECT count(*) as cnt,user_id FROM `api_keys` WHERE `user_keys` LIKE '%$token%'");  
...
$statusMessage = 'Invalid User Key';
 $results[] =  array('statusCode' => $statusCode,'statusMessage' => $statusMessage);
...
if($rs->num_rows > 0)
{
   ...
   // $user_id=$val[0];
	...
	if($user_type==1 && $val['dnd_check']==0)
	{
	...
	if($val['no_ndnc'] == 1 && $val['dnd_check'] == 0){  //Transactional SMPP
		...
		//$sms_port = 15013;
		$sender = $from;
	$port_type="LT1";$portTypeNAS= 'NAST2';	
...
$checkSenderName = $mysqli->query("SELECT * FROM sender_names WHERE user_id = '".$user_id."' AND sender_name = '".trim($from)."' AND status = 1 ");
    ...
    for($i=0;$i<count($mm);$i++) {
		...
		if($val['no_ndnc']==1 && $val['dnd_check']==1 ){
...
$error_msg= "invalid sender ID please use six characters sender ID";
  $results[]=array("statusCode" => $statusCode, "toMobileno"=>$to,"Ack"=> $error_msg);
     echo   json_encode($results); 
