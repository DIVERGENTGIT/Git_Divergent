	...
	if($count == 0) { 
		...
		if($campaign_rs->num_rows > 0 )   
		{
			...
			$no_of_messages_tmp = 0;
	 		...
	 		$insertQuery="insert into campaigns_to  (campaign_id,unique_msg_id,user_id,to_mobile_no,sms_text,sms_count,sent_on,dlr_status,error_text,port_no,short_url) values "; 
			$values="";
			$offset=0;  
			...
			{	
				...
				if($user_available_credits <= 0) {
					$values .= "('$campaign_id','$unique_msg_id','$userId','$to_mobile','$sms_text1','$smsLength',now(),'11','Insufficient Credits','$available_port','$short_code'),"; 
				}elseif($is_block_listed > 0)
				{    
					$values .= "('$campaign_id','$unique_msg_id','$userId','$to_mobile','$sms_text1','$smsLength',now(),'2','Block Listed Number','$available_port','$short_code'),";  
				}elseif($is_invalid_no)
				{								
					$values .= "('$campaign_id','$unique_msg_id','$userId','$to_mobile','$sms_text1','$smsLength',now(),'16','Invalid Number','$available_port','$short_code'),"; 
				} 
				else {  
					...
					if(!$ndnc['no_ndnc'])
					{
						...
						if($isDND > 0)
						{
							...
							$values .= "('$campaign_id','$unique_msg_id','$userId','$to_mobile','$sms_text1','$smsLength',now(),'3','DND Number','$available_port','$short_code'),"; 
						} else {
	 						...
	 						//}
	 						$values .= "('$campaign_id','$unique_msg_id','$userId','$to_mobile','$sms_text1','$smsLength',now(),'0','NULL','$available_port','$short_code'),";
						}  
					} else {     
					if($ndnc['dnd_check'])
					{
