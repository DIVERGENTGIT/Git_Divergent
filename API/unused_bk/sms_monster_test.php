...
$sms_text=explode('\r',$sms_text);
$sms_text= implode(" ",$sms_text);
$sms_text=explode('\t',$sms_text);
$sms_text= implode(" ",$sms_text);
$sms_text=str_replace("\'","'",$sms_text);
$sms_text=str_replace('\"','"',$sms_text);
$sms_text = str_replace("\n", " ", $sms_text);
$sms_text = str_replace("\t", " ", $sms_text);
 $sms_text = str_replace("\r", " ", $sms_text); 
 $sms_text = preg_replace('/\s+/', ' ', $sms_text);
 $sms_text = trim($sms_text);  
$message_length=strlen($sms_text);
if($message_length == 0){
    $message_length = 1;
}
...
$rs=$mysqli->query("select user_id,available_credits,no_ndnc,template_check,International,AllowedCountry from users where username='$username'  ");
...
if($rs->num_rows > 0)
{
   ...
   // $job_id_rs = $mysqli->query("  INSERT INTO  sms_api_job_ids   SET user_id = '$user_id',  created_on = NOW()  ");
     $job_id = 657567;//$mysqli->insert_id; 

    for($i=0;$i<count($mm);$i++) { 
       $to_mobile = $to=trim($mm[$i]);
		$error = FALSE;
 ...
 $sender_names_query = "SELECT sender_name from sender_names where user_id='$user_id' and sender_name='".trim($from)."'" ;
		...
		if($no_of_messages<=$available_credits) 
		{
			...
			if($error)
			{
				...
				{
					//$mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,error_text,job_id,port_no) values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'12','Not a valid Sender Name','$job_id','$sms_port')");
					  echo "{Job Id: $job_id, Ack: Not a valid Sender Name,mobileNo:$to}";
				} else if(!$isValidTemplate)
				{
 
					//$mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,error_text,job_id,port_no) values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'13','Not a valid Template','$job_id','$sms_port')");
  echo "{Job Id: $job_id, Ack: Not a valid Template,mobileNo:$to}";
	    ...
	    $CountryRoute = array("971" => "33013","91" => "33013","972" => "33013","971" => "33013",
	    "968" => "33013","966" => "33013","974" => "33013","90" => "33013","973" => "33013",
	    "962" => "33013","965" => "33013","60" => "33013","95" => "33013",
	    "63" => "33013","65" => "33013","84" => "33013","62" => "33013");
               ...
               //   $mysqli->query("insert into campaigns_to(campaign_id,to_mobile_no,sms_text,sent_on,dlr_status,error_text) values('$campaign_id','$to_mobile','$sms_text',now(),'2','Block Listed Number')");
	...
	if($International==1)
	{
		...
		if($is_invalid_no==0){
			...
			//$data="insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,job_id,port_no)values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'$job_id','$available_port')";

//$mysqli->query("insert into sms_api_messages user_id,sender_name,message,noofmessages,to_mobileno,ondate,job_id,port_no)values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'$job_id','$available_port')");
			$smsId = 78678;//$mysqli->insert_id;

			$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($from)."&to=$to&text=".urlencode($sms_text);
			if($type=="0"){
			$URL .="&mclass=0";
		}
			$URL .= "&dlr-mask=19&dlr-url=".urlencode(SERVER_NAME."/intapidlr.php?campaign_id=$smsId&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
  			//http_send($URL,$available_port);
    echo "{Job Id: $job_id, Ack: Messages has been sent,mobileNo:$to}";
		}else
		{ 

	//$mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,job_id,port_no,dlr_status,error_text) values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'$job_id','$available_port','16','Not Allowed Country')");
 echo "{Job Id: $job_id, Ack: Not Allowed Country,mobileNo:$to}";
		}
				...
				//$mysqli->query("insert into user_credits_logs(before_campaign_credits,after_campaign_credits,current_campaign_credits, user_id,job_id) values('$available_credits','$balance','$no_of_messages','$user_id','$job_id')");
		...
		$data=$data."URL--".$URL ; 
		$date= date('Y-m-d');
		error_log($data."\r\n",3,"/var/www/html/strikerapp/api_log/mons-inter-api_".$date.".log"); 
...
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($from)."&to=$to&text=".urlencode($sms_text);
		$URL .= "&dlr-mask=19&dlr-url=".urlencode("http://182.18.165.185/intapidlr.php?campaign_id=$smsId&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
         ...
         //   $mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,job_id,port_no)    values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'11','$job_id','$sms_port')");
                            
                            $date= date('Y-m-d');
                           // $data="insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,job_id,port_no) values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'11','$job_id','$sms_port')";
                            
                            error_log($data."\r\n",3,"/var/www/html/strikerapp/api_log/insuffient_bal_mons-inter-api_".$date.".log"); 
