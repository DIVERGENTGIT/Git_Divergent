...
error_log($data.$date."\r\n",3,"/var/www/html/strikerapp/api_log/xml-api_request_$date123.log");
...
$sender = $from;  
  ...
  $application_priority = substr($sender,0,5); 
$getPortValue = $mysqli->query("SELECT sending_port_no from sms_queue where application_priority = '".$application_priority."'");
if($getPortValue->num_rows > 0) {
	$getPortValueRes = $getPortValue->fetch_array();
	$sms_port = $getPortValueRes['sending_port_no'];
}else{
	$getPortValue = $mysqli->query("SELECT sending_port_no from sms_queue where application_priority = 'LT2'");
	$getPortValueRes = $getPortValue->fetch_array();
	$sms_port = $getPortValueRes['sending_port_no'];
}
...
if($rs->num_rows > 0)
{
  ...
  $application_priority = substr($from,0,5); 
$getPortValue = $mysqli->query("SELECT sending_port_no from sms_queue where application_priority = '".$application_priority."'");
if($getPortValue->num_rows > 0) {
	$getPortValueRes = $getPortValue->fetch_array();
	$sms_port = $getPortValueRes['sending_port_no'];
