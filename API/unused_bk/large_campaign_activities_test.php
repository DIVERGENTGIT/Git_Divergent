...
{
$query="SELECT * FROM $table_name  WHERE status='0' order by id desc limit 1";
...
if($rslt->num_rows > 0)
	...
	$original_file="/var/www/html/strikerapp/old/".$data['file_path'];  
	...
	$StartMem = memory_get_usage();
		...
		for ($i = $from_row-1; $i <= $to_row; $i++) 
			...
			//echo "insert into schedule_campaigns_to set campaign_id='$campaign_id',unique_msg_id='$unique_num',sms_text='$message',to_mobile_no='$mobile_no',created_on='".date('Y-m-d H:i:s')."'";
 
				$mysqli->query("insert into schedule_campaigns_to set campaign_id='$campaign_id',sms_text='$message',to_mobile_no='$mobile_no',created_on='".date('Y-m-d H:i:s')."'");
