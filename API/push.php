...
$from=$_REQUEST['senderID'];      
$to_mobiles=$_REQUEST['msisdn'];
$type=$_REQUEST['type'];  
...
if($rs->num_rows > 0)
{
    ...
    for($i=0;$i<count($mm);$i++) {
        ...
        if($no_of_messages<=$available_credits) {
            ...
            if($is_block_listed){
                $mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,error_text)
                        values('$user_id','$from','$message1','$no_of_messages','$to',now(),'2','Block Listed Number')");
            } else {
                if(!$val['no_ndnc']){
                    ...
                    if($isDND){  
                        $mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,error_text)
                            values('$user_id','$from','$message1','$no_of_messages','$to',now(),'3','DND Number')");
                    } else {  
                    ...
                    $mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate) values('$user_id','$from','$message1','$no_of_messages','$to',now())");
                    $smsId = $mysqli->insert_id;
