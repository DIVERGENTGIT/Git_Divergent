 ...
 if($sms_port > 0) {
   ...
   include("/var/www/html/strikerapp/smslib/config.inc");
    ...
    for($i=0;$i<count($mm);$i++) {
		...
		$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode(SERVER_NAME."/apidlr.php?campaign_id=$smsId&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&sms_port=$sms_port");
