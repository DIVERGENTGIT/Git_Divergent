...
$custom_campaign_id = ($_REQUEST['custom_campaign_id'])?$_REQUEST['custom_campaign_id']:0;
...
//Unicdoesms
$is_unicode=$_REQUEST['is_unicode'];
...
*/
	...
	if($user_id == 5127 ){ // STRIKER SUPPORT - OTP
		...
		if($from == 'STRIKR' || $from == 'PWDOTP' || $from == 'OFCOTP' || $from == 'OFCALT' ) { 
			//$sms_port ="46013";    
			$sms_port =51113;//37113;// 37213;//"46013";//"37113";    
		} // || $senderName == 'PASWRD'  
...
if($sms_port > 0) {
	...
	for($i=0;$i<count($mm);$i++) {    
		...
		values('".$user_id."','".$from."','".$message1."','".$no_of_messages."','".$to."',now(),'".$job_id."','".$sms_port."')");
							...
							//if($user_id == 6260) {
								//$URL .= "&dlr-mask=19&dlr-url=".urlencode("$returnurl?campaign_id=$job_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&sms_port=$sms_port");
							//}else{	
								$URL .= "&dlr-mask=19&dlr-url=".urlencode(SERVER_NAME."/apidlr.php?campaign_id=$smsId&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&sms_port=$sms_port&custom_campaign_id=$custom_campaign_id");
							//}
