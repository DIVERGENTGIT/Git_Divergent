...
echo $message = urldecode(urldecode($message));  exit;
...
$sms_port ="47913";
	...
	$to = str_replace($remove, "", $to );
        ...
        $to=trim($to);
 $seriers = $mysqli->query("SELECT Service_Areas,zonecode FROM `series_wise` where Number_Starts_With=substring($to,1,length(Number_Starts_With))"); 
