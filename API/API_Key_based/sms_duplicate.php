...
$mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,job_id,port_no)values('$user_id','$from','$message1','$no_of_messages','$to',now(),'3','$job_id','$sms_port')");
...
$textmd5=md5($text1);
$checkContent = $mysqli->query("SELECT count(*) FROM duplicatecheck_api WHERE md5text = '$textmd5'");
 $checkContentData = $checkContent->fetch_array();
  $is_duplicate = $checkContentData[0];
