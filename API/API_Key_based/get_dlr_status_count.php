...
if($rs->num_rows > 0){
	$getJIdQuery = $mysqli->query("SELECT created_on FROM sms_api_job_ids WHERE job_id = '{$job_id}'");
while($getJIdQuery_rs = $getJIdQuery->fetch_array(MYSQLI_ASSOC))
{
 ...
 if($days_diff <= 1){
    ...
    $result .= "{";
    $getDLRQuery = $mysqli->query("SELECT to_mobileno, dlr_status, error_text FROM $table_name WHERE job_id = '{$job_id}'");
    if($getDLRQuery->num_rows > 0){
		...
		if($dlr_type==0)
		{
		 $result .= "{Delivered : $deliveredCount, DND Numbers: $dndCount,Invalid Numbers : $invalidCount, Failed Numbers: $failedCount} ";
		}
		if($dlr_type==1)
		{
		 $result .= "{Delivered : $deliveredCount}";
		}
		if($dlr_type==2)
		{
		 $result .= "{Failed Numbers: $failedCount}";
		}  
		if($dlr_type==3)
		{
		 $result .= "{DND Numbers: $dndCount}";
		}
		if($dlr_type==4)
		{
		 $result .= "{Invalid Numbers : $invalidCount}";
		}
