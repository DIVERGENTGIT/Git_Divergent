...
$rs=$mysqli->query("select user_id,available_credits,no_ndnc,dnd_check,template_check from users where username='$username' and password='$password'");
if($rs->num_rows > 0)
{
	$val= $rs->fetch_array();
	$user_id=$val[0];
                ...
                if(!$val['no_ndnc']){
                    ...
                    $query="insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,job_id,port_no)
                       ...
                       $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender)
										...
										$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode(SERVER_NAME."apidlr_xml.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
