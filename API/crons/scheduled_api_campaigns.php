...
elseif($ndnc['no_ndnc'] == 1 && $ndnc['dnd_check'] == 1){ //SEMI  transactional
 	...
 	$senderName_kennel = $sender;
		...
		if($to_details_rs->num_rows > 0) {
			while($to_details_val =  $to_details_rs->fetch_array(MYSQLI_ASSOC))
			{
				...
				if(empty($to_mobile) || !is_numeric($to_mobile)) {
					$to_mobile = 0;
				} 
                ...
                if($is_block_listed > 0){
                    $mysqli->query("insert into sms_api_messages(user_id,sender_name,job_id,to_mobileno,message,noofmessages,`ondate`,dlr_status,error_text) values('$user_id','$sender_name','$job_id','$to_mobile','$sms_text1',$no_of_messages,now(),'2','Block Listed Number')");
                   	   ...
                   	   $mysqli->query("insert into sms_api_messages(user_id,sender_name,job_id,to_mobileno,message,noofmessages,`ondate`,dlr_status,error_text) values('$user_id','$sender_name','$job_id','$to_mobile','$sms_text1',$no_of_messages,now(),'16','Invalid Number')");
			 			//$block_no_count++;
                    }else {
                    if(!$ndnc['no_ndnc']){
  ...
  $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($senderName_kennel)."&to=91$to_mobile&text=".urlencode($sms_text);
                            $URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode(SERVER_NAME."/dlr.php?campaign_id=$job_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&sms_port=$available_port");
