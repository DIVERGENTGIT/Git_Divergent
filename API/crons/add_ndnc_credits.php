<?php 
$link = mysql_pconnect("localhost","strikerapp",'Off!c3@v2017') or die(mysql_error());
...
if(mysql_num_rows($users_rs) > 0) {  
	while($users_val = mysql_fetch_array($users_rs)) 
	{
		...
		//$query = "select count(*) from campaigns_to where dlr_status = 3 and campaign_id in (select campaign_id from campaigns where user_id = '$user_id' and date(created_on) = DATE_SUB(curdate(), INTERVAL 1 DAY) )";
		 $query1 = "select count(sms_text) as cnt,length(sms_text) as msglen from campaigns_to where (dlr_status = 3) and campaign_id in (select campaign_id from campaigns where user_id = '$user_id' and  is_scheduled='0' and date(created_on) = DATE_SUB(curdate(), INTERVAL 1 DAY) ) group by length(campaigns_to.sms_text) ";
	  ...
	  while($resultSet1_val = mysql_fetch_array($resultSet1)) 
	  {
		...
		$final_returns_credits=0;
		 ...
		 $schedule_query = "select count(sms_text) as cnt,length(sms_text) as msglen from campaigns_to where (dlr_status = 3) and  campaign_id in (select campaign_id from campaigns where user_id = '$user_id' and  is_scheduled='1' and campaign_status=2 and date(scheduled_on) = DATE_SUB(curdate(), INTERVAL 1 DAY) ) group by length(campaigns_to.sms_text) ";
