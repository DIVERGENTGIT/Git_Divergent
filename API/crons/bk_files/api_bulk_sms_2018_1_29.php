...
while($report = $getSMSReport->fetch_array()) {
	...
	{  
		...
		case in_array($semi,$userActType) : $portType = "LS2"; // semi trans 
						    break;
		case in_array($tran,$userActType): $portType = "LT2"; // trans
						   break;
		case (in_array($promo,$userActType) || in_array($promo2,$userActType)) : $portType = "LP2"; // promo
											 break; 
	}  
	...
	if($tempRecord) { $mysqli->query("DELETE FROM sms_api_bulk_messages WHERE tempID = '".$tempID."' "); }
}
