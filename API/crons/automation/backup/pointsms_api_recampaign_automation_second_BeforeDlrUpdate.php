...
$getReCampaigns = $mysqli->query("SELECT message_id,user_id,sender_name,message,to_mobileno,port_no,error_text,dlr_status FROM sms_api_messages WHERE  re_campaign_status = 2 AND (error_code = '083' OR error_code = '036' OR error_code = '019' OR error_code = '225' OR error_code = '245' OR error_code = '100' OR error_code = '103' OR error_code = '106' OR error_code = '024' OR (dlr_status = 16 AND error_text ='Misc Error') ) AND date(ondate) = date(NOW()) AND (port_no IN ($bsnlPortStr)  OR  port_no LIKE '1%' )   LIMIT 5000");       
