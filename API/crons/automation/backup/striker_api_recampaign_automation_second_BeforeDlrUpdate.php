	  ...
	  //      		$port_no = 45113;
	    ...
	    //    	}    
	   		 ...
	   		 if($dlr_status == 16 && $error_text = 'Misc Error') {
		        	 		 $dateTime = date('Y-m-d');     
		        	 		$logText = "Message ID : $message_id,Dlr Status : $dlr_status, with $error_text";
				...
				$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("https://www.smsstriker.com/API/DLRS/apidlr.php?campaign_id=$message_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&sms_port=$port_no");  
			 	http_send($URL,$port_no);       
					                   
				error_log("\n".date('Y-m-d H:i:s')."| Recampaign API Automation For Message ID - $message_id | To Number $to_mobile with URL ".$URL."\r\n",3,"/var/www/html/logs/Automation_logs/API/striker_api_IEA_".$logName.".log");  
