...
$getReCampaigns = $mysqli->query("SELECT campaign_id,user_id,sender_name,is_unicode_sms FROM campaigns WHERE re_campaign_status = 0 AND campaign_status = 2 AND user_id in (select user_id from users where reseller_id = 350) AND date(created_on) = date(NOW()) LIMIT 1 ");    
