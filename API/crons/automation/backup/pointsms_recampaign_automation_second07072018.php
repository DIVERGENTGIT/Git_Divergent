				...
				while($campaignsProcessDataRes = $getCampaignsProcessData->fetch_assoc()) {
					$sms_txt = trim($campaignsProcessDataRes['sms_text']);
				        ...
				        if($port_no != '45113') {
							...
							//$getUserType = $mysqli->query("SELECT * FROM sms.notify_misc_error WHERE port_no = '".$port_no."' AND created_on = date(now())");
							//if($getUserType->num_rows == 0) {
						 		//$mysqli->query("INSERT INTO sms.notify_misc_error SET port_no = '".$port_no."', campaign_id = '".$campaign_id."' ,source='Pointsms' "); 
						 	//}  
			        	 	}  */
