...
$getReCampaigns = $mysqli->query("SELECT campaign_id,user_id,sender_name,is_unicode_sms FROM campaigns WHERE re_campaign_status = 0 AND campaign_status = 2  AND campaign_id = 1716668   LIMIT 1");
...
if($getReCampaigns->num_rows > 0 ) {    
	while($campaignResult = $getReCampaigns->fetch_assoc()) {
		...
		$mysqli->query("UPDATE campaigns SET re_campaign_status = 1 WHERE campaign_id = '".$campaign_id."'  "); 
	        ...
	        $getCampaignsProcessData = $mysqli->query("SELECT sms_text,to_mobile_no,port_no FROM campaigns_to WHERE campaign_id ='".$campaign_id."' AND (error_code = '083' OR error_code = '036')");         
