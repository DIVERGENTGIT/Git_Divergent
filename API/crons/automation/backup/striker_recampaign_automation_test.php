				        ...
				        if($getUserType->num_rows > 0) {
				        	...
				        	$dnd_check = $userTypeRes['dnd_check'];
				        	 ...
				        	 if($port_no != '45113') {  
								...
								$getUserType = $mysqli->query("SELECT * FROM notify_misc_error WHERE port_no = '".$port_no."' AND created_on = date(now())");
								if($getUserType->num_rows == 0) {
							 		$mysqli->query("INSERT INTO notify_misc_error SET port_no = '".$port_no."', campaign_id = '".$campaign_id."'  "); 
							 	}  
				        	 	}
