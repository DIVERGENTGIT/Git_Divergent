...
$getReCampaigns = $mysqli->query("SELECT message_id,user_id,sender_name,message,to_mobileno,port_no,dlr_status,error_text FROM sms_api_messages WHERE  re_campaign_status = 0 AND (error_code = '083' OR error_code = '036' OR error_code = '019' OR error_code = '225' OR error_code = '245' OR error_code = '100' OR error_code = '103' OR error_code = '106' OR error_code = '024' OR (dlr_status = 16 AND error_text ='Misc Error' ) ) AND date(ondate) = date(NOW()) AND (port_no IN ($bsnlPortStr)  OR  port_no LIKE '1%' )   LIMIT 5000");       
	//$getReCampaigns = $mysqli->query("SELECT message_id,user_id,sender_name,message,to_mobileno,port_no,dlr_status,error_text FROM sms_api_messages WHERE (error_code = '083' OR error_code = '036' OR error_code = '019' OR error_code = '225' OR error_code = '245' OR error_code = '100' OR error_code = '103' OR error_code = '106' OR error_code = '024' OR (dlr_status = 16 AND error_text ='Misc Error')) AND re_campaign_status = 0 AND date(ondate) = date(NOW()) AND (port_no IN ($bsnlPortStr)  OR  port_no LIKE '1%') LIMIT 5000"); 
			...
			$mclass ="&mclass=''";      
				...
				if($port_no > 0) {
					...
					{      
						...
						$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("http://pointsms.in/DLRS/api_reseller_dlr.php?campaign_id=$message_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&sms_port=$port_no");
					 	http_send($URL,$port_no);         
								         
						error_log("\n".date('Y-m-d H:i:s')."| Recampaign API Automation For Message ID - $message_id | To Number $to_mobile with URL ".$URL."\r\n",3,"/var/www/html/logs/Automation_logs/API/pointsms_api_BNL_".$logName.".log");  
