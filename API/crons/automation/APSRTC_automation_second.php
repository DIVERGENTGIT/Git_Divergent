...
$getReCampaigns = $mysqli->query("SELECT message_id,user_id,sender_name,message,to_mobileno,port_no FROM sms_api_messages WHERE  re_campaign_status = 1 AND re_campaign_dlr_status = 1 AND (error_code = '001' OR error_code = '005' OR error_code = '010' OR error_code = '021' ) AND user_id = 4130 AND date(ondate) = date(NOW()) LIMIT 500");  
