...
if($user_id==4139 || $user_id==4131) //Narayana and manikanth challa
{
...
$sms_port ="47213";
    ...
    for($i=0;$i<count($mm);$i++) {
		...
		echo "{Job Id: $job_id, Ack: Not a valid Template}";
		...
		echo "{Job Id: $job_id, Ack: Vendor Specific Error: Please contact your provider}";			
				}	
			
			}else
			{
			$text1=$username.$smstext.$sender.$mobile_no;  
 	   ...
 	   if($is_duplicate > 0){ 			 
 			$mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,error_text,job_id,port_no)
                        values('$user_id','$from','$message','$no_of_messages','$to',now(),'16','Duplicate Msg','$job_id','$sms_port')");
		echo "{Job Id: $job_id, Ack: Duplicate Msg}";
...
//$no_of_messages=ceil($message_length/160);
		...
		if($no_of_messages<=$available_credits) 
		{
			...
			if($error)
			{
				...
				{
					$mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,error_text,job_id,port_no) values('$user_id','$from','$message','$no_of_messages','$to',now(),'12','Not a valid Sender Name','$job_id','$sms_port')");                echo "{Job Id: $job_id, Ack: Not a valid Sender Name}";
				} else if(!$isValidTemplate)
				{
