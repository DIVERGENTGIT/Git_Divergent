...
$rs=$mysqli->query("select user_id,available_credits,no_ndnc,template_check,International,AllowedCountry from users where username='$username' and password=md5('$password')");
					...
					$available_port = "33013";  //Default Port
						...
						if((strlen($to_mobile) >= 6 and strlen($to_mobile)<=16)) {
							if(strcmp($AllowedCountry,"*")==0){
								...
								//}
$positions = 'MobileNo - ' .$to. ' - '.$pos1. ' ,'.$pos2.' ,'.$pos3.' ,'.$pos4;		$date= date('Y-m-d');

					if($pos1 === false && $pos2 === false && $pos3 === false && $pos4 === false) {
						if($pos3==13 || $pos2==40 || $pos2==5 || $pos2==54 ||  $pos2==19){
						$is_invalid_no=1;}else{
						$is_invalid_no=1;}
						error_log($positions."\r\n",3,"/var/www/html/strikerapp/api_log/mons_pos_inval".$date.".log"); 
						}else{		
									if(substr($to_mobile, 0, 1)==1 || substr($to_mobile, 0, 1)==0){
									if($pos3==13 || $pos2==40 || $pos2==5 || $pos2==54 ||  $pos2==19){
									$is_invalid_no=1;}else{
									$is_invalid_no=1;}
									error_log($positions."\r\n",3,"/var/www/html/strikerapp/api_log/mons_pos_inval".$date.".log"); 
									...
									$is_invalid_no=0;
									error_log($positions."\r\n",3,"/var/www/html/strikerapp/api_log/mons_pos".$date.".log"); 
									}
									if($pos3==13 || $pos2==40 || $pos2==5 || $pos2==54 ||  $pos2==19){
									$is_invalid_no=1;
									error_log($positions."\r\n",3,"/var/www/html/strikerapp/api_log/mons_pos_inval".$date.".log"); 
	...
	$mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,job_id,port_no,dlr_status,error_text)
...
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($from)."&to=$to&text=".urlencode($sms_text);
		$URL .= "&dlr-mask=19&dlr-url=".urlencode("http://182.18.165.185/intapidlr.php?campaign_id=$smsId&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
            ...
            $mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,job_id,port_no)
                            values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'11','$job_id','$sms_port')");
                            
                            $date= date('Y-m-d');
                            $data="insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,job_id,port_no)
                            values('$user_id','$from','$sms_text','$no_of_messages','$to',now(),'11','$job_id','$sms_port')";
