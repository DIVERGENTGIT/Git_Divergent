...
if($from_date && $to_date) {
	...
	//if($days_diff_from && $days_diff_to) {
		...
		foreach($apsrtcUsers as $userID) {
			...
			{
				$getUserNameRes = $getUserName->fetch_array();
	   			 $userName = $getUserNameRes['username'];
	
				/*$report[$i]['SID'] = $i;
				$report[$i]['SName'] = $userName;*/
				$report['SID'] = $i;
				$report['SName'] = $userName;
				$servicecount = 0;
				...
				if($getSMSAPICountRes->num_rows > 0) {
					while($response = $getSMSAPICountRes->fetch_array()) {
				...
				if($getAPICountRes->num_rows > 0) {
					while($response = $getAPICountRes->fetch_array()) {
						$servicecount += $response['sms_count'];
					}
				}
...
array_push($arr_push,$report);
	...
	echo explode("\n",htmlentities($apsrtcReport->asXML()), 2)[1];
	exit;*/
				echo json_encode(array(array('Message' => 'Data Not Available')));
}
