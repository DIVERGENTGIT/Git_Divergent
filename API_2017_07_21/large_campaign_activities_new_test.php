...
$sqlto=$mysqli->query("SELECT COUNT(*) AS COUNT FROM $table_name  WHERE status='0' and campaign_id = 1463090");
...
{ 
	...
	// select  large_campaign_activities_new which status is zero means new campaign every time 1
 	$query=$mysqli->query("SELECT * FROM $table_name  WHERE status='0' and campaign_id = 1463090 order by id asc limit 1 ");
	if($query->num_rows > 0)
	{
								...
								if($shorturl_input != NULL)
								{
								 ...
								 $message = $mysqli->real_escape_string($message);
  								 ...
  								 $message = trim($message);  
									...
									echo $sms_length=$sms_length_tmp; echo "</br>";  
