...
if(!$db_link)
{
    $db_link=mysql_connect("localhost","smsstrikerapp",'$tr!k3r@2009') or die(mysql_error());
}
else
{
    $db_link=mysql_pconnect("localhost","smsstrikerapp",'$tr!k3r@2009') or die(mysql_error());
}
...
include("/var/www/html/strikerapp/smslib/config.inc");
	...
	if($International==1)
	{
		...
		if($is_invalid_no==0){
			...
			$URL .= "&dlr-mask=19&dlr-url=".urlencode("http://www.smsstriker.com/API/DLRS/intapidlr.php?campaign_id=$smsId&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
