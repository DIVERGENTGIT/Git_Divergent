...
if(isset($_GET['payment'])) {
	...
	if(mysqli_num_rows($result)>0)
	{
			...
			$checknumstatussql = "SELECT sum(no_of_keywords)  as no_of_keywords ,sum(no_of_sms)  as no_of_sms,status
			FROM longcode_tmp
			WHERE user_id IN($registeruser_id) and  longcode_number IN ($longcode_numbers)  and service_type='shared' and status = 3 
			...
			//print_r(mysqli_fetch_array($checknumstatusresult));
	...
	*/
	...
	longcode_number IN ($longcode_numbers) and status = 3 and service_type='shared'
	group by longcode_number limit 3"; 
//exit;
	...
	$noofsms,$smsprice,$amount,$ServiceTax,$service_tax,$total_amount,$pgresponse,$mobile,$email,$no_of_keywords);
				...
				$sql1="update price_enquery set  is_created=0 where epg_txnID=$trn_id";
				$result = mysqli_query($link,$sql1) or die('Error query:  '.$sql1);
				print_r(json_encode(array('status'=>"success","message"=>"Transaction Cancelled")));	
					}
