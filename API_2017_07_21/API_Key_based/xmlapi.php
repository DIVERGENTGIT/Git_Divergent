...
$rs=$mysqli->query("select user_id,available_credits,no_ndnc,template_check from users where username='".$username."' and password='".$password."'");
if($rs->num_rows > 0)
{
    $val=$rs->fetch_array();
    $user_id=$val[0];
...
$URL .= "&dlr-mask=19&dlr-url=".urlencode(SERVER_NAME."/apidlr_xml.php?campaign_id=$smsId&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
        ...
        if($error_msg){
  $results[]=array("JobId" =>$job_id,"toMobile"=>$to, "Ack"=> $error_msg);
    }else{
  $results[]=array("JobId" =>$job_id, "Ack"=> "Messages has been sent");
