...
$rs = $mysqli->query("select user_id, no_ndnc,dnd_check, available_credits from users where username='".$username."' and password= '".md5($password)."' ");
if($rs->num_rows > 0) {
    $val = $rs->fetch_object();     
    $no_ndnc = $val->no_ndnc;
...
{
$actbal=array("ActType"=>"Semi Transactional","Balance"=>$balance);
}
if($dnd_check==0 && $no_ndnc==1)
{
$actbal=array("ActType"=>"Transactional","Balance"=>$balance);
}
if($no_ndnc==0)
{
$actbal=array("ActType"=>"Promotional","Balance"=>"$balance");
}
