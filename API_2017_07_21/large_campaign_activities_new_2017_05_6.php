		...
		if(is_file($original_file)) { 
			...
			if(file_exists($original_file)) {	 
				...
				$StartMem = memory_get_usage();
					...
					$shortInput = '';$getsendShorturl = '';$message = '';$mobileNoCount = 0;
 					$values = ''; $offset=0;
									...
									if($pos === false) {
										...
										$shortUrlInput = $mysqli->query("SELECT long_url FROM shorturl_db.short_urls WHERE short_code = '".trim($short_code)."'");
     										...
     										if($shortUrlInput->num_rows > 0) {
												...
												$shortInput = $shortUrlInputRes['long_url'];		
			  		...
			  		if($total_no_of_sms > 0)
					{
						...
						$mysqli->query("UPDATE campaigns SET sms_text= '".$message."' ,long_url ='".$shortInput."',shorturl_text = '".$getsendShorturl."',phone_nos_count = phone_nos_count + '$mobileNoCount' WHERE campaign_id = '".$campaign_id."'");  
 					...
 					$mysqli->query("update $table_name set status='2',no_of_sms='".$total_no_of_sms."' where id='$id'"); 
					if($is_schedule == 0) {
		...
		if($campaign_rs->num_rows > 0 )   
		{
			...
			{	
				...
				if($user_available_credits <= 0) {
					$values .= "('$campaign_id','$unique_msg_id','$to_mobile','$sms_text1',now(),'11','Insufficient Credits','$available_port'),"; 
				}elseif($is_block_listed)
				{    
...
function scheduleCampaignsTo() {
	...
	$campaign_rs = $mysqli->query("select campaign_id,user_id,phone_nos_count from campaigns where is_scheduled='1' and campaign_status='1' AND source_type IN (3,4) and scheduled_on <= now()");
	...
	if($campaign_rs->num_rows > 0) {
		while($campaign_val= $campaign_rs->fetch_array(MYSQLI_ASSOC)) {
			...
			if($getLargeCampaignData->num_rows > 0) {  
				...
				scheduledCampaign($tableCount,$user_id,$campaign_id);   
...
function scheduledCampaign($count,$userId,$campaign_id)   
{	
