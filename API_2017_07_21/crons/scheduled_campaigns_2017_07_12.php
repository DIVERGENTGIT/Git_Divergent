...
if($campaign_rs->num_rows >0) {
	...
	$campaign_details_rs = $mysqli->query("select user_id,campaign_id,sender_name,sms_text,long_url,shorturl_text,campaign_type,is_unicode_sms,no_of_messages from campaigns where campaign_id in ($campaign_ids)");
		...
		if($to_details_rs->num_rows > 0) 
		{
			...
			while($to_details_val =  $to_details_rs->fetch_array(MYSQLI_ASSOC))
			{
                ...
                if($is_block_listed > 0){
                    $mysqli->query("insert into campaigns_to(campaign_id,to_mobile_no,sms_text,sent_on,dlr_status,error_text,short_url) values('$campaign_id','$to_mobile','$sms_text1',now(),'2','Block Listed Number','$short_code')");
                } 
                elseif($is_invalid_no){
                   	  $mysqli->query("insert into campaigns_to(campaign_id,to_mobile_no,sms_text,sent_on,dlr_status,error_text,port_no,short_url) values('$campaign_id','$to_mobile','$sms_text1',now(),'16','Invalid Number','$available_port','$short_code')");
						//$block_no_count++;
                    }
                    ...
                    if(!$ndnc['no_ndnc']){
                        //check for dnd number
                       	 ...
                       	 $isDND = $checkDndRow[0];
                       	 if($isDND){
                            $mysqli->query("insert into campaigns_to(campaign_id,to_mobile_no,sms_text,sent_on,dlr_status,error_text,short_url) values('$campaign_id','$to_mobile','$sms_text1',now(),'3','DND Number','$short_code')");
                           	...
                           	$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode(SERVER_NAME."/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
                            	...
                            	$mysqli->query("insert into campaigns_to(campaign_id,to_mobile_no,sms_text,sent_on,port_no,short_url) values('$campaign_id','$to_mobile','$sms_text1',now(),'$available_port','$short_code')");
