...
if($campaign_rs->num_rows > 0) 
{
    ...
    $campaign_details_rs = $mysqli->query("select user_id,campaign_id,campaign_name,sender_name,sms_text,campaign_type,is_unicode_sms,long_url,shorturl_text,no_of_messages,phone_nos_count from campaigns where campaign_id in ($campaign_ids)");
	while($campaign_details_val = $campaign_details_rs->fetch_array(MYSQLI_ASSOC)) 
	{
		...
		$ndnc = $mysqli->query("select username,no_ndnc,dnd_check from users where user_id='$user_id'");
...
$available_port = $available_port_val['sending_port_no'];
		...
		{
			if($to_mobile[0]=='7' or $to_mobile[0]=='8' or $to_mobile[0]=='9' )
			{
				$is_invalid_no=0;
                       ...
                       error_log("Semi ".$dndquery."dndstatus ".$isDND."\n",3,ERRORLOGS);
                        if($isDND > 0){
                          ...
                          $values.="('$campaign_id','$user_id','$sender','$to_mobile','$sms_text1','$smsLength',now(),'3','DND Number','$available_port','$short_code'),";  
                            ...
                            $values.="('$campaign_id','$user_id','$sender','$sender','$to_mobile','$sms_text1','$smsLength',now(),'0','NULL','$available_port','$short_code'),";
                        }
