...
$datelog= date('Y-m-d');
error_log($data."\r\n",3,"/var/www/html/strikerapp/api_log/striker_api_log/apimargadarsi_".$datelog.".log");
    ...
    for($i=0;$i<count($mm);$i++) {
		...
		if($no_of_messages<=$available_credits) 
		{
			if($error)
			{
				...
				{
				$query="insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,error_text,job_id,port_no)	values('$user_id','$from','$message','$no_of_messages','$to',now(),'5', 'Vendor Specific Error: Please contact your provider','$job_id','$sms_port')";	
...
$query=	"insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,job_id,port_no)values('$user_id','$from','$message','$no_of_messages','$to',now(),'3','$job_id','$sms_port')";					
$mysqli->query($query);
		 ...
		 $query="insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,job_id,port_no)
                            values('$user_id','$from','$message','$no_of_messages','$to',now(),'11','$job_id','$sms_port')";
...
if($usre_id=='4754' && $numberlength==12){
...
//error_log($query."\r\n",3,"/var/www/vhosts/www.smsstriker.com/htdocs/api_log/striker_api_log/apistriker_".$date.".log");
		  ...
		  echo "{Job Id: $job_id, Ack: DND Number,mobileNo:$to}";
                    ...
                    $query="insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,job_id,port_no)
                        ...
                        $smsId = $mysqli->insert_id;
if($usre_id=='4754'){
                        $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender)."&to=$to&text=".urlencode($message1);
}else{
   $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender)."&to=91$to&text=".urlencode($message1);
}
