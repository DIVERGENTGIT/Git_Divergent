...
if($rs->num_rows > 0)
{
	...
	$mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,job_id,port_no,dlr_status,error_text)
		...
		$data=$data."URL--".$URL ; 
		$date= date('Y-m-d');
		error_log($data."\r\n",3,"/var/www/vhosts/www.smsstriker.com/htdocs/api_log/mons-inter-api_".$date.".log"); 
...
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($from)."&to=$to&text=".urlencode($sms_text);
		$URL .= "&dlr-mask=19&dlr-url=".urlencode("http://182.18.165.185/intapidlr.php?campaign_id=$smsId&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
            ...
            $mysqli->query("insert into sms_api_messages(user_id,sender_name,message,noofmessages,to_mobileno,ondate,dlr_status,job_id,port_no)
                            ...
                            error_log($data."\r\n",3,"/var/www/vhosts/www.smsstriker.com/htdocs/api_log/insuffient_bal_mons-inter-api_".$date.".log"); 
        }
    } //for
