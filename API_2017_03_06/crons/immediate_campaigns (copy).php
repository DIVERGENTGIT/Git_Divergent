...
$campaign_rs = $mysqli->query("select campaign_id,campaign_name from campaigns where is_scheduled='0' and campaign_status='1' LIMIT 10");
...
if($campaign_rs->num_rows > 0) 
{
	...
	while($campaign_details_val = $campaign_details_rs->fetch_array(MYSQLI_ASSOC)) 
	{
		...
		$ndnc = $mysqli->query("select no_ndnc,dnd_check from users where user_id='$user_id'");
	...
	$dnd_check_transuser = $ndnc['dnd_check'];
		...
		//added by bharath for transscrub users
		/*if($dnd_check_transuser)
                        	{
                        		$available_port='21013';
                        	}	
                        	*/
	if($user_id=='1395' || $user_id=='3361' || $user_id=='3304')
	{
	...
	if($user_id=='4121' || $user_id=='250') //baljimarket
	{
	$available_port='47113';
	}

	if($user_id=='4131'  || $user_id=='4130' )
	{
