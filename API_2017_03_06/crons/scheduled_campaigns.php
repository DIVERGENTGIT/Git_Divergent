...
global $mysqli;
$campaign_rs = $mysqli->query("select campaign_id from campaigns where is_scheduled='1' and campaign_status='1' and scheduled_on <= now()");
$campaigns = "";
 	...
 	$getQueryRes = $mysqli->query("SELECT sending_port_no FROM sms_queue where application_priority='$portType' order by queued, sent asc limit 1");
		...
		if($to_details_rs->num_rows > 0) 
		{
			...
			while($to_details_val =  $to_details_rs->fetch_array(MYSQLI_ASSOC))
			{
                ...
                $is_block_listed = $blockedNumberRow;
                if($is_block_listed){
                ...
                elseif($is_invalid_no){
                    ...
                    if(!$ndnc['no_ndnc']){
                        ...
                        if($mysqli->errno){
							fwrite($fp,"[".date("Y-m-d H:i:s")."]MySQL Error :".$mysqli->errno().": ".$mysqli->errno()." Query : SELECT count(*) FROM dnd_db.ndnc_data WHERE dnc_number = '{".$to_mobile."}'\n");
						}
                        ...
                        if($isDND){
                            ...
                            if($mysqli->errno){
								fwrite($fp,"[".date("Y-m-d H:i:s")."]MySQL Error :".$mysqli->errno.": ".$mysqli->errno." Query : insert into campaigns_to(campaign_id,to_mobile_no,sms_text,sent_on,dlr_status,error_text) values('".$campaign_id."','".$to_mobile."','".$sms_text1."',now(),'3','DND Number')\n");
							}
                        } else {
                           ...
                           $URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode(SERVER_NAME."/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
                            ...
                            if($mysqli->errno){
								fwrite($fp,"[".date("Y-m-d H:i:s")."]MySQL Error :".$mysqli->errno.": ".$mysqli->errno." Query : insert into campaigns_to(campaign_id,to_mobile_no,sms_text,sent_on,port_no) values('".$campaign_id."','".$to_mobile."','".$sms_text1."',now(),'".$available_port."'\n");
							}
                        }
                    } else {
