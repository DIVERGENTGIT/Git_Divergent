	...
	if($query->num_rows > 0)
	{
		...
		if(is_file($original_file)) {  
			if(file_exists($original_file)) {	 
				...
				$file_completepath2 = '';   
				$file_completepath3 = ''; 
				$process_table1 = 'schedule_campaigns_to_'.$campaign_id.'_1';   
				$process_table2 = 'schedule_campaigns_to_'.$campaign_id.'_2';   
				$process_table3 = 'schedule_campaigns_to_'.$campaign_id.'_3';   
								...
								if(strlen($mobile_no)>0 && $mobile_no!=0)
								{
									...
									if($maxrows <= 300000) { 
										...
										$datatoload = $campaign_id.",\"".$unique_num."\",".$message.",\"".$mobile_no."\",".date('Y-m-d H:i:s')."\n";         
										 $file_completepath1 = "/var/lib/mysql-files/load_data_".$campaign_id."_1.csv";
										 error_log($datatoload,3,$file_completepath1);  // track the logs for file data and path
									}else if($maxrows <= 600000) { 
										
										$datatoload = $campaign_id.",\"".$unique_num."\",".$message.",\"".$mobile_no."\",".date('Y-m-d H:i:s')."\n";         
										 $file_completepath2 = "/var/lib/mysql-files/load_data_".$campaign_id."_2.csv";
										 error_log($datatoload,3,$file_completepath2);  // track the logs for file data and path
									}else { 
										
										  $datatoload = $campaign_id.",\"".$unique_num."\",".$message.",\"".$mobile_no."\",".date('Y-m-d H:i:s')."\n";         
										  $file_completepath3 = "/var/lib/mysql-files/load_data_".$campaign_id."_3.csv";  
...
//  $rs = $mysqli->query($sqlstmnt);
$rs = $mysqli->prepare($sqlst);
