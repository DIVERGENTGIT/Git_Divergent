...
/*
$dlrdata=mysql_fetch_array(mysql_query("select count(*) as cnt from campaigns_to where error_code is null and campaign_id='$campaign_id' and to_mobile_no='$to' limit 1"));

if($dlrdata['cnt']!=0)
{
mysql_query("update campaigns_to set dlr_status='$dlr',error_code='$error_code',
error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) 
where error_code is null and campaign_id='$campaign_id' and to_mobile_no='$to'");
}
*/

			//if(mysql_affected_rows() < 1)fwrite($fp,"DLR.PHP ==> update campaigns_to set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where campaign_id='$campaign_id' and to_mobile_no='$to'\n");
	//exit;	
    } else if($file_name == "apidlr.php") {
        mysql_query("update sms_api_messages set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where message_id='$campaign_id' and to_mobileno='$to'");
			//if(mysql_affected_rows() < 1) fwrite($fp,"APIDLR.PHP ==> update sms_api_messages set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where message_id='$campaign_id' and to_mobileno='$to'\n");
		
    } else if($file_name == "reseller_dlr.php") {

    mysql_query("update sms_reseller.campaigns_to set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where campaign_id='$campaign_id' and to_mobile_no='$to'");
...
where error_code is null and campaign_id='$campaign_id' and to_mobile_no='$to'");
			...
			//if(mysql_affected_rows() < 1) fwrite($fp,"RESELLER.PHP ==> update sms_reseller.campaigns_to set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where campaign_id='$campaign_id' and to_mobile_no='$to'\n");
		
    } else if($file_name == "api_reseller_dlr.php") {
        mysql_query("update sms_reseller.sms_api_messages set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where message_id='$campaign_id' and to_mobileno='$to'");
			//if(mysql_affected_rows() < 1) fwrite($fp,"APIRESELLER.PHP ==> update sms_reseller.sms_api_messages set dlr_status='$dlr',error_code='$error_code',error_text='$error_txt',delivered_on=FROM_UNIXTIME($ts) where message_id='$campaign_id' and to_mobileno='$to'\n");
