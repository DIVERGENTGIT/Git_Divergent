...
jQuery.validator.addMethod("myEmail", function(value, element) {
  ...
  //var baseurl="http://10.10.10.199/smsstriker/";
// sms form save1
	$("#save").on("click",function(){
		...
		if($("#sms-form-save1").valid())
		{
			...
			var urlstr=baseurl+"smsequiry.php?noofsms="+noofsms+"&smstype="+smstype+"&subcription="+subcription+"&method_save=save";
////console.log(urlstr);			
...
//console.log(urlstr);
			$.ajax({
					...
					success: function(data){
								if(data.resend){
							$(".emsg").html('OTP Sent to Register Email');
	...
	$("#save2").click(function () {
		if($("#sms-form-save2").valid())
		{
			...
			var mobile=$("#mobile").val();
				var id=$('#userid').val();
				var urlstr=baseurl+"smsequiry.php?username="+name+"&email="+email+"&mobile="+mobile+"&userid="+id+"&save2";
			//console.log(urlstr);
			$.ajax({
					type: "GET",
					url: urlstr,
					...
					alert("OTP Sent to Register Email");
var urlstr=baseurl+"sendemail.php?sendotp=sendotp&pid="+id;
				//console.log(urlstr);
				// send email
				 $.ajax({
...
var localurl="http://10.10.10.112/shorturl/";
var serverurl="http://ion.bz/";
