...
echo json_encode(array("status"=>200,"otp"=>$generateotp));		
	...
	$query = "SELECT noofkeywords,pagetype FROM price_enquery where id = '".$_GET['userid']."'";
	$result = mysqli_query($link,$query) or die('Error query:  '.$query);
	$rec = mysqli_fetch_assoc($result);
 	$noofsms = $rec['noofkeywords'];
	$source = $rec['pagetype'];
...
$noofkeywords = mysqli_real_escape_string($link,$_GET['noofkeywords']);
$servicetype = mysqli_real_escape_string($link,$_GET['servicetype']);
$subcription = mysqli_real_escape_string($link,$_GET['subscription']);
$replytype = mysqli_real_escape_string($link,$_GET['replaytype']);

$pageType = '';
if($servicetype == 'Dedicated') {
	$pageType = 'home-dedicated';
}else if ($servicetype == 'Shared') {
	$pageType = 'home-shared';
}  


$insert_query = "INSERT INTO `price_enquery` (`noofkeywords`, `servicetype`,`replytype`, `subcription`,`pagetype`) VALUES
 ('$noofkeywords','$servicetype','$replytype','$subcription','$pageType');";
