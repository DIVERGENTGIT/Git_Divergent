...
exit;
 ...
 $rowsPerPage = 3; //how many rows to show per page
	...
	if(!empty($search)):
		$category="select Count(id) AS numrows from fee_receipt where roll_no='$search' or app_no='$search' order by id desc";
	else:
...
$result = mysql_query($category) or die('Error, query Failed');
	...
	$lastPage = $maxPage;
      ...
      for($page_ctr = $startPage; $page_ctr <= $lastPage; $page_ctr++)
      {
         ...
         {
            $nav .= " <a href=\"$self?page=$page_ctr\" style=' text-decoration:none;' >$page_ctr</a> ";
         } 
      }
// Creating Previous and next link and the link to go straight to the first and last page
	...
	{
		$page = $pageNum - 1;
		$prev = "<a href=\"$self?page=$page\" style=' text-decoration:none;' >Prev</a>";
		$first = " <a href=\"$self?page=1\" style='text-decoration:none;'>First</a> ";
	}
	else
	{
...
<table width="950" border="0" align="center" cellpadding="0" cellspacing="0">
                    ...
                    <td height="408" colspan="2" valign="top" style="background:url(images/middlebg3.jpg) repeat-x;border-left:#942e2a 2px solid; padding:10px;">
                      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                        <tr>
                         ...
                         <tr> <td  colspan="5"   width="350"  ><span style="float:left;"><form method="post" name="searchform" action="<?php echo $_SERVER['PHP_SELF'];?>"><input type="text"   style=" width: 250px;
 ...
 <?php
if(!empty($search)):
$query="select * from fee_receipt  where roll_no='$search' or app_no='$search' order by id desc LIMIT ".$offset.", ".$rowsPerPage;
else:
$query="select * from fee_receipt order by id desc LIMIT ".$offset.", ".$rowsPerPage;
									...
									{ 	$i = 0;
                                     ...
                                     ');"> Receipt1</a>|
                                       <a href="JavaScript:newPopup
                                      ('http://www.smsstriker.com/mahindra/admin/receipt.php?vid=<?php echo $fee_row['id']; ?>
                                     ');">
                                   Receipt2</a>|<a href="JavaScript:newPopup
                                      ('http://www.smsstriker.com/mahindra/admin/feereceipts_old.php?vid=<?php echo $fee_row['id']; ?>
                                     ');">
                                  Receipt3</a>|
                                    
									 
                                      <!--<img src="images/view.png" alt="view" width="20" height="20">-->

								</td>
                                      <td align="center" valign="middle" bgcolor="#f7f7f7"><a href="editfee.php?id=<?php echo $fee_row['id']; ?>"  ><img src="images/edit.png" width="20" height="20" alt="edit"></a></td>
                                      
                                      <td align="center" valign="middle" bgcolor="#f7f7f7"><a href="?del=<?php print $fee_row['id']; ?>"   onClick="return confirm('Are you sure you want to delete ..?')"><img src="images/del.png" alt="delete" width="20" height="20"></a></td>
                                    </tr>
									 <?php
									 }
									 }
