					...
					if($pgresponse=='Transaction Successful')
					 {  
						 $on_date=date("Y-m-d H:i:s");
						   ...
						   $query1="INSERT INTO `user_payments` ( `user_id`, `no_of_sms`, `price`, `amount`, `service_tax`, `service_tax_percent`, `total_amount`, `on_date`, `payment_type`, `transaction_id`) VALUES ('$registeruser_id', '$noofsms', '$smsprice', '$amount', '$tax_amount', '$ServiceTax', '$total_amount','$on_date',1,$trn_id)"; 
...
$getServiceDetails = "SELECT  * FROM longcode_subscription WHERE user_id = '".$registeruser_id."' and longcode_number IN ($longcode_numbers) and status = 1 AND transaction_id ='".$trn_id."' limit 1";    
$getServiceDetails_res = mysqli_query($link,$getServiceDetails);  
$data_array = array();
while($data = mysqli_fetch_array($getServiceDetails_res)) {  
	$data_array[] = $data;
		  
//longcode_number,longcode_type,no_of_sms,subscription_duration,number_cost,
//package_cost, amount,price_per_long_code
}
 
 //print_r($data_array);exit;
  send_invoice_generator($invoice_id,$epg_txnID,$created_on,$registeruser_id,$smstype,$payment_id,$organization,$name,$username,$on_date,$noofsms,$smsprice,$amount,$ServiceTax,$service_tax,$total_amount,$longcode_numbers,$data_array); 
...
function send_invoice_generator($invoice_id,$epg_txnID,$created_on,$registeruser_id,$smstype,$payment_id,$organization,$name,$username,$on_date,$no_of_sms,$price_per_sms,$amount,$ServiceTax,$service_tax,$total_amount,$longcode_numbers,$serviceData)
{	 
  	...
  	for($i=0;$i<count($serviceData);$i++){
        ...
        global $invoice_url;
 $url = $invoice_url."invoice_code/longcodeRenewal.php";  
  	$parameters = "tn=$epg_txnID&invoice_id=$invoice_id&cd=$created_on&rid=$registeruser_id&smstype=$smstype &payment_id=$payment_id&od=$on_date&org=$organization&name=$name&st=$ServiceTax&amt=$amount&noofsms=$no_of_sms&total_amount=$total_amount&prices=$price_per_sms&username=$username&tax_amt=$service_tax&longcode=$longcode_numbers&service=$serviceData";
 	//print_r($array);exit;
	$ch = curl_init();  
