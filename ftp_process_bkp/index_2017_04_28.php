<?php 
include("/var/www/html/strikerapp/ftp_process/dbconnect/config.php");   
...
function getCampaignData() { 
	...
	require('/var/www/html/strikerapp/ftp_process/spreadsheet-reader-master/php-excel-reader/excel_reader2.php');
	require('/var/www/html/strikerapp/ftp_process/spreadsheet-reader-master/SpreadsheetReader.php');
	global $mysqli;    
	$acount_column = 0;
	$mobile_no_column = 1; 
	$text_column = 2; 
	...
	$mysqli->query("UPDATE ftp_campaign SET status = '2' WHERE campaign_id = '".$campaign_id."'");  
		...
		$total_rows = intval(shell_exec("wc -l $file_path"));  
  		...
  		$checkpath1 = "/var/lib/mysql-files/campaign_data/act_campaign_to_".$campaign_id."_1.csv";  
		$checkpath2 = "/var/lib/mysql-files/campaign_data/act_campaign_to_".$campaign_id."_2.csv"; 
		$checkpath3 = "/var/lib/mysql-files/campaign_data/act_campaign_to_".$campaign_id."_3.csv"; 
			...
			foreach ($Spreadsheet as $Key => $Row) // take a single spreadsheet read each line in excel sheet
			{
				...
				if($uploded_data[$i][$mobile_no_column] > 0)
				{
					...
					if(strlen($mobile_num) > 0 && $mobile_num != 0)
					{
						...
						if($max_row <= 300000) {
						//if($max_row <= 1000) {               // If num_of_rows <= 3 lakh creates one csv file   
						...
						//}elseif($max_row <= 600000) {  
						}elseif($max_row > 300000 && $max_row <= 600000) {           // If (num_of_rows > 3 lakh && rowcount <= 6 lakh ) creates two csv file   
							$datatoload = $campaign_id.",\"".$account_num."\",".$mobile_num.",\"".$sms_text."\",".date('Y-m-d H:i:s')."\n";         
...
$unicode_sms = "&coding=0&charset=utf-8";
// $unicode_sms = "&coding=0&charset=ISO-8859-1";
		...
		/*	switch ($userActType)
			...
			if($process_table1_status == 0)
			{
				$table_count += 1;
				$table_1 = $process_table1;
				
			}
			if($process_table2_status == 0 ) 
			{
				$table_count += 1;
				$table_2 = $process_table2;
 
			}
			if($process_table3_status == 0)
			{
				$table_count += 1;
...
//if($dlr_status!=11){
 $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender)."&to=91$to_mobile&text=".urlencode($sms_text1);										$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("http://182.18.139.110/API/DLRS/ftp_dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");										 http_send($URL,$available_port);
...
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender)."&to=91$to_mobile&text=".urlencode($sms_text1);
								$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("http://http://182.18.139.110/API/DLRS/ftp_dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
