...
{	
  ...
  include("/var/www/html/strikerapp/smslib/config.inc");
	...
	global $SERVER_NAME; 
	 ...
	 $campaign_rs = $mysqli->query("select * FROM ftp_campaign WHERE status = '2' AND (process_table1_status = 0 AND process_table2_status = 0 AND process_table3_status = 0) LIMIT 1"); 	
	...
	{    
		...
		{	
		   ...
		   $mysqli->query("UPDATE ftp_campaign SET status = '4' WHERE campaign_id = '".$campaign_id."'");   		
			...
			$userquery = $mysqli->query("select username,no_ndnc,dnd_check,template_check,available_credits from users where user_id = '".$campaign_details_val['user_id']."' "); 
			...
			$before_process_credits = $available_credits = $ndnc['available_credits'];
			...
			if($no_ndnc == 1 && $dnd_check == 1) {  // promotional

				$available_port_val = $mysqli->query("SELECT sending_port_no FROM sms_queue where application_priority='ACTSR' ");
				//$available_port = $available_port_val->fetch_array(MYSQLI_ASSOC);
				while($available_port = $available_port_val->fetch_array(MYSQLI_ASSOC)) {
					$availablePorts[] = $available_port['sending_port_no'];
				}   
			}else  {  
				$available_port_val = $mysqli->query("SELECT sending_port_no FROM sms_queue where application_priority='ACTTR' ");  
				while($available_port = $available_port_val->fetch_array(MYSQLI_ASSOC)) {
					$availablePorts[] = $available_port['sending_port_no'];
				}
			}
			...
			if(isset($table_name)) { 
				...
				/*if($user_id==4857){
		                        ...
		                        $available_port = 51113;
		                        }elseif($i == 2) {
		                        //      $available_port = 44313;
		              		  $available_port = 52113;
		                        }else{
		                //      $available_port = 44413;
		                                $available_port = 53113;
		                        }
				...
				$check_table_exixts = $mysqli->query("SELECT * FROM $table_name ");       
	 			...
	 			{        
					...
					{  
						/** Port Distribution **/
						$csv_file_size = $to_details_rs->num_rows;
						$totalPorts = count($availablePorts);
  						$kennelLength = ceil($csv_file_size/$totalPorts);  
...
$sms_length_tmp=ceil(strlen($sms_text)/160);
			...
			{  
				if($no_ndnc == 0){
					$senderName_kennel = "BA-611128";
					 ...
					 //Checking Credits 
					if($available_credits >= $sms_length_tmp) {
						$available_credits = $available_credits-$sms_length_tmp;
  ...
  $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($senderName_kennel)."&to=91$to_mobile&text=".urlencode($sms_text1);
...
$error_text='NULL';
							...
							{								
								...
								$values = "";     
						     	 ...
						     	 $mysqli->query("insert into  ftp_campaigns_to  (campaign_id,acccount_num,to_mobile_no,sms_text,sms_count,sent_on,dlr_status,error_text,port_no) values ('$campaign_id','$account_num','$to_mobile','$sms_text1','$sms_length_tmp',now(),'11','Insufficient Credits','$available_port')");    
						     }    
...
$mysqli->query ($final_query);
					...
					if($no_of_messages_tmp == $no_of_messages)    
					{
						...
						if(isset($table_1)) {
							...
							//Drop the table after process completion of each table
							$mysqli->query("DROP TABLE $table_name ");
						  
					}else{
						//If campaign process fails, sending an sms alert to support team
		   ...
		   //Updatin status that campaign process is completed successfully
		  ...
		  if($no_of_messages_tmp == 0) {   
			...
			//Updating campaign status, whenever campaign process is incomplete 
		        $mysqli->query("UPDATE ftp_campaign SET status = '2' WHERE campaign_id = '".$campaign_id."'");   

		   }else{
		   	 $mysqli->query("INSERT INTO ftp_process_credits_log (user_id,campaign_id,before_process_credits,after_process_credits) VALUES($user_id,$campaign_id,$before_process_credits,$available_credits) ");
		   	
		   	$mysqli->query("update users set available_credits = $available_credits where user_id='$user_id'");
		   }	  
