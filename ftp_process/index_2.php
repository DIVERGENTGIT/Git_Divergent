...
if($user_id=='4904')
{
			$available_port_val = $mysqli->query("SELECT sending_port_no FROM sms_queue where application_priority='LS2' order by queued, sent asc limit 1");
...
//if($dlr_status!=11){
 $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender)."&to=91$to_mobile&text=".urlencode($sms_text1);										$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("http://182.18.165.185/ftp_dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");										http_send($URL,$available_port);
