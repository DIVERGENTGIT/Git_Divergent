...
$check_file_exists = $mysqli->query("SELECT count(*) as cnt FROM `ftp_campaign` where `csv_file` = '".$file_path."' AND date(`created_date_time`)=date(now())");     
				 	...
				 	$count = 0; 	 	
				 	//if($size > 600000) { 
					if($size > 1000000) {  // If filesize is greater than 6 lakh create three dynamic tables 
						$count = 3;   
						    
					}elseif($size > 600000) {  // If filesize is greater than 3 lakh create two dynamic tables 
   						$count = 2;
					}else{  
			...
			foreach ($Spreadsheet as $Key => $Row) // take a single spreadsheet read each line in excel sheet
			{
				...
				if($uploded_data[$i][$mobile_no_column] > 0)
				{
					...
					if(strlen($mobile_num) > 0 && $mobile_num != 0)
					{
						...
						//if($max_row <= 300000)
						if($max_row <= 600000) {               // If num_of_rows <= 3 lakh creates one csv file   
...
//	}elseif($max_row <= 600000) {  
						}elseif($max_row > 600000 && $max_row <= 1000000) {           // If (num_of_rows > 3 lakh && rowcount <= 6 lakh ) creates two csv file   
							$datatoload = $campaign_id.",\"".$account_num."\",".$mobile_num.",\"".$sms_text."\",".date('Y-m-d H:i:s')."\n";         
...
{	
	...
	include("/var/www/vhosts/www.rkadvertisings.com/htdocs/smslib/functions2.inc");
...
//if($dlr_status!=11){
 $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender)."&to=91$to_mobile&text=".rawurlencode($sms_text1);										$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("http://182.18.165.185/ftp_dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");										http_send($URL,$available_port);
...
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender)."&to=91$to_mobile&text=".rawurlencode($sms_text1);
