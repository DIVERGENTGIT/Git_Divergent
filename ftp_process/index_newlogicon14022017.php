...
{	
	...
	//$campaign_rs = $mysqli->query("select * FROM ftp_campaign WHERE status = '2'"); 
	 $campaign_rs = $mysqli->query("select * FROM ftp_campaign WHERE status = '2' AND (process_table1_status = 0 OR process_table2_status = 0 OR process_table3_status = 0)"); 	
	if($campaign_rs->num_rows > 0 ) 
	{
		...
		$unicode_sms = "&coding=2&charset=Latin1";
		...
		// $available_port = 48113;
				...
				$check_table_exixts = $mysqli->query("SELECT * FROM $table_name ");       
	 			...
	 			{        
					...
					{  
						$no_of_messages = $to_details_rs->num_rows; 
