...
function createCampaign() { 
	...
	global $mysqli;  
 	$dir ='';
	$get_ftp_users = $mysqli->query("SELECT user_id,no_ndnc,dnd_check,available_credits FROM users WHERE is_ftp =1");  

	while ($ftpUsers = $get_ftp_users->fetch_array(MYSQLI_ASSOC)) {

		$user_id= $ftpUsers['user_id'];
		$available_credits= $ftpUsers['available_credits'];
 	...
 	if($available_credits <= 0) {
 		//$mysqli->query("INSERT INTO ftp_process_credits_log (user_id) VALUES($user_id) ");
   	}else{		
				...
				if($uploded_data[$i][$mobile_no_column] > 0)
				{
					...
					if(strlen($mobile_num) > 0 && $mobile_num != 0)
					{
						...
						if($max_row <= 300000) { 
							...
							$values .= "('".$campaign_id."','".$account_num."','".$mobile_num."','".$sms_text1."','".date('Y-m-d H:i:s')."'),";   
