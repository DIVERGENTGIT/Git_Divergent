	...
	{    
		...
		{	
		   ...
		   $mysqli->query("UPDATE campaigns SET campaign_status = '4' WHERE campaign_id = '".$campaign_id."'");   		
			...
			$available_credits = $ndnc['available_credits'];
			  
			$logName = $user_id.'_'.date('Y-m-d');
			error_log("\n".date('Y-m-d H:i:s')."| Before Process User Available Credits  |".$available_credits."\r\n",3,"/var/www/html/FTPCampaign_Log/CreditsLog/query_".$logName.".log");   
...
$error_text='NULL';      
	...
	$dlr_status=11;
	$error_text='Insufficient Credits'; 
}
...
if($available_credits >= $sms_length_tmp) {
	...
	$error_text='Insufficient Credits'; 
			...
			if($available_credits >= $sms_length_tmp) {
				$creditsDeductionLog = "update users set available_credits = $available_credits where user_id='$account_num'";  
				$logName = $account_num.'_'.date('Y-m-d');
				error_log("\n".date('Y-m-d H:i:s')."| User Available Credits $available_credits |".$creditsDeductionLog."\r\n",3,"/var/www/html/FTPCampaign_Log/CreditsLog/query_".$logName.".log"); 
				  
				$available_credits = $available_credits - $sms_length_tmp;
				$mysqli->query("update users set available_credits = $available_credits where user_id='$account_num'");
			}    
