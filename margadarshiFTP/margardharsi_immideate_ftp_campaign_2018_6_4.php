	 			...
	 			{        
					...
					{  
						$no_of_messages = $no_of_messages+$to_details_rs->num_rows; 
 					  	$insertQuery="insert into campaigns_to  (campaign_id,unique_msg_id,user_id,to_mobile_no,sms_text,sms_count,sent_on,dlr_status,error_text,port_no) values ";    
			 			$values="";
			 			$offset=0;  
...
$sms_length_tmp=ceil(strlen($sms_text)/160);
			...
			if(!$checked) 
							...
							if($is_block_listed)
							{   
								$values .= "('$campaign_id','$uniqueNo','$account_num','$to_mobile','$sms_text1','$sms_length_tmp',now(),'2','Block Listed Number','$available_port'),";  
							}else if(!$isValidTemplate)
						{
 							
							$values .= "('$campaign_id','$uniqueNo','$account_num','$to_mobile','$sms_text1','$sms_length_tmp',now(),'13','Not a valid Template','$available_port'),";  
						}elseif($is_invalid_no)
							{								
							
								$values .= "('$campaign_id','$uniqueNo','$account_num','$to_mobile','$sms_text1','$sms_length_tmp',now(),'16','Invalid Number','$available_port'),"; 
							} else {  
								...
								if($ndnc['dnd_check'])
								{
									...
									if($isDND)
									{
										...
										$values .= "('$campaign_id','$uniqueNo','$account_num','$to_mobile','$sms_text1','$sms_length_tmp',now(),'3','DND Number','$available_port'),"; 
									} else {
...
$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("http://www.smsstriker.com/API/DLRS/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");

http_send($URL,$available_port);    
$dlr_status=0;
$error_text='NULL';      
 
										$values .= "('$campaign_id','$uniqueNo','$account_num','$to_mobile','$sms_text1','$sms_length_tmp',now(),'$dlr_status','$error_text','$available_port'),";
