...
//echo file_get_contents("http://www.test.hyderabadwater.gov.in/ppmp/PPMReceiver.aspx");  
require($Email_lib);
...
case 'sms':
     ...
     send_email_admin($name,$email,$mobile,$noofsms,$months,$smstype);
...
if($_GET['sendotp']=='sendotp') { 
	...
	send_email_otp($name,$email,$mobile,$otp);
	if($_GET['source'] == 'landingPage') {  
		send_leadInfo_to_sales($name,$mobile,$email,$noofsms,$pagetype,$base_url);
		//send_ppc_email_admin($name,$email,$mobile,$noofsms);	
	}
}   
...
function send_ppc_email_admin($name,$email,$mobile,$noofsms) {
...
$mail->AddAddress("sales@smsstriker.com", "SMSSTRIKER");
$mail->WordWrap = 50;
$mail->IsHTML(true);  
$mail->Subject = "New PPC Lead";
 ...
 $message = '<html><head><link href="https://fonts.googleapis.com/css?family=Lato:300,400,700" rel="stylesheet">
	...
	<div style="width:100%;max-width:600px;float: left;">
	       <p> Dear Team,</p></br>
	        ...
	        <p>Here are the new lead information.</p>
		<p>Name : '.$username.'</p>	
		<p>Email : '.$email.'</p>
		<p>Mobile : '.$mobile.'</p>
		<p>No of SMS : '.$noofsms.'</p>
...
function send_leadInfo_to_sales($name,$number,$email,$noofsms,$source,$base_url) { 
	...
	{     
		...
		{ 
			...
			$toNumbers = '9985257711';//'8501822227';
...
function send_email_admin($name,$email,$mobile,$noofsms,$months,$type) {
...
//echo $message_plans;     <p>Request for'.$demo.'</p> on line 804
//exit;
...
<thead>
<tr style="background:#00b8b6;"><td rowspan="2" style="text-align:center;-moz-border-top-left-radius: 10px;-webkit-border-top-left-radius: 10px;border-top-left-radius: 10px;">SMS Pack Range</td><td style="text-align:center;" >Price Per SMS (<span class="indian-rupee"><img style="width: 9px;margin-left: 2px;" src="http://strikersoftsolutions.com/images/firstring-email/rupee-indian-white.png"  alt=""></span>) </td><td rowspan="2" style="text-align:center;-moz-border-top-right-radius: 10px;-webkit-border-top-right-radius: 10px;border-top-right-radius: 10px;">&nbsp;</td></tr>
...
<div class="halfwidth03 text-right" style="width:100%;max-width: 200px;float:right;">          </div>
</div>
<div class="fullwidth" style="width:100%;float:left;padding-left:15px;padding-right:15px;">
					...
					//if($pgresponse=='Transaction Cancelled')
					 {
						 ...
						 $query1="INSERT INTO `user_payments` ( `user_id`, `no_of_sms`, `price`, `amount`, `service_tax`,`service_tax_percent`, `total_amount`, `on_date`, `payment_type`, `transaction_id`,`added_by`,`note`)  VALUES ('$registeruser_id', '$noofsms', '$smsprice', '$amount', '$tax_amount', '$ServiceTax', '$total_amount','$on_date',1,$trn_id,'1','Payment Gateway')";
...
$mail->Password = "Str!ker@123"; // SMTP password
$mail->FromName = "smsstriker";
$mail->SetFrom('support@smsstriker.com', 'User Payment');

//$mail->AddReplyTo($email, $name); // indicates ReplyTo headers
$mail->AddAddress($email, $name);
$mail->AddBCC("supraja.thanga@smsstriker.in", "SMS Payment");
...
$mail->AddBCC("accounts@smsstriker.com", "SMSSTRIKER");
$mail->AddBCC("naveen@smsstriker.com", "SMSSTRIKER");
$mail->AddBCC("naveen@strikersoft.in", "SMSSTRIKER");
...
/*if(isset($_GET['accondDetails'] == 1)) {
 	$trnID = $_REQUEST['trnID'];
	$query =  "SELECT pe.registeruser_id FROM transaction_history th INNER JOIN price_enquery pe on pe.epg_txnID=th.epg_txnID WHERE th.epg_txnID = '".$trnID."' group by th.payment_id order by th.payment_id desc limit 1"; 
	$result = mysqli_query($link,$query);
	if(mysqli_num_rows($result) > 0)
	{
		while($value = mysqli_fetch_assoc($result)) {
			$userID = $value['registeruser_id'];
			$getUserData = "SELECT username,accountName,accountPwd,email FROM users WHERE user_id = '".$userID."'";
			...
			while($userDataRes = mysqli_fetch_assoc($getUserDataRes)) {
				...
				$accountName = $userDataRes['accountName']; 
				$accountPwd = $userDataRes['accountPwd']; 
				send_email_user_account_details($name,$email,$accountName,$accountPwd);
			}
		}
...
function send_email_user_account_details($name,$email,$accountName,$accountPwd){ 
	$username=ucfirst($name); 
	$mail = new PHPMailer();
	...
	//$mail->AddReplyTo($email, $name); // indicates ReplyTo headers
	$mail->AddAddress($email, $name);
	$mail->AddBCC("sandeepthicse@gmail.com", "login Details");
	//$mail->AddBCC("supraja.thanga@smsstriker.in", "SMS Payment");
	//$mail->AddBCC("accounts@smsstriker.com", "SMS Payment");
	//$mail->AddBCC("naveen@smsstriker.com", "SMS Payment");
	$mail->WordWrap = 50;
