...
$this->product->update_price_rowid($product['id'],$noofsms,$price,$amount,$tax_amount,$total_amount,$product['cartid']);
                        ...
                        <td>
     <form method='post' action="<?php echo base_url()?>index.php/products/addToCart">   
     ...
     <input type ="button" class="btn btn-sm btn-default cart-first-form addtocart<?php echo $product['id'] ?>" value="Add to cart" />
                         ...
                         <td><input type ="button" class="btn btn-sm btn-default cart-first-form" id="cartremove<?php echo $product['id'] ?>" value ="Remove" /></td>
                        </tr>
	...
	$.ajax({
		method: "POST",
			...
			success: function(data){
				...
				window.location=baseurl+"index.php/products/index";
			},
			error: function(){
...
$(".qty<?php echo $product[id] ?>").on("blur",function(){
		...
		*/
		var id=$("#productid<?php echo $product['id'] ?>").val();
		var baseurl="<?php echo base_url(); ?>";
		var urlstr=baseurl+"index.php/products/updateprice";	
		...
		url: urlstr,
		dataType:"json",
		data:{pid:id,noofsms:noofsms,update:"update",cartid:cartid,tax:tax},
		success: function(data){
...
$(".addtocart<?php echo $product['id'] ?>").on("click",function(){
		...
		*/
		var id=$("#productid<?php echo $product['id'] ?>").val();
		var baseurl="<?php echo base_url(); ?>";
		var urlstr=baseurl+"index.php/products/addToCart";	
		var cartid="<?php echo $product['cartid'] ?>";
		var tax="<?php echo $tax; ?>";
		//alert(urlstr);
		...
		if(noofsms>499)
		{
			...
			url: urlstr,
			dataType:"json",
			data:{id:id,qty:noofsms,update:"update",cartid:cartid,tax:tax,description:description},
			success: function(data){
