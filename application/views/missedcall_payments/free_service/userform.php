...
?>
			...
			//execute post
			$did_result = curl_exec($did_conn);
			$getsilver_didlist_api_data=json_decode($did_result, true);
		 	curl_close($did_conn);
			$rs =$getsilver_didlist_api_data;
			if(count($getsilver_didlist_api_data)>0)
			{
			 $freeservice =$getsilver_didlist_api_data;
			}
			else
			{
			$freeservice=array();
...
//print_r($freeservice);
if(@$freeservice['message']!='used')
{
?>
<div class="col-md-12 col-sm-12 col-xs-12 btnhidebuy padding_zero">
<input type="button" value="Subcription" class="submit_btn buy-btnhide add_mrnumb neworderbtn" style="display:none;" >
</div>
...
?>
		...
		<div class="col-md-12 col-sm-12 col-xs-12 padding_zero">
		<div class="col-md-12 col-sm-12 col-xs-12 padding_zero">
<input type="button" name="" value="Free Service Confirm" class="add_mrnumb submit_btn userlogin">
...
<span class="servicenummsg"></span>
		...
		<span  class="passwordmsg" style="float:right;"> </span>
		</div>
		
		<div class="col-md-12 col-sm-12 col-xs-12 padding_zero">
<input type="button" name="" value="Free Service Confirm" class="add_mrnumb submit_btn userregistration">
		</div>
...
// user registration start
	...
	console.log(did_numbers);
						...
						if(response.message!='User Name already in use')
						{
							...
							if(response.message!='Mobile number already in use')
						       ...
						       $(".mobilemsg").html('');
								...
								$.ajax({
								type:"GET",
								url:"<?php echo base_url()?>Free_service/userregistration",
								data:{
								...
								{
									$(".servicenummsg").html(responsedata.message);
								...
								$(".servicenummsg").html(responsedata.message);
								$(".servicenummsg").css('color','green');
								
								$.ajax({
								type:"GET",
								url:"<?php echo base_url()?>Free_service/sendemail",
								data:{
								sendemail:"sendemail",
								user_id:user_id,
								username:username,
				...
				{
				 $(".usernamemsg").html("Please enter username");
