...
$( document ).ready(function( $ ) {
	var trn_id=  "<?php echo $_REQUEST['tn']?>";
       ...
       var urlvar="<?php echo base_url(); ?>API/longcode/invoice_shared.php?payment=payment&trn_id="+trn_id+"&servicetax="+servicetax+"&tax="+tax+"&smsprice="+smsprice;
...
<div class="col-sm-12 col-md-12 col-xs-12 form-div padding-zero">
<div class="col-sm-3 col-md-3 col-xs-12 padding-zero">
<label class="form_lable">No of Keywords</label>
</div>
<div class="col-sm-7 col-md-7 col-xs-12  padding-ltzero padding-mzero">
<?php
$confirm_nos=@$_SESSION['confirm_nos'];

/*
$sql2="select sum(no_of_sms) as no_of_sms,sum(amount) as amount,sum(total_tax) as total_tax,
sum(total_amount) as total_amount,sum(no_of_keywords) as no_of_keywords,
price_per_long_code,subscription_duration	
 from longcode_tmp where status=1 and service_type='shared' and user_id=$user_id and longcode_number in ($confirm_nos)";
 */
 
$sql2="select * from longcode_tmp where service_type='shared'
and status=1 and user_id=$user_id group by longcode_number";
$query_result=$this->db->query($sql2)->result();
...
foreach($query_result as $key=>$longcode)
{
$amount += $longcode->amount; // package cost and number cost
$total_tax += $longcode->total_tax;
$total_amount += $longcode->total_amount;

if($longcode->no_of_keywords!='')
{
echo $noofkeywords += $longcode->no_of_keywords;
}

$subscription_duration=$longcode->subscription_duration;
...
{
//echo "test";
redirect(base_url()."longcode/shared");
}
?>
</div>
</div>
...
<?php
     ...
     if($subscription_duration=='3 Months')
      ...
      if(@$_SESSION['confirm_nos']!='')
      {
		 ...
		 $sql1="select * from price_enquery  where registeruser_id=$user_id and longcode_numbers='$longcode_numbers' and payment_status=0 and description='shared' group by longcode_numbers";
	 ...
	 '$amount','$total_tax','$total_amount','$longcode_numbers','shared')";
		 ...
		 {
			$ids=$value->id;
			$sql1="update price_enquery set noofkeywords=$noofkeywords,price_per_sms='$price_per_long_code',subcription=$subscription,
			amount='$amount',tax_amount='$total_tax',total_amount='$total_amount' where id=$ids";
			$this->db->query($sql1);
		 }
...
$this->session->set_userdata('qty',$noofsms);
$this->session->set_userdata('sms_price',$price_per_long_code);
$this->session->set_userdata('amount',$amount);
$this->session->set_userdata('tax_amount',$total_tax);
...
<input type="hidden" name="qty" value="<?php echo $noofsms=0;?>">
...
<input type="hidden" name="sms_price" value="<?php echo $price_per_long_code=0;?>">
...
$did_nos=@$_SESSION['confirm_nos'];
$user_id=$this->session->userdata('user_id');
$sql3="update  longcode_tmp set  status=0 where status=1 and service_type='shared' and user_id=$user_id and longcode_number in ($did_nos)";
$this->db->query($sql3);
