...
$( document ).ready(function( $ ) {
	var trn_id=  "<?php echo $_REQUEST['tn']?>";
       ...
       var urlvar="<?php echo base_url(); ?>API/longcode/invoice_dedicated.php?payment=payment&trn_id="+trn_id+"&servicetax="+servicetax+"&tax="+tax+"&smsprice="+smsprice;
...
price_per_long_code,subscription_duration	
 from longcode_tmp where status=1 and service_type='dedicated' and user_id=$user_id and longcode_number in ($confirm_nos)";*/

$sql2="select * from longcode_tmp where service_type='dedicated'
...
if($longcode->no_of_sms!='')
{
$noofsms += $longcode->no_of_sms;
}
if($longcode->no_of_keywords!='')
{
$noofkeywords += $longcode->no_of_keywords;
}
...
{
//echo "test";
redirect(base_url()."longcode/dedicated");
}
?>
</div>
</div>
...
<?php
     ...
     if($subscription_duration=='3 Months')
      ...
      if(@$_SESSION['confirm_nos']!='')
      {
		 ...
		 $sql1="select * from price_enquery  where registeruser_id=$user_id and longcode_numbers='$longcode_numbers' and payment_status=0 and description='dedicated' group by longcode_numbers";
...
$noofkeywords=0;
	 ...
	 '$amount','$total_tax','$total_amount','$longcode_numbers','dedicated')";
