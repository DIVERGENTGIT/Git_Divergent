...
<div class="col-md-12 col-sm-12 col-xs-12 padding_zero">
<h3 class="right-content-title"><img src="<?php echo base_url();?>images/payments-icon.png" class="right-title-img">Longcode Package Reports</h3>
</div>
<div class="col-md-12 col-sm-12 col-xs-12 padding_zero">
<div class="col-md-12 col-sm-12 col-xs-12 padding_zero">
			...
			<?php 
				...
				foreach($userlongcodes as $serviceData) {  $count++; ?>
					<tr id="<?php echo $serviceData['longcode_id'];?>">
						...
						<td><?php if($serviceData['no_of_sms']>0) {echo $serviceData['no_of_sms'];}else { echo "0";}?></td>
						<td><?php if($serviceData['used_incoming_sms']>0) {echo $serviceData['used_incoming_sms'];}else { echo "0";}?></td>
						<td><?php
						 $available=$serviceData['no_of_sms']-$serviceData['used_incoming_sms'];
...
<script>
 ...
 //console.log(data);
			var result = $.parseJSON(data);   
 			if(result['userpackage'][0].longcode_type == 'SILVER') {
				number_cost = 0;
			}else if (result['userpackage'][0].longcode_type == 'GOLD') {
				number_cost = 500;

			}else if(result['userpackage'][0].longcode_type == 'PLATINUM') {
				number_cost = 1000;
			}
  		if(result['userpackage'][0].service_type == 'shared' ) {
				$('#service_number_cost'+longcode_id).text('0'); 
			}else{
						$('#service_number_cost'+longcode_id).text(number_cost); 
			}  
	 ...
	 var number_cost = 0; var no_of_keywords = 0;	
 	 $.ajax({
		...
		{  
			 console.log(data);
			 var result = $.parseJSON(data);  
			 if(result['package'][0].subscription_duration  == '1 Month' ) {
				var duration = '1';
			 }else if(result['package'][0].subscription_duration == '3 Months') {
				var duration = '3';
			 }else if(result['package'][0].subscription_duration == '6 Months') {
				var duration = '6';
			 }else if(result['package'][0].subscription_duration == '1 Year') {
				var duration = '12';
			 }
			if(result['userpackage'][0].longcode_type == 'SILVER') {
				number_cost = 0;
 		 	 ...
 		 	 if(result['userpackage'][0].service_type == 'shared' )
			 {
 				...
 				//console.log(no_of_keywords);
				 // number_price = parseInt(number_cost)*parseInt(duration)*parseInt(no_of_keywords);
				  number_price = '0';
			 } else{
		
				   number_price = parseInt(number_cost)*parseInt(duration);
			}  

			 $('#tax_amount'+longcode_id).val();
	  	 	 $('#grand_total'+longcode_id).val();
			 var amount = parseInt(result['package'][0].amount) + parseInt(number_price);
			 ...
			 $('#service_amount'+longcode_id).val(amount);
			...
			$('#number_cost'+longcode_id).val(number_price);
			var service_total = parseInt(total);  
			 $('#service_total_amount'+longcode_id).val(service_total);   
			$('#no_of_keywords'+longcode_id).val(no_of_keywords);

			$('#service_number_cost'+longcode_id).text(number_price); 
...
<span class="form_lable03">Current Package :</span>
</div>
