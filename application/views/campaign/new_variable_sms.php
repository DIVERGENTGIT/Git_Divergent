...
<?php 
	...
	echo form_open('campaign/newVariableSMS',
    	$form_attributes = array('id' => 'variable_sms_file_upload', 'class' => 'missedcall_allform', 'name' => 'variable_sms_file_upload', 'method' => 'post')
	); 
?>
 ...
 <div class="col-sm-12 col-md-12 col-xs-12 form-div padding_zero">
    <div class="col-sm-2 col-md-2 col-xs-12 tbresp-label padding_zero">
                   ...
                   <div class="col-sm-7 col-md-7 tbresp-rt padding_mzero col-xs-12">
				    <div class="col-sm-12 col-md-12 col-xs-12 padding_zero">
					<?php echo form_textarea(array('name' => 'sms_text','onKeyup' => "calculateSMSLength()", 'id' => 'sms_text', 'placeholder' => 'Type here', 'rows' => 4, 'cols' => 30, 'class' => '', 'value' => set_value('sms_text')));?>
</div>
           <div class="col-sm-12 col-md-12 col-xs-12 padding_zero"> 
...
<li>Please upload xls,xlsx filetypes and size must be less than 30MB</li>
</ul>
 </div>
...
<div class="col-md-12 col-sm-12 col-xs-12 padding_zero">
	...
	<div class="col-md-7 col-sm-7 tbresp-rt padding_mzero col-sm-12">
		<div class="col-md-12 col-sm-12 col-xs-12 form-div padding_zero" id="previewCust" style='display:none;'>
 
			<a href="#previewmodel" data-toggle="modal" class="add_pluse" id="sampletext" data-target="#previewmodel"   >
				<i class="fa fa-plus"></i>
				<span onClick="customizePreview();" > preview</span>
			</a> 
		</div> 
	</div>
</div>
...
<div class="col-md-7 col-sm-7 col-sm-12 col-sm-offset-2 tbresp-rt padding_mzero col-md-offset-2">
 ...
 <input type="hidden" name="to_row" id="to_row"  value='0' />
...
<?php echo form_submit(array('name' => 'sendsms','value' => 'Send', 'type'=>'submit', 
	'class' => 'submit_btn sendbtnmrgn','id'=> 'customizeForm' ,'data-placement' => 'top','style' => ''));?>
...
/* $(document).ready(function() {
	$('form#variable_sms_file_upload').submit(function() { 
...
function getUploadedFile() { 
	...
	var file = $("#userUploadedFile").prop('files')[0];
 	...
 	if(size > 30){
			$('#fileupload_error').text(''); 
		  $('#fileupload_error').text("Allowed file size exceeded. (Max. 30 MB) ");
	}else if($.inArray(ext, ['xls','xlsx']) == -1) {
$('#fileupload_error').text(''); 
	  	$('#fileupload_error').text('Uploaded file is not a valid extension!');
 		...
 		enctype: 'multipart/form-data',  
   		...
   		success: function(data) { 
			...
			if(result.error != '') {  
				...
				$('#uploadedFileName').val(result.file_name); 
				//$('#read_from_row').val(0);   uploadedFileName
				//$('#read_to_row').val(0);  
...
function customizePreview() {   
	$('#previewCust').css("display","block");
	var fileName = $('#uploadedFileName').val();
	...
	///var formData = new FormData(document.getElementById("variable_sms_file_upload")); 
	$.ajax({  
		url: "<?php echo base_url();?>campaign/customizePreview",
		type: "POST",
		//enctype: 'multipart/form-data',  
		//processData: false,  
		//contentType: false,    
		data: {'fileName':fileName,'sms_text':sms_text,'from_row':from_row,'to_row':to_row,'mobile_column':mobile_column},
		success: function(data) { 
				console.log(data); 
				$('.previewCustomData').text('');
				var result = $.parseJSON(data);
				...
				$.each(result, function (i, item) { 
					 ...
					 tbody += '<tr><td>' + item[0] + '</td><td>' + item[1] + '</td><td>' +item[2]+'</td> </tr>';

	 
				});     
  
 
				$('.previewCustomData').append(tbody); 
...
<div class="modal fade in" id="previewmodel" aria-hidden="false" >
...
<!-- Modal content-->
		<div class="modal-content col-md-12 col-sm-12 col-xs-12 padding_zero">
			...
			<div class="modal-body col-md-12 col-sm-12 col-xs-12">
				<div class="col-sm-12 col-sm-12 col-sm-12  padding_zero">
					<p class="text-center"><b>Campaign Preview</b> </p> 
					<p class="messagedisplay" id="displaysampletext">
						<table class="table table_all" width="100%" cellpadding="0" cellspacing="0">
							<thead>
								<tr><th style=" font-weight:200 !important;">Mobile</th><th style=" font-weight:200 !important;">Message</th><th style=" font-weight:200 !important;">No.OF.SMS</th></tr>
							</thead>
							<tbody class ="previewCustomData"> 
							</tbody>
						</table>
					</p>			
				</div>
...
<span class="row_input"><input type="hidden" id="read_from_row" value = '0' placeholder="From"></span>
<span class="row_input pull-right"><input type="hidden" id="read_to_row" value = '0' placeholder="To"></span>
</div>
<div class="col-md-12 col-sm-12 padding_zero form-div col-xs-12">
...
<script>
$('form#variable_sms_file_upload').submit(function() {
	var mobilecolumn = $('#uploadedFileMobileColumns').val();
	var uploadedFile = $('#userUploadedFile').val();
		$('#fileupload_error').text('');
	if(uploadedFile.length == '') {
		$('#fileupload_error').text('');
		$('#fileupload_error').text('Please upload your file.');
 	  	//$('#customizeForm').show(); 
		return false;
	}else if(mobilecolumn == '') {
		$('#fileupload_error').text('');
		$('#fileupload_error').text('Please customize your file data, Mobile number column is required.'); 
	  	//$('#customizeForm').show();    
		return false;  
	}else{  
