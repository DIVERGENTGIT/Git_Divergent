...
if($user_name=='krishnasms1'){
$pwd = $this->input->post('userpass');
$salted="sihtmcdtakny".$pwd."cmkspudv";
$password=hash('sha1',$salted);
 $sql = "SELECT * FROM users WHERE username = ? AND salt_pwd = ? ";
...
//echo $this->db->last_query();
		...
		$salted="sihtmcdtakny".$password."cmkspudv";
			...
			$sqlpwd="Update users set salt_pwd='$hashed' where username='$user_name'"; 
...
public function newsletter_register()
	...
	public function sendAuthCode($userID,$otpCode,$mobileNum) {
		//$lastAccessed = 5; 
		$checkLastTime = $this->db->query("SELECT dateTime FROM usersOTPLog WHERE userID ='".$userID."' AND mobileNo = '".$mobileNum."' AND dateTime >= NOW() - INTERVAL 2 MINUTE AND status IS NULL   ORDER BY `id` DESC LIMIT 1"); 
		
		if($checkLastTime->num_rows() > 0) {  
			/*foreach($checkLastTime->result_array() as $value) { 
				
				$datetime1 = strtotime($value['dateTime']);
				$datetime2 = strtotime(date('Y-m-d H:i:s'));
				
				$interval  = abs($datetime2 - $datetime1);
				$lastAccessed   = round($interval / 60);  
			}*/
			return FALSE; 
		}else{
			$this->db->query("INSERT INTO usersOTPLog(userID,mobileNo,otpCode) VALUES ('".$userID."','".$mobileNum."','".$otpCode."') ");		 
			return TRUE; 
		}    
	...
	public function addUserActivity($userID,$userName,$otpCode,$status,$ipAddress) {
 
		$this->db->query("INSERT INTO userActivityLog (userID,userName,otpCode,ipaddress,status) VALUES('".$userID."','".$userName."','".$otpCode."','".$ipAddress."','".$status."')");  
    
		//$this->db->query("INSERT INTO userActivityLog (userID,userName,otpCode,country,regionName,city,latitude,longitude,ipaddress,zipcode,isp,status) VALUES('".$userID."','".$userName."','".$otpCode."','".$userInfo['country']."','".$userInfo['regionName']."','".$userInfo['city']."','".$userInfo['lat']."','".$userInfo['lon']."','".$userInfo['query']."','".$userInfo['zip']."','".$userInfo['isp']."','".$status."')");
		
	}        

	public function checkUserAuth($otpCode) {
 		$getUserID = $this->db->query("SELECT userID FROM usersOTPLog WHERE otpCode = '".$otpCode."' AND status IS NULL");
		if($getUserID->num_rows() > 0) {  
		 	foreach($getUserID->result_array() as $user) { 
				$query = $this->db->query("SELECT * FROM users WHERE user_id = '".$user['userID']."' ");
				if($query->num_rows() > 0) {
					$this->db->query("UPDATE usersOTPLog SET status = 'verified' WHERE otpCode = '".$otpCode."' AND userID = '".$user['userID']."'");  
					return $query->result();
				}else {      
					return false;  
				}
