...
function get_SMSCredits($userId)
	...
	*/
	...
	{
		  $query =  $this->db->query("SELECT campaign_id,created_on  FROM campaigns WHERE user_id  = '".$userid."' ");
		  	$re = array();
			 $x = 0;
			...
			//	$shortcode = $short_code;
				...
				if($date_diff <= 1)
				{
					$query =  $this->db->query("SELECT campaigns.sms_text,campaigns.campaign_id,shorturl_text as long_url,created_on as date_created,RIGHT(shorturl_text, 7 ) AS short_code,(SELECT COUNT(*) as count from campaigns_to where campaigns_to.campaign_id = campaigns.campaign_id ) count FROM sms.campaigns WHERE sms.campaigns.user_id = '".$userid."' AND sms.campaigns.shorturl_text != '' ORDER BY campaign_id desc");  
	...
	public function getShortCodeUrls($campaign_id,$short_code)
	{
...
public function shortcode_all_reports($user_id,$short_code,$campaign_id = null)
{	   //   date_default_timezone_set('Asia/Calcutta');
