...
public function unassigncampainsvalues($mobile,$user_id,$reseller_id)
{
	...
	function createUnicodeShortUrlCampaign($userId,$sms_type,$is_unicode_sms,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$total_numbers)
	{
		// calculate SMS length  
		if(mb_strlen($sms_text)>70)
			$sms_length_tmp=ceil(mb_strlen($sms_text)/63);
...
$sql = $this->db->select("count(to_mobileno) as total_count,
		...
		$array['invalid_number_length_count'] = $invalid_number_length_count->row('invalid_number_length_count');
		 ...
		 $misc_error_count = $this->db->query("SELECT count(*) as misc_error_count FROM $table_name WHERE (dlr_status IN (2,11,16) AND (error_code IN ('0x00000450',1077,100,020,019,410,409, 205,202,200,195,193,178,176,166,164,162,160,145,036,034,031,011, 090,109,252,124,102,037,021,032,035,144,146,161,163,165,167,177,192,194,196,201,204,404) OR error_text = 'Spam Error' OR error_text = 'Black-listed number' OR error_text = 'Insufficient Credits' ) OR (dlr_status = 2 AND  error_text ='Block Listed Number')) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");         
		...
		$array['invalid_number_length_count'] = $invalid_number_length_count->row('invalid_number_length_count');
		 ...
		 $misc_error_count = $this->db2->query("SELECT count(*) as misc_error_count FROM $table_name WHERE (dlr_status IN(2,11,16)  AND (error_code IN ('0x00000450',1077,100,020,019,410,409, 205,202,200,195,193,178,176,166,164,162,160,145,036,034,031,011, 090,109,252,124,102,037,021,032,035,144,146,161,163,165,167,177,192,194,196,201,204,404) OR error_text = 'Spam Error' OR error_text = 'Black-listed number' OR error_text = 'Insufficient Credits' ) OR (dlr_status = 2 AND  error_text ='Block Listed Number')) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");         
		$array['misc_error_count'] = $misc_error_count->row('misc_error_count');
...
$sql = $this->db->select("count(to_mobile_no) as total_count,
		...
		$array['invalid_number_length_count'] = $invalid_number_length_count->row('invalid_number_length_count');
		 ...
		 $misc_error_count = $this->db->query("SELECT count(*) as misc_error_count FROM $table_name WHERE (dlr_status IN(2,11,16) AND (error_code IN ('0x00000450',1077,100,020,019,410,409, 205,202,200,195,193,178,176,166,164,162,160,145,036,034,031,011, 090,109,252,124,102,037,021,032,035,144,146,161,163,165,167,177,192,194,196,201,204,404,'','NULL') OR (error_text = 'Spam Error' OR error_text = 'Black-listed number' OR error_text = 'Insufficient Credits')) OR (dlr_status = 2 AND  error_text ='Block Listed Number')) AND campaign_id = '".$campaign_id."'");       
		$array['misc_error_count'] = $misc_error_count->row('misc_error_count');
