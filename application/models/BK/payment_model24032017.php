...
class payment_model extends CI_Model{
	...
	function order_history($user_id,$from_date,$to_date,$tr_ID,$trn_status,$trn_service,$limit,$offset) {
	 ...
	 if($tr_ID!='')
		...
		$sql="select pe.servicetype,th.epg_txnID,th.noofsms,th.longcode_credits,th.shorturl_credits,th.total_amount,th.created_on,th.payment_status from transaction_history th INNER JOIN price_enquery pe on th.epg_txnID=pe.epg_txnID
		 where th.user_id=$user_id $payment_status $from_date $to_date $tr_ID  $payment_service  order by th.payment_id desc limit $offset,$limit";
   ...
   $payment_status=" AND th.payment_status IN('Transaction Successful','Transaction Cancelled')";
	 ...
	 if($tr_ID!='')
		...
		$sql="select pe.servicetype,th.epg_txnID,th.noofsms,th.longcode_credits,th.shorturl_credits,
		th.total_amount,th.created_on,th.payment_status from transaction_history th INNER JOIN price_enquery pe on th.epg_txnID=pe.epg_txnID
		 where th.user_id=$user_id $payment_status $from_date $to_date $tr_ID  $payment_service order by th.payment_id desc";
