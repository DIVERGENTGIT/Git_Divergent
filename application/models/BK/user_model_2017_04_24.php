...
$url='http://www.smsstriker.com/myaccount/codeotp_mverify.html';
?>
...
echo "<script>alert('Message Sent Succesfully');</script>";	
					...
					redirect('myaccount/codeotp_mverify');
...
public function newsletter_register()
	...
	public function checkCoupon($couponCode,$user_id) {
		...
		if($checkUserValidity->num_rows() > 0) {
			...
			$query = $this->db->select('offer_percent')
					 ->from('coupons')
					 ->where('coupon',$couponCode)
					 ->where('date(endDate) >=',date('Y-m-d'))
					 ->get();
			if($query->num_rows() > 0) {
				$this->db->insert("INSERT INTO price_enquery (registeruser_id,couponCode) VALUES('".$user_id."','".$couponCode."')");
				 return 1;
			}else{
 				return 2;  
			}
	...
	public function getCouponOffer($user_id) {
		$checkUserValidity = $this->db->query("SELECT `couponCode` FROM (`price_enquery`) WHERE `pgresponse_code` IS NULL AND `pgresponse` IS NULL AND `registeruser_id` = '".$user_id."' LIMIT 1");
 		//$total = 1000;$planPrice = '0.001';
		if($checkUserValidity->num_rows() > 0) {
			$couponCode = $checkUserValidity->row('couponCode'); 
			$query = $this->db->select('discountNum,offer_percent,price')
					 ->from('coupons')
					 ->where('coupon',$couponCode)
					 ->where('date(endDate) >=',date('Y-m-d'))
					 ->limit(1)
			...
			if($query->num_rows() > 0) {
				...
				/*foreach($query->result_array() as $response) {
					...
					if($discountType == 1) {
						...
						if($planPrice > $offerPercent) {
							$planPrice = $planPrice-$offerPercent;						
						}else{
							$planPrice = $planPrice;  
						}  
						$finalPrice = $total;
				...
				return array('total' => $finalPrice,'planPrice' => $planPrice); */
			}else{  
 				return FALSE;            
			}    
		}else{	    
