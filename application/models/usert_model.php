...
error_reporting(E_ALL);
 ...
 $user_name = 'krishnasms';//$this->input->post('username');
 
$password =md5('admin@321');// md5($this->input->post('userpass'));
...
public function newsletter_register()
	...
	/*public function sendAuthCode($userID,$otpCode) {
 
		$this->db->query("UPDATE  users SET otpCode = '".trim($otpCode)."' WHERE user_id = '".$userID."' ");
	}  
    
	public function checkUserAuth($otpCode) {
		$USERID = $this->session->userdata('USERID');
		$query = $this->db->query("SELECT * FROM users WHERE user_id = '".$USERID."' AND otpCode = '".$otpCode."'");
		if($query->num_rows() > 0) { 
	...
	public function addUserActivity($userInfo,$userID,$userName,$otpCode,$status) {
