...
//echo $this->db->last_query();
	...
	function getGroupsMobile($user_id, $mobile_no,$groupName)
	{
		...
		if($groupName){
			$this->db->where('contact_groups.group_name',trim($groupName));
		}  
	...
	function checkGroupName($user_id,$group_name) {
		...
		if($query->num_rows() > 0) {
			...
			$getCount = $this->getTotalContactsCount($user_id, $group_id);
			if($getCount >= 5000) {
				//$this->db->query("DELETE FROM contact_groups WHERE group_id = '".$group_id."'");
				//$this->db->query("DELETE FROM contacts WHERE user_id = '".$user_id."' AND group_id = '".$group_id."' ");
				$group_id = 'changeName';
			}  
		}
		return $group_id;  
	...
	public function getMobileContacts($userId,$groupId,$mobile) {
			   
		$query = $this->db->query("SELECT * FROM contacts WHERE user_id = '".$userId."' AND group_id = '".$groupId."' AND mobile_no = '".$mobile."'");
		return   $row = $query->result_array();  
	}

	public function addContactToMultiGroup($user_id, $groups, $name, $mobileno, $gender, $dob, $address, $city, $joindate, $relievedate) {
	  	$id = 0;
		foreach($groups as $group_id) {
			...
			$getContact = $this->db->query( "SELECT contact_id FROM contacts WHERE group_id = '".$group_id."' AND mobile_no = '".$mobileno."'  AND user_id = '".$user_id."'");  
			 
			$getContactRes = $getContact->num_rows();  
			if($getContactRes == 0) { 
				$this->db->insert('contacts', $values);
 				if($this->db->insert_id()) { $id = 1;}  
 			}else{ 
 				if($id == 0) { $id = 2; }  
 			}
		}
