...
$sql = $this->db->select("count(to_mobileno) as total_count,
		...
		$dnd_count = $this->db->query("SELECT count(*) as dnd_count FROM $table_name WHERE (dlr_status = 3 OR dlr_status = 16)  AND ( error_code IN (0x00000481)  OR error_text = 'DND Number' )  AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."' ");     
		...
		$multiple_submit_count = $this->db->query("SELECT count(*) as multiple_submit_count FROM $table_name WHERE dlr_status = 16 AND (error_code = 1077 OR error_code = 411 OR error_text = 'Duplicate Msg' OR error_text ='Multiple Submission'  ) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		...
		$dnd_count = $this->db2->query("SELECT count(*) as dnd_count FROM $table_name WHERE (dlr_status = 3 OR dlr_status = 16)  AND ( error_code IN (0x00000481)  OR error_text = 'DND Number' ) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."' ");     
		$array['dnd_count'] = $dnd_count->row('dnd_count'); 
  
		$invalid_nos_count = $this->db2->query("SELECT count(*) as invalid_nos_count FROM $table_name WHERE (dlr_status = 16 OR dlr_status = 2)  AND ( error_code IN (1080,1081,001,005,015)  OR error_text = 'Invalid Number' ) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."' ");     
		$array['invalid_nos_count'] = $invalid_nos_count->row('invalid_nos_count');
		
		$multiple_submit_count = $this->db2->query("SELECT count(*) as multiple_submit_count FROM $table_name WHERE dlr_status = 16 AND (error_code = 1077 OR error_code = 411 OR error_text = 'Duplicate Msg' OR error_text ='Multiple Submission'  ) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		$array['multiple_submit_count'] = $multiple_submit_count->row('multiple_submit_count');
		
		$invalid_number_length_count = $this->db2->query("SELECT count(*) as invalid_number_length_count FROM $table_name WHERE dlr_status = 16 AND (error_code = '0x00000404' OR error_text = 'Invalid destination number')  AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		$array['invalid_number_length_count'] = $invalid_number_length_count->row('invalid_number_length_count');
		...
		$invalid_sender_ids_count = $this->db2->query("SELECT count(*) as invalid_sender_ids_count FROM $table_name WHERE dlr_status = 16 AND (error_code = '0x00000455' OR error_text = 'Sender ID not found') AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		$array['invalid_sender_ids_count'] = $invalid_sender_ids_count->row('invalid_sender_ids_count');  
		
			
		$out_of_range_count = $this->db2->query("SELECT count(*) as out_of_range_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (002,027,203,010,016) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		$array['out_of_range_count'] = $out_of_range_count->row('out_of_range_count');  
		
		$memory_capacity_exceeded_count = $this->db2->query("SELECT count(*) as memory_capacity_exceeded_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (003,013,412,009) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		$array['memory_capacity_exceeded_count'] = $memory_capacity_exceeded_count->row('memory_capacity_exceeded_count');

		$mobile_equipment_error_count = $this->db2->query("SELECT count(*) as mobile_equipment_error_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (004,012,007) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."' ");     
		$array['mobile_equipment_error_count'] = $mobile_equipment_error_count->row('mobile_equipment_error_count');
		
		$network_error_count = $this->db2->query("SELECT count(*) as network_error_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (005,008,161,034,033,017) AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		$array['network_error_count'] = $network_error_count->row('network_error_count');
		  
		$barring_count = $this->db2->query("SELECT count(*) as barring_count FROM $table_name WHERE dlr_status = 2  AND error_code  = 006  AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		$array['barring_count'] = $barring_count->row('barring_count');
		
		$promo_blocked_count = $this->db2->query("SELECT count(*) as promo_blocked_count FROM $table_name WHERE dlr_status = 2  AND error_code = 044 AND user_id = '".$user_id."' AND date(ondate) = '".$ondate."'");     
		$array['promo_blocked_count'] = $promo_blocked_count->row('promo_blocked_count');
		}
		...
		$mnth_difff = 0;//$month1 - $month;    
 		...
 		for($i = 0;$i<=$mnth_difff;$i++) {
			...
			$table_name = "sms_api_messages_".$mnth.$yr;  
	 		 ...
	 		 if($table_name) { 
				   $query = "select  * from $table_name where user_id='$user_id' and ondate >= DATE_SUB(NOW(),INTERVAL 6 MONTH)";
				if($from_date) {
					$query .= " and date(ondate) >= '$from_date'";
				}
				if($to_date) {
		...
		$mnth_difff = 0;//$month1 - $month;    
 		...
 		for($i = 0;$i<=$mnth_difff;$i++) {
			...
			$table_name = "sms_api_messages_".$mnth.$yr;  
	 		 ...
	 		 if($table_name) { 
				   $query = "select  * from $table_name where user_id='$userID' and ondate >= DATE_SUB(NOW(),INTERVAL 6 MONTH)";
