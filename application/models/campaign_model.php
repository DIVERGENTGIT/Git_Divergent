...
public $db2;public $db3;
	function __construct()
	{
		parent::__construct();
			$this->db2 = $this->load->database('backupdb', TRUE);
			$this->db3 = $this->load->database('shorturl', TRUE);
	...
	function checkIsValidSenderName($user_id,$sender)
	{
		$this->db->select()
			 ->from('sender_names')
			 ->where('user_id',$user_id)
			 ->where('sender_name',trim($sender))
 			 ->where('status', 1);
		$rs = $this->db->get();   
		if($rs->num_rows() > 0) {
			return TRUE;
	...
	public function getNASPortNumber($portType) {
		$this->db->select('sending_port_no')
			 ->from('sms_queue')	
			 ->where('application_priority', trim($portType));
		$rs = $this->db->get(); 
 
		if($rs->num_rows() > 0) {      
			return $rs->row('sending_port_no');
		}else{
			return 0;    
		}  
	}
	
	function createCampaign($userId,$sms_type,$sms_text,$sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$total_numbers)
	{
...
$errorlog_balancecheck="/var/www/html/strikerapp/balance_logs/deducted_log/balance_deduction_".date("Ymd").".log";
//error_log("| Query : $query_text date & time : $curdate |"."\n",3,$errorlog_balancecheck);
	...
	function addSMSCredits($userId, $no_of_sms)
	{
		...
		//$errorlog_balancecheck="/var/www/html/strikerapp/balance_logs/added_log/balance_added_".date("Ymd").".log";
		//error_log("| Query : $query_text date & time : $curdate |"."\n",3,$errorlog_balancecheck);
...
return   $row = $query->result_array(); 
	...
	function getFirstPriorityPort($port_type)
	{
		...
		$this->db->select('sending_port_no')
			->from('sms_queue')
			->where('application_priority',trim($port_type))
			->order_by('queued','asc')
			->limit(1);	
	...
	public function getFirstPriorityPortType($smsport) {
		$this->db->select('sending_port_no')
			 ->from('sms_queue')	
			 ->where('sending_port_no', trim($smsport))
			 ->where('used_for','BSNL');
		$rs = $this->db->get(); 
	...
	function get_campaign_numbers($campaign_id, $days_diff, $campaign_ondate)
	{
		...
		if($days_diff <= 1) {
			...
			$this->db->select('sms_text,to_mobile_no,sent_on,sms_count,dlr_status,error_code,error_text,delivered_on')
			->from($table_name)
			->where('campaign_id', $campaign_id)
		...
		$rs = $this->db->get();
		 $this->db->last_query();
			...
			$this->db2->select('sms_text,to_mobile_no,sent_on,sms_count,dlr_status,error_code,error_text,delivered_on')
			->from($table_name)
			->where('campaign_id', $campaign_id)
			->order_by('sent_on','desc');
	...
	{
		$query = "select  date(ondate) as ondate,sum(noofmessages) as noofmsg from sms_api_messages where user_id='$user_id' and ondate >= DATE_SUB(NOW(),INTERVAL 6 MONTH)";
	...
	function get_sms_api_details($ondate,$user_id,$days_diff,$limit = NULL,$offset = NULL) 
	{
		if($days_diff <= 1){
		    $table_name = "sms_api_messages";
			$query = $this->db->select('ondate,to_mobileno,sender_name,message,noofmessages,dlr_status,error_text,delivered_on')
				->from($table_name)
				->where("user_id in ($user_id)")
			...
			$table_name = "campaigns_backup.sms_api_messages_".$month.$year;
			$query = $this->db2->select('ondate,to_mobileno,sender_name,message,noofmessages,dlr_status,error_text,delivered_on')
				->from($table_name)
				->where("user_id in ($user_id)")
				->where('date(ondate)',$ondate) 
				->order_by('ondate','desc')
	...
	/*function get_sms_api_details_total($ondate,$user_id,$days_diff)
	{
 
		 if($days_diff <= 1){
		    $table_name = "sms_api_messages";
			$query = $this->db->select('ondate,to_mobileno,sender_name,message,noofmessages,dlr_status,error_text')
				->from($table_name)
				->where('user_id',$user_id)
				->where('date(ondate)',$ondate) 				
				->order_by('ondate','desc')
			...
			$month = substr($ondate,5,2);
			$table_name = "campaigns_backup.sms_api_messages_".$month.$year;
			$query = $this->db2->select('ondate,to_mobileno,sender_name,message,noofmessages,dlr_status,error_text')
				->from($table_name)
				->where('user_id',$user_id)
				->where('date(ondate)',$ondate) 
				->order_by('ondate','desc')
	...
	function get_sms_api_details_total($ondate,$mobile_number,$user_id,$days_diff)
	{
		 ...
		 if($days_diff <= 1){
			...
			$year = substr($ondate,0,4);
			$month = substr($ondate,5,2);
			$table_name = "sms_api_messages_".$month.$year;
			 $this->db2->select('ondate,to_mobileno,sender_name,message,noofmessages,dlr_status,error_text');
				$this->db2->from($table_name);
				$this->db2->where("user_id in ($user_id)");
				 
				if($mobile_number) {  
					$this->db2->where('to_mobileno',$mobile_number);
				}	
			
				$this->db2->where('date(ondate)',$ondate);
				$this->db2->order_by('ondate','desc');
				$query =  $this->db2->get();
		}   
   		//echo $this->db->last_query();
	...
	function getTotalNumbersCount($campaign_id, $days_diff, $campaign_ondate,$mobileNum)
	{  

		if($days_diff <= 1) {
			$table_name = "sms.campaigns_to";
			 if($mobileNum) {
					 $this->db->where('to_mobile_no',trim($mobileNum));
				 }
			$this->db->where('campaign_id',$campaign_id);
		...
		$table_name = "campaigns_backup.campaigns_to_".$month.$year;
		 if($mobileNum) {
					 $this->db2->where('to_mobile_no',trim($mobileNum));
				 }
	...
	function getCampaignNumbers($campaign_id,  $days_diff,$mobileNum, $campaign_ondate, $offset = NULL, $limit = NULL)
	{
		if($days_diff <= 1) {
			$table_name = "sms.campaigns_to";
			...
			->from($table_name)
			->where('campaign_id', $campaign_id); 
			if($mobileNum) { 	 		$this->db->where('to_mobile_no', trim($mobileNum)); }
		...
		$rs = $this->db->get();
		 $this->db->last_query();
			...
			$month = substr($campaign_ondate,5,2);
			$table_name = "campaigns_backup.campaigns_to_".$month.$year;
			$this->db2->select('sms_text,to_mobile_no,sent_on,dlr_status,error_code,error_text,delivered_on')
			->from($table_name)
			->where('campaign_id', $campaign_id); 
			if($mobileNum) { 	 		$this->db2->where('to_mobile_no', trim($mobileNum)); }

			   
			$this->db2->order_by('sent_on','desc')
			 ->limit($limit, $offset);
		$rs =$this->db2->get();
		 $this->db2->last_query();
	...
	/*function get_SMS_API_Reports($user_id,$days_diff, $campaign_ondate, $off_set, $limit)
	{
 
		if($days_diff <= 1) {
			$table_name = "sms_api_messages"; 
			$query = $this->db->query("SELECT distinct date(ondate) as ondate,sum(noofmessages) as noofmsg,message_id $table_name  WHERE user_id =$user_id order by ondate desc limit  $off_set,$limit");
			...
			$query = $this->db2->query("SELECT distinct date(ondate) as ondate,sum(noofmessages) as noofmsg,message_id FROM $table_name  WHERE user_id =$user_id order by ondate desc limit  $off_set,$limit"); 
 		}  
	...
	function get_SMS_API_Reports($user_id,$from_date,$to_date,$off_set,$limit)
	{ 
		...
		$query .= " group by date(ondate) order by date(ondate) desc limit $off_set,$limit  ";
		$rs = $this->db->query($query);
		//echo $this->db->last_query();
		return $rs->result();  
	}

	function get_SMS_API_Reports1($user_id,$mobileNum,$from_date,$to_date,$sn)
	{ 
		...
		if($mobileNum) {  
			$query .= " and to_mobileno  = '".trim($mobileNum)."'";
		}   
		if($sn) {  
			$query .= " and sender_name  = '".trim($sn)."'";
		} 
	...
	//$table_name = 'dnd_db.'.$num_series.'_series';
        ...
        $table_name = 'dnd_db.ndnc_data';
        $table_check = 'ndnc_data';  
    ...
    function isBlockListed($mobile_no,$user_id)
    {
...
$sql = "SELECT sum(pending_dlrs_count) AS pending_dlrs_count,sum(delivered_count) AS delivered_count,sum(expired_count) AS expired_count,sum(dnd_count) AS dnd_count,sum(invalid_count) AS invalid_count FROM campaigns where campaign_id=$campaign_id";
			...
			$campaigns_value = $campaigns->result();
	...
	public function get_all_smsreports($user_id,$from_date,$to_date)
	{
		...
		$users_list = $this->db->query("SELECT group_concat(user_id) AS users FROM users WHERE reseller_id IN ($user_id) ");
		...
		if($from_date!='' && $to_date!='')
		{	
			  $campaigns_query = "select campaign_id from campaigns where user_id in ($users) $condition";
		}
		else
		{
			$campaigns_query = "select campaign_id,user_id from campaigns where date(created_on) >= ( CURDATE() - INTERVAL 1 DAY ) AND user_id in ($users)";
		}
	...
	public function get_usersms_download($user_id,$from_date,$to_date)
	{
		...
		if($this->session->userdata('agent_id')  == "")	
		{
			...
			if($users_list->num_rows()  > 0)
			{
				...
				if($from_date!='' && $to_date!='')
				{	
					$campaigns_query = "select campaign_id from campaigns where  user_id in($user_id) $condition";
				}
				else
				{
					$campaigns_query = "select campaign_id,user_id from campaigns where  date(created_on) >= ( CURDATE() - INTERVAL 1 DAY ) AND user_id in($user_id)";
				}
				$campaigns_rs = $this->db->query($campaigns_query);
				$k = 0;
				if($campaigns_rs->num_rows() > 0) 
...
$date_total = $delivered_count + $expired_count + $dnd_count + $pending_dlr_count + $invalid_count;
			...
			else
			{
				...
				if($from_date!='' && $to_date!='')
				{	
					$campaigns_query = "select campaign_id from campaigns where  user_id = '$user_id' $condition";
				}
				else
				{
					$campaigns_query = "select campaign_id,user_id from campaigns where   date(created_on) >= ( CURDATE() - INTERVAL 1 DAY ) AND user_id = '$user_id'";
				}
				$campaigns_rs = $this->db->query($campaigns_query);
				...
				if($campaigns_rs->num_rows() > 0) 
				{
					...
					foreach($campaigns_res as $campaigns_val) 
					{
						...
						//counts
						$query = "SELECT
				    (count(CASE WHEN dlr_status is NULL THEN to_mobile_no END) + count(CASE WHEN dlr_status =0 THEN to_mobile_no END)) as pending_dlr_count,   count(CASE WHEN dlr_status =1 THEN to_mobile_no END) as delivered_count,
						    count(CASE WHEN dlr_status=2 THEN to_mobile_no END) as expired_count,
						    count(CASE WHEN dlr_status=3 THEN to_mobile_no END) as dnd_count,
						    count(CASE WHEN dlr_status=16 THEN to_mobile_no END) as invalid_count,sent_on
						    FROM campaigns_to WHERE campaign_id='$campaign_id' group by date(sent_on)";
...
//	================= model funtions API MIS Report start==============
	...
	public function get_all_api_smsreports($user_id,$from_date,$to_date)
	{
		...
		$users_list = $this->db->query("SELECT group_concat(user_id) AS users FROM users WHERE reseller_id  IN ($user_id)");
		$res = $users_list->result_array();
 
		$users = $res[0]['users'];
		
		if($users != NULL)
		{ 
			$users = $res[0]['users'].','.$user_id;
		}
		else
		{
			$users = $user_id;
		...
		if($campaigns->num_rows() > 0) 
		{
			$campaigns_valueRes = $campaigns->result_array();
			foreach($campaigns_valueRes as $campaigns_value) 
			{
				$delivered_count = $delivered_count + $campaigns_value['delivered_count'];
				$expired_count = $expired_count + $campaigns_value['expired_count'];
				$dnd_count = $dnd_count + $campaigns_value['dnd_count'];
...
public function get_userapi_download($user_id,$from_date,$to_date)
	{
		...
		if($this->session->userdata('agent_id')  == "")	
		{
			...
			if($users_list->num_rows() > 0)
			{
				$users_listRes = $users_list->result_array();
				foreach($users_listRes as $res) 
				{
					$users = $res['users'];
					if($from_date!='' && $to_date!='')
					{
						...
						if($rs->num_rows() > 0)
						{
							$rs_listRes = $rs->result_array();
 							foreach($rs_listRes as $val) 
							{
								$delivered_count = $val['delivered_count'];
								$expired_count = $val['expired_count'];
								$dnd_count = $val['dnd_count'];
					...
					if($campaigns->num_rows() > 0) 
					{
						$campaignsRes = $campaigns->result_array();
						foreach($campaignsRes as $campaigns_value) 
						{
							$delivered_count = $campaigns_value['delivered_count'];
							$expired_count = $campaigns_value['expired_count'];
							$dnd_count = $campaigns_value['dnd_count'];
				...
				if($from_date!='' && $to_date!='')
				{
					...
					if($rs->num_rows() > 0)
					{
						$rsRes = $rs->result_array();
						foreach($rsRes as $val)
						{
							$delivered_count = $val['delivered_count'];
							$expired_count = $val['expired_count'];
							$dnd_count = $val['dnd_count'];
		...
		else
		{
			...
			$sql = "SELECT sum(delivered_count) AS delivered_count,sum(expired_count) AS expired_count,sum(dnd_count) AS dnd_count,sum(invalid_count) AS invalid_count,sum(sms_count) AS total_sms,on_date FROM sms_api_daily_count where user_id = '$user_id' $condition group by date(on_date) ORDER BY on_date DESC";
...
function get_credits_logs($userId,$from_date,$to_date,$offset,$limit)
	{
		...
		$query = "select * from user_credits_log where user_id='$userId' $condition1 $condition2 limit $offset,$limit";
	...
	function get_credits_logs_count($userId,$from_date,$to_date)
	{
		...
		$query = "select * from user_credits_log where user_id='$userId' $condition1 $condition2";
		$res = $this->db->query($query);
		return $res->result();
	}
...
function get_SMSCredits($userId) 
	...
	function insert_SMSCredits_logs($userId, $total_numbers_ex_invalidno,$current_balance,$campaign_id,$ipAddress)
	{
		$balance =  $current_balance - $total_numbers_ex_invalidno;
 
		$job_id = 0; $date = date('Y-m-d');
		//$log = 'CID - '.$campaign_id. ', BeforeCampaignBal - ' .$current_balance.', CurrentPush- '.$total_numbers_ex_invalidno.',AfterCampaignBal - '.$balance.', IPAddress - '.$ipAddress;     
		//error_log($log."\r\n",3,"/var/www/html/logs/user_credits_logs/log_".$date.".log");  

		$this->db->query("insert into user_credits_log(before_campaign_credits,after_campaign_credits,current_campaign_credits,user_id,campaign_id,job_id,IPAddress) values('$current_balance','$balance','$total_numbers_ex_invalidno','$userId','$campaign_id','$job_id','$ipAddress')");
		return 1;
	}  
	...
	*/
	...
	public function shorturl_reportssearch_new($userid,$limit,$off_set,$from_date,$to_date)
	{
		 $fromdate = date("Y-m-d",strtotime($from_date));
		$todate = date("Y-m-d",strtotime($to_date));
		$condition1  = '';
		if($from_date!='' && $to_date!='')
		{
			$condition1 = " AND date(created_on)>='$fromdate' AND date(created_on)<='$todate'";
		}  
		  $query =  $this->db->query("SELECT campaigns.sms_text,campaigns.campaign_id,shorturl_text as long_url,created_on as date_created,RIGHT(shorturl_text, 7 ) AS short_code,phone_nos_count FROM sms.campaigns WHERE sms.campaigns.user_id = '".$userid."' AND sms.campaigns.shorturl_text != '' $condition1  ORDER BY campaign_id desc limit $off_set,$limit");  
	...
	public function shorturl_reportssearch_newcount($userid,$from_date,$to_date)
	{
		$fromdate = date("Y-m-d",strtotime($from_date));
		$todate = date("Y-m-d",strtotime($to_date));
		$condition1  = '';
		if($from_date!='' && $to_date!='')
		{
		  ...
		  $query =  $this->db->query("SELECT campaigns.sms_text,campaigns.campaign_id,shorturl_text as long_url,created_on as date_created,RIGHT(shorturl_text, 7 ) AS short_code,phone_nos_count FROM sms.campaigns WHERE sms.campaigns.user_id = '".$userid."' AND sms.campaigns.shorturl_text != '' $condition1  ORDER BY campaign_id desc ");  
	...
	public function getShortCodeUrls($campaign_id,$short_code,$limit,$offset)
	{
 
 	   $query =  $this->db->query("SELECT campaign_id,created_on  FROM campaigns WHERE campaign_id = '".$campaign_id."' ");
		// $query =  $this->db->query("SELECT to_mobile_no,sms_text,sent_on,short_url FROM campaigns_to WHERE campaign_id = '".$campaign_id."' "); 
		 //return $query->result_array();
			...
			foreach($query->result_array() AS $res)
			{
				...
				if($date_diff <= 1) 
				{
					$sql = "SELECT campaign_id,to_mobile_no,sms_text,sent_on,short_url from sms.campaigns_to where campaign_id = '".$res['campaign_id']."' limit $offset,$limit ";
				...
				else
				{
					...
					$sql1 = "SELECT campaign_id,to_mobile_no,sms_text,sent_on,short_url from " .$table_name . " where  campaign_id = '".$res['campaign_id']."' limit $offset,$limit";
					$res_1 = $this->db2->query($sql1);
	...
	public function getShortCodeUrlsCount($campaign_id,$short_code)
	{
		...
		// $query =  $this->db->query("SELECT to_mobile_no,sms_text,sent_on,short_url FROM campaigns_to WHERE campaign_id = '".$campaign_id."' "); 
		 //return $query->result_array();
			...
			foreach($query->result_array() AS $res)
			{
				...
				if($date_diff <= 1)
				{
					$sql = "SELECT campaign_id,to_mobile_no,sms_text,sent_on,short_url from sms.campaigns_to where campaign_id = '".$res['campaign_id']."'";
					$res_1 = $this->db->query($sql);
				...
				else
				{
					$createddate = date("mY",strtotime($res['created_on']));
					$table_name = "campaigns_backup.campaigns_to_".$createddate;
					$sql1 = "SELECT campaign_id,to_mobile_no,sms_text,sent_on,short_url from " .$table_name . " where  campaign_id = '".$res['campaign_id']."'";
					$res_1 = $this->db->query($sql1);
					/*foreach($res_2->result_array() AS $res123)
					{  
						$re[$x] = array_merge($res,$res123);
	...
	function getUserNames($user_id)
	{
   	...
   	function getAbhiBusUsersNames($userIDS)
	{
...
public function shortcode_reports($user_id,$short_code)
{	 
...
public function shortcode_all_reports($user_id,$short_code,$campaign_id = null,$limit,$offset)
{	 
$query = "SELECT su.long_url, su.short_code, sui.device_type, sui.ip_address, sui.operating_system, sui.build_by, sui.browser_type, sui.created_on FROM shorturl_db.short_urls su INNER JOIN shorturl_db.shorturl_table_info sui ON sui.short_url = su.short_code WHERE user_id ='$user_id' AND su.short_code = '$short_code'";
	...
	foreach($result->result_array() AS $res)
	{
		...
		{
			//$sql = "select to_mobile_no from sms.campaigns_to where short_url = '$shortcode'";
			$sql = "select to_mobile_no from sms.campaigns_to where campaign_id = '$campaign_id' AND short_url = '".$shortcode."'";
			$res_1 = $this->db->query($sql);
		...
		else  
		{
			$createddate = date("mY",strtotime($res['created_on']));
			$table_name = "campaigns_backup.campaigns_to_".$createddate;
			//$sql1 = "select to_mobile_no from " .$table_name . " where campaign = campaign_id short_url = '$shortcode'";
			$sql1 = "select to_mobile_no from " .$table_name . " where campaign_id = '$campaign_id' AND short_url = '$shortcode'";
			$res_2 = $this->db2->query($sql1);
			foreach($res_2->result_array() AS $res123)
			{
				$re[$x] = array_merge($res,$res123);
...
public function shortcode_all_reportsCount($user_id,$short_code,$campaign_id = null)
{
	   //   date_default_timezone_set('Asia/Calcutta');
...
/*
echo $query = "SELECT su.long_url, su.short_code, sui.device_type, sui.ip_address, sui.operating_system, sui.build_by, sui.browser_type, sui.created_on FROM shorturl_db.short_urls su INNER JOIN shorturl_db.shorturl_table_info sui ON sui.short_url = su.short_code WHERE user_id ='$user_id'";*/
...
//return $this->db->last_query();
	...
	foreach($result->result_array() AS $res)
	{
		...
		{
			//$sql = "select to_mobile_no from sms.campaigns_to where short_url = '$shortcode'";
			$sql = "select to_mobile_no from sms.campaigns_to where campaign_id = '$campaign_id'";
			$res_1 = $this->db->query($sql);
		...
		else
		{
			...
			$table_name = "campaigns_backup.campaigns_to_".$createddate;
			//$sql1 = "select to_mobile_no from " .$table_name . " where campaign = campaign_id short_url = '$shortcode'";
			$sql1 = "select to_mobile_no from " .$table_name . " where campaign_id = '$campaign_id'";
	...
	/*
	            $sql = "select to_mobile_no from sms.campaigns_to";
			$res_1 = $this->db->query($sql);
...
public function unassigncampainsvalues($mobile,$user_id,$reseller_id)
{
	...
	function campaignToNormalShorturl($campaign_id,$user_id,$senderName,$sendShorturl,$sms_length,$mobile_no, $message = null,$port_no = null, $dlr = null, $error_text = null)
	{
		$unique_msg_id = $this->get_unique();
		$values = array( 
		'campaign_id' => $campaign_id,
	...
	function createFileShortUrlCampaign($userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$total_numbers)
	{
		// calculate SMS length
		if(strlen($sms_text)>160)
			$sms_length_tmp=ceil(strlen($sms_text)/153);
...
if($is_schedule)
{
$total_no_of_sms=0;$total_numbers = 0;
}		 
	...
	function createContactShortUrlCampaign($userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$total_numbers)
	{
		// calculate SMS length
		if(strlen($sms_text)>160)
			$sms_length_tmp=ceil(strlen($sms_text)/153);
		...
		$values = array(
			...
			'long_url' => $shorturl_input,
			'shorturl_text' => $shorturl_text,
			'source_type' => 5, // ADDED ON 2017-02-09, NORMAL SMS TYPE
			'port_no' => $sms_port,
			'phone_nos_count' => $total_numbers
		);  
		if($is_schedule == 1) {  
	...
	function createShortUrlCampaign($userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$total_numbers)
	{
		// calculate SMS length
		if(strlen($sms_text)>160)
			$sms_length_tmp=ceil(strlen($sms_text)/153);
	...
	function createUnicodeShortUrlCampaign($userId,$sms_type,$is_unicode_sms,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$total_numbers,$sms_length)
	{
		// calculate SMS length  
		/*if(mb_strlen($sms_text)>70)
			$sms_length_tmp=ceil(mb_strlen($sms_text)/63);
	...
	function createCustomizedCampaign($userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$total_mobile_no,$sms_length)
	{
		// calculate SMS length  
		//if(strlen($sms_text)>160)
			//$sms_length_tmp=ceil(strlen($sms_text)/153);
...
if($is_schedule)
{
$total_no_of_sms=0;$total_mobile_no = 0;
}
		$values = array(  
			'user_id' => $userId,
			'sender_name' => $sender,
			...
			'shorturl_text' => $shorturl_text,   
			'port_no' => $sms_port,
'phone_nos_count' =>  $total_mobile_no
		);    
		if($is_schedule == 1) {
			$values['is_scheduled'] = $is_schedule;
			$values['scheduled_on'] = $scheduled_date;
	...
	public function getRecentTemplatesInfo($user_id,$from_date,$to_date) {
...
$sql = $this->db->select("count(to_mobileno) as total_count,
		...
		$dnd_count = $this->db->query("SELECT count(*) as dnd_count FROM $table_name WHERE (dlr_status = 3 OR dlr_status = 16)  AND ( error_code IN (0x00000481)  OR error_text = 'DND Number' )  AND user_id IN ($user_id) AND date(ondate) = '".$ondate."' ");     
		$array['dnd_count'] = $dnd_count->row('dnd_count');   

		$invalid_nos_count = $this->db->query("SELECT count(*) as invalid_nos_count FROM $table_name WHERE (dlr_status = 16 OR dlr_status = 2)  AND ( error_code IN (1080,1081,001,005,015)  OR error_text = 'Invalid Number' ) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."' ");     
		$array['invalid_nos_count'] = $invalid_nos_count->row('invalid_nos_count');
		
		$multiple_submit_count = $this->db->query("SELECT count(*) as multiple_submit_count FROM $table_name WHERE dlr_status = 16 AND (error_code = 1077 OR error_code = 411 OR error_text = 'Duplicate Msg' OR error_text ='Multiple Submission'  ) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['multiple_submit_count'] = $multiple_submit_count->row('multiple_submit_count');
		
		$invalid_number_length_count = $this->db->query("SELECT count(*) as invalid_number_length_count FROM $table_name WHERE dlr_status = 16 AND (error_code = '0x00000404' OR error_text = 'Invalid destination number')  AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['invalid_number_length_count'] = $invalid_number_length_count->row('invalid_number_length_count');
		
		 $misc_error_count = $this->db->query("SELECT count(*) as misc_error_count FROM $table_name WHERE (dlr_status IN (2,11,16) AND (error_code IN ('0x00000450',270,271,1077,100,020,019,410,409, 205,202,200,195,193,178,176,166,164,162,160,145,036,034,031,011, 090,109,252,124,102,037,021,032,035,144,146,161,163,165,167,177,192,194,196,201,204,404) OR error_text = 'Spam Error' OR error_text = 'Black-listed number' OR error_text = 'Insufficient Credits' ) OR (dlr_status = 2 AND  error_text ='Block Listed Number')) AND user_id   IN ($user_id) AND date(ondate) = '".$ondate."'");         
		$array['misc_error_count'] = $misc_error_count->row('misc_error_count');
		    
		$invalid_sender_ids_count = $this->db->query("SELECT count(*) as invalid_sender_ids_count FROM $table_name WHERE dlr_status = 16 AND (error_code = '0x00000455' OR error_text = 'Sender ID not found') AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['invalid_sender_ids_count'] = $invalid_sender_ids_count->row('invalid_sender_ids_count');  
		
			
		$out_of_range_count = $this->db->query("SELECT count(*) as out_of_range_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (002,027,203,010,016) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['out_of_range_count'] = $out_of_range_count->row('out_of_range_count');  
		
		$memory_capacity_exceeded_count = $this->db->query("SELECT count(*) as memory_capacity_exceeded_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (003,013,412,009) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['memory_capacity_exceeded_count'] = $memory_capacity_exceeded_count->row('memory_capacity_exceeded_count');
		
		$mobile_equipment_error_count = $this->db->query("SELECT count(*) as mobile_equipment_error_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (004,012,007) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."' ");     
		$array['mobile_equipment_error_count'] = $mobile_equipment_error_count->row('mobile_equipment_error_count');
		
		$network_error_count = $this->db->query("SELECT count(*) as network_error_count FROM $table_name WHERE dlr_status = 2   AND error_code IN (005,008,161,034,033,017) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['network_error_count'] = $network_error_count->row('network_error_count');
		  
		$barring_count = $this->db->query("SELECT count(*) as barring_count FROM $table_name WHERE dlr_status = 2  AND error_code  = 006  AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['barring_count'] = $barring_count->row('barring_count');
		
		$promo_blocked_count = $this->db->query("SELECT count(*) as promo_blocked_count FROM $table_name WHERE dlr_status = 2  AND error_code = 044 AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['promo_blocked_count'] = $promo_blocked_count->row('promo_blocked_count');
			...
			$month = substr($ondate,5,2);
			$table_name = "campaigns_backup.sms_api_messages_".$month.$year;
				$sql = $this->db2->select("count(to_mobileno) as total_count,
			(count(CASE WHEN dlr_status is NULL THEN to_mobileno END) + count(CASE WHEN dlr_status=0 THEN to_mobileno END)) as pending_dlr_count,
			count(CASE WHEN dlr_status =1 THEN to_mobileno END) as delivered_count,
			count(CASE WHEN dlr_status=2 THEN to_mobileno END) as expired_count, 
			count(CASE WHEN dlr_status=3 THEN to_mobileno END) as dnd_count, 
		...
		$dnd_count = $this->db2->query("SELECT count(*) as dnd_count FROM $table_name WHERE (dlr_status = 3 OR dlr_status = 16)  AND ( error_code IN (0x00000481)  OR error_text = 'DND Number' ) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."' ");     
		$array['dnd_count'] = $dnd_count->row('dnd_count'); 
  
		$invalid_nos_count = $this->db2->query("SELECT count(*) as invalid_nos_count FROM $table_name WHERE (dlr_status = 16 OR dlr_status = 2)  AND ( error_code IN (1080,1081,001,005,015)  OR error_text = 'Invalid Number' ) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."' ");     
		$array['invalid_nos_count'] = $invalid_nos_count->row('invalid_nos_count');
		
		$multiple_submit_count = $this->db2->query("SELECT count(*) as multiple_submit_count FROM $table_name WHERE dlr_status = 16 AND (error_code = 1077 OR error_code = 411 OR error_text = 'Duplicate Msg' OR error_text ='Multiple Submission'  ) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['multiple_submit_count'] = $multiple_submit_count->row('multiple_submit_count');
		
		$invalid_number_length_count = $this->db2->query("SELECT count(*) as invalid_number_length_count FROM $table_name WHERE dlr_status = 16 AND (error_code = '0x00000404' OR error_text = 'Invalid destination number')  AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['invalid_number_length_count'] = $invalid_number_length_count->row('invalid_number_length_count');
		
		 $misc_error_count = $this->db2->query("SELECT count(*) as misc_error_count FROM $table_name WHERE (dlr_status IN(2,11,16)  AND (error_code IN ('0x00000450',270,271,1077,100,020,019,410,409, 205,202,200,195,193,178,176,166,164,162,160,145,036,034,031,011, 090,109,252,124,102,037,021,032,035,144,146,161,163,165,167,177,192,194,196,201,204,404) OR error_text = 'Spam Error' OR error_text = 'Black-listed number' OR error_text = 'Insufficient Credits' ) OR (dlr_status = 2 AND  error_text ='Block Listed Number')) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");         
		$array['misc_error_count'] = $misc_error_count->row('misc_error_count');
		    
		$invalid_sender_ids_count = $this->db2->query("SELECT count(*) as invalid_sender_ids_count FROM $table_name WHERE dlr_status = 16 AND (error_code = '0x00000455' OR error_text = 'Sender ID not found') AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['invalid_sender_ids_count'] = $invalid_sender_ids_count->row('invalid_sender_ids_count');  
		
			
		$out_of_range_count = $this->db2->query("SELECT count(*) as out_of_range_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (002,027,203,010,016) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['out_of_range_count'] = $out_of_range_count->row('out_of_range_count');  
		
		$memory_capacity_exceeded_count = $this->db2->query("SELECT count(*) as memory_capacity_exceeded_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (003,013,412,009) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['memory_capacity_exceeded_count'] = $memory_capacity_exceeded_count->row('memory_capacity_exceeded_count');

		$mobile_equipment_error_count = $this->db2->query("SELECT count(*) as mobile_equipment_error_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (004,012,007) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."' ");     
		$array['mobile_equipment_error_count'] = $mobile_equipment_error_count->row('mobile_equipment_error_count');
		
		$network_error_count = $this->db2->query("SELECT count(*) as network_error_count FROM $table_name WHERE dlr_status = 2  AND error_code IN (005,008,161,034,033,017) AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['network_error_count'] = $network_error_count->row('network_error_count');
		  
		$barring_count = $this->db2->query("SELECT count(*) as barring_count FROM $table_name WHERE dlr_status = 2  AND error_code  = 006  AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['barring_count'] = $barring_count->row('barring_count');
		
		$promo_blocked_count = $this->db2->query("SELECT count(*) as promo_blocked_count FROM $table_name WHERE dlr_status = 2  AND error_code = 044 AND user_id IN ($user_id) AND date(ondate) = '".$ondate."'");     
		$array['promo_blocked_count'] = $promo_blocked_count->row('promo_blocked_count');
		}
	...
	function get_sms_api_details_by_filter($ondate,$user_id,$days_diff,$sender,$mobile_number)
	{
		if($days_diff <= 1){
		    $table_name = "sms.sms_api_messages";
		    $this->db->select('ondate,to_mobileno,sender_name,message,noofmessages,dlr_status,error_text,delivered_on');
		$this->db->from($table_name);
		$this->db->where("user_id in ($user_id)");
		 if($sender) {
			$this->db->where('sender_name',$sender);
		...
		$rs = $this->db->get();
		 //return $this->db->last_query();
		    ...
		    $month = substr($ondate,5,2);
		    $table_name = "campaigns_backup.sms_api_messages_".$month.$year;
		    $this->db2->select('ondate,to_mobileno,sender_name,message,noofmessages,dlr_status,error_text,delivered_on');
		$this->db2->from($table_name);
		$this->db2->where("user_id in ($user_id)");
		 if($sender) {
			$this->db2->where('sender_name',$sender);
		 }
		if($mobile_number) {
			$this->db2->where('to_mobileno',$mobile_number);
		}	
	...
	function createUserBulShorturl($data_array,$shortUrl_array) { 
		 $this->db->insert_batch('user_bulk_shorturl',$data_array); 
		 $this->db->insert_batch('shorturl_db.short_urls',$shortUrl_array);	
		  
	}
	function getUserShortUrls($user_id,$fromdate,$todate,$limit,$offset) {  
		$this->db->select();
	...
	function getUserShortUrlsCount($user_id,$fromdate,$todate) {  
	...
	function getUserUrls($user_id,$shorturlId,$limit = '0',$offset='0') {
	
 		if($limit > 0) {
			$query =  $this->db->query("SELECT `user_bulk_shorturl`.*, `shorturl_db`.`short_urls`.`counter` FROM (`user_bulk_shorturl`) JOIN user_shorturl ON user_bulk_shorturl.shorturl_id = user_shorturl.shorturl_id LEFT JOIN `shorturl_db`.`short_urls` ON `shorturl_db`.`short_urls`.`short_code` = `user_bulk_shorturl`.`shortCode` WHERE `user_shorturl`.`user_id` = '".$user_id."' AND `user_bulk_shorturl`.`shorturl_id` = '". $shorturlId."' limit $offset,$limit"); 
		}else{
	...
	function getUserUrlsData($user_id,$shorturlId,$limit,$offset) {
	...
	function getUserUrlsCount($user_id,$shorturlId) {
 ...
 values ($userId,'".$getorderinfo->row('noofsms')."','".$getorderinfo->row('price_per_sms')."',$amount,'".$getorderinfo->row('tax_amount')."','".$getorderinfo->row('total_amount')."',$transaction_id,'".$getorderinfo->row('authcode')."','".$getorderinfo->row('pgresponse')."')");    
	...
	public function updateCampaignSmsText($campaign_id,$user_id,$sms_text,$campaign_long_url,$campaign_shorturl_text,$short_code)
	{
		$sql=$this->db->query("UPDATE sms.campaigns SET sms_text ='".$sms_text."',long_url= '".$campaign_long_url."',shorturl_text = '".$campaign_shorturl_text."' WHERE user_id ='".$user_id."' AND campaign_id = '".$campaign_id."'"); 
		 $this->db->insert('shorturl_db.short_urls',array('long_url' => $campaign_long_url,'user_id' => $user_id,'short_code'
 => $short_code,'date_created' => date('Y-m-d H:i:s')  ));  
		  
    
	}  
	public function addShortUrl($long_url,$user_id,$short_code) {
		 $this->db->insert('shorturl_db.short_urls',array('long_url' => $long_url,'user_id' => $user_id,'short_code'
 => $short_code,'date_created' => date('Y-m-d H:i:s')  ));  
		//return $this->db->last_query();
	}    
  
	public function updateSMSText($campaign_id,$sms_text)
	{
		 $this->db->query("UPDATE campaigns SET sms_text ='".$sms_text."' WHERE campaign_id = '".$campaign_id."'");

	}

	public function shortcode_download_all_reports($user_id,$short_code,$campaign_id) {
		$query = "SELECT campaign_id,created_on FROM campaigns where campaign_id = '".$campaign_id."' ";
	...
	if($result->num_rows() > 0 ) {
 		...
 		$res = $result->row('created_on');
		$created_date = date("Y-m-d",strtotime($res));
		$current_date = date("Y-m-d");
		$date_diff = (strtotime($current_date) - strtotime($created_date)) / (60 * 60 * 24);
		...
		if($date_diff <= 1)  
		{
			 ...
			 $sql = "select to_mobile_no,short_url from sms.campaigns_to where campaign_id = '$campaign_id' ";
			 $res_1 = $this->db->query($sql);
			 $m = 0;
			 foreach($res_1->result_array() AS $res12)
			 {
 
				//  $query1 = $this->db->query("SELECT su.long_url, su.short_code, sui.device_type, sui.ip_address, sui.operating_system, sui.build_by, sui.browser_type, sui.created_on FROM shorturl_db.short_urls su LEFT JOIN shorturl_db.shorturl_table_info sui ON sui.short_url = su.short_code INNER JOIN sms.campaigns WHERE su.user_id ='$user_id'  and su.short_code = '".$res12['short_url']."'");
				  ...
				  if($query1->num_rows() > 0 ) {
					...
					// $re[$m] = $result;
					 foreach($result as $record) {
						$re[$m]['long_url'] = $record['long_url'];
						$re[$m]['short_code'] = $record['short_code'];
						$re[$m]['device_type'] = $record['device_type'];
						$re[$m]['ip_address'] = $record['ip_address'];
						$re[$m]['operating_system'] = $record['operating_system'];
						$re[$m]['build_by'] = $record['build_by'];
						$re[$m]['browser_type'] = $record['browser_type'];
						$re[$m]['created_on'] = $record['created_on'];
						$re[$m]['to_mobile_no'] = $res12['to_mobile_no'];	
		...
		{
			 $createddate = date("mY",strtotime($res));
			$table_name = "campaigns_backup.campaigns_to_".$createddate; 
			$sql1 = "select to_mobile_no,short_url from " .$table_name . " where campaign_id = '$campaign_id' ";
			$res_2 = $this->db2->query($sql1);
 			$m = 0;
			foreach($res_2->result_array() AS $res123)
			{
/*
$query1 = $this->db2->query("SELECT su.long_url, su.short_code, sui.device_type, sui.ip_address, sui.operating_system, sui.build_by, sui.browser_type, sui.created_on FROM shorturl_db.short_urls su LEFT JOIN shorturl_db.shorturl_table_info sui ON sui.short_url = su.short_code WHERE su.user_id ='$user_id'  and su.short_code = '".$res123['short_url']."'");*/
...
//print_r($result); exit;
					// $re[$m] = $result;
					 foreach($result as $record) {
						...
						$re[$m]['browser_type'] = $record['browser_type'];
						$re[$m]['created_on'] = $record['created_on'];
						$re[$m]['to_mobile_no'] = $res123['to_mobile_no'];	
	...
	public function getShortcodeInput($shortCode) {  	
		/*$query = $this->db->select('long_url')
			->from('shorturl_db.short_urls')
			->like('short_code',$shortCode)	
			->get();*/
		$query = $this->db->query("SELECT long_url FROM shorturl_db.short_urls WHERE short_code='".trim($shortCode)."'");
 
		if($query->num_rows() > 0) {
			return $query->row('long_url');  
		}else{
			return false;
		}
...
public function updateCampaignShortUrl($campaign_id,$shortInput,$getsendShorturl,$smstext )
	{


		 $this->db->query('UPDATE campaigns SET sms_text= "'.$smstext.'" ,long_url ="'.$shortInput.'",shorturl_text = "'.$getsendShorturl.'" WHERE campaign_id = "'.$campaign_id.'"');
	 ...
	 function get_SMS_campaign_count($campaign_id, $days_diff, $ondate)
	{
...
$sql = $this->db->select("count(to_mobile_no) as total_count,
			...
			count(CASE WHEN dlr_status=2 THEN to_mobile_no END) as expired_count, 
			count(CASE WHEN dlr_status=3 THEN to_mobile_no END) as dnd_count, 
			(count(CASE WHEN dlr_status=16 THEN to_mobile_no END) + count(CASE WHEN dlr_status=4 THEN to_mobile_no END) + count(CASE WHEN dlr_status=5 THEN to_mobile_no END) ) as invalid_count")
				->from($table_name) 
				->where('campaign_id',$campaign_id)
				//->where('date(sent_on)',$ondate)		 		
				->order_by('sent_on','desc');
				$rs = $this->db->get(); 
		...
		$dnd_count = $this->db->query("SELECT count(*) as dnd_count FROM $table_name WHERE (dlr_status = 3 OR dlr_status = 16)  AND ( error_code IN (0x00000481)  OR error_text = 'DND Number' ) AND campaign_id = '".$campaign_id."' ");     
		$array['dnd_count'] = $dnd_count->row('dnd_count'); 
  
		$invalid_nos_count = $this->db->query("SELECT count(*) as invalid_nos_count FROM $table_name WHERE (dlr_status = 16 OR dlr_status = 2)  AND ( error_code IN (1080,1081,001,005,015)  OR error_text = 'Invalid Number' ) AND campaign_id = '".$campaign_id."' ");     
		$array['invalid_nos_count'] = $invalid_nos_count->row('invalid_nos_count');
		
		$multiple_submit_count = $this->db->query("SELECT count(*) as multiple_submit_count FROM $table_name WHERE dlr_status = 16 AND (error_code = 1077 OR error_code = 411   OR error_text = 'Duplicate Msg'  OR error_text ='Multiple Submission'  ) AND campaign_id = '".$campaign_id."' ");     
		$array['multiple_submit_count'] = $multiple_submit_count->row('multiple_submit_count');
		...
		$array['invalid_number_length_count'] = $invalid_number_length_count->row('invalid_number_length_count');
		 ...
		 $misc_error_count = $this->db->query("SELECT count(*) as misc_error_count FROM $table_name WHERE (dlr_status IN(2,11,16) AND (error_code IN ('0x00000450',270,271,1077,100,020,019,410,409, 205,202,200,195,193,178,176,166,164,162,160,145,036,034,031,011, 090,109,252,124,102,037,021,032,035,144,146,161,163,165,167,177,192,194,196,201,204,404,'','NULL') OR (error_text = 'Spam Error' OR error_text = 'Black-listed number' OR error_text = 'Insufficient Credits')) OR (dlr_status = 2 AND  error_text ='Block Listed Number')) AND campaign_id = '".$campaign_id."'");       
		$array['misc_error_count'] = $misc_error_count->row('misc_error_count');
		...
		$dnd_count = $this->db2->query("SELECT count(*) as dnd_count FROM $table_name WHERE (dlr_status = 3 OR dlr_status = 16)  AND ( error_code IN (0x00000481)  OR error_text = 'DND Number' ) AND campaign_id = '".$campaign_id."' ");     
		$array['dnd_count'] = $dnd_count->row('dnd_count');   
		$invalid_nos_count = $this->db2->query("SELECT count(*) as invalid_nos_count FROM $table_name WHERE (dlr_status = 16 OR dlr_status = 2)  AND ( error_code IN (1080,1081,001,005,015)  OR error_text = 'Invalid Number' ) AND campaign_id = '".$campaign_id."' ");     
		$array['invalid_nos_count'] = $invalid_nos_count->row('invalid_nos_count');
		
		$multiple_submit_count = $this->db2->query("SELECT count(*) as multiple_submit_count FROM $table_name WHERE dlr_status = 16 AND (error_code = 1077 OR error_code = 411 OR error_text = 'Duplicate Msg' OR error_text ='Multiple Submission' ) AND campaign_id = '".$campaign_id."'");     
		$array['multiple_submit_count'] = $multiple_submit_count->row('multiple_submit_count');
	...
	public function checkCodeExists($shortCode) {
		$shortCode = trim($shortCode);
		$query = $this->db->query("SELECT id FROM shorturl_db.short_urls WHERE short_code = '".$shortCode."'");
		if($query->num_rows() > 0) {
			return 0;   
		}else{  
			return 1;  
	...
	public function getCampaignStatus($campaign_id) {
		$query = $this->db->select('campaign_status')
			 ->from('campaigns')
			 ->where('campaign_id',$campaign_id)
			 ->get();
		if($query->num_rows() > 0) {
			return $query->row('campaign_status');   
		}else{  
			return 0;  
		}  
	}
function get_sms_api_days1($user_id,$mobileNum,$days_diff,$from_date,$to_date,$offset,$limit)
	{
		
 		if(!$mobileNum || $days_diff <= 2) {
			...
			return $this->getAPIReportsByMobileSearch($user_id,$mobileNum,$days_diff,$from_date,$to_date);	 
		}
	}

function get_total_sms_api_days1($user_id,$mobileNum,$days_diff,$from_date,$to_date)
	{  
 		...
 		if(!$mobileNum || $days_diff <= 2) {
			...
			return $this->getTotalAPIReportsByMobileSearch($user_id,$mobileNum,$days_diff,$from_date,$to_date);	 
		}
	}  	
	function getTotalAPIReportsByMobileSearch($user_id,$mobileNum,$days_diff,$from_date,$to_date) {
		$response = 0; 
		$year = substr($from_date,0,4);
		$month = substr($from_date,5,2);  
		$year1 = substr($to_date,0,4);
		$month1 = substr($to_date,5,2);
		$mnth_difff = $month1 - $month;  
 		for($i = 0;$i<=$mnth_difff;$i++) {
 		 	if($i == 0) { 
 				$mnth = $month;      
 		 		$yr = $year;
 		 	}else{
			...
			$table_name = "campaigns_backup.sms_api_messages_".$mnth.$year1;  
		
	 		if($table_name) { 
				   $query = "select  date(ondate) as ondate,sum(noofmessages) as noofmsg from $table_name where user_id='$user_id' and ondate >= DATE_SUB(NOW(),INTERVAL 6 MONTH)";
				if($mobileNum) {
					$query .= " and to_mobileno  = '".trim($mobileNum)."'";  
				}  
	...
	function getAPIReportsByMobileSearch($user_id,$mobileNum,$days_diff,$from_date,$to_date) {
		$response = array();
		$year = substr($from_date,0,4);
		$month = substr($from_date,5,2);  
		$year1 = substr($to_date,0,4);
	...
	public function get_sms_api_details_filter_count($userID,$days_diff,$mobileNum,$fromDate,$toDate,$sender_name,$off_set,$limit) {
		 ...
		 $records = 0;
 		 if($off_set == 0) {
			...
			if($mobileNum) { $this->db->where('to_mobileno',$mobileNum);} 
			
			if($sender_name) { $this->db->where('sender_name',$sender_name);} 
			
			if($fromDate) {
				$this->db->where("date(ondate) >=",$fromDate);
			}
			if($toDate) {
				 $this->db->where("date(ondate) <=",$toDate);
			} 
		...
		$records += $this->getAPIReports_filter_count($userID,$mobileNum,$days_diff,$fromDate,$toDate,$sender_name);
	  ...
	  function getAPIReports_filter_count($user_id,$mobileNum,$days_diff,$from_date,$to_date,$sender_name) {
		$response = 0;
		$year = substr($from_date,0,4);
		$month = substr($from_date,5,2);  
		$year1 = substr($to_date,0,4);
		$month1 = substr($to_date,5,2);
		$mnth_difff = 0;//$month1 - $month;    
 		...
 		for($i = 0;$i<=$mnth_difff;$i++) {
			...
			$table_name = "sms_api_messages_".$mnth.$yr;    
		
	 		 if($table_name) { 
				   $query = "select  * from $table_name where user_id in($user_id) and ondate >= DATE_SUB(NOW(),INTERVAL 6 MONTH)";
				if($from_date) {
					$query .= " and date(ondate) >= '$from_date'";
				}
				if($to_date) {
				...
				if($sender_name) {
					$query .= " and sender_name  = '".trim($sender_name)."'";  
				} 
				 
 
				$query .= " order by date(ondate) desc";
 
				$rs = $this->db2->query($query); 
				$result = $rs->num_rows();
				$response  = $response+$result; 
			} 
	...
	public function get_sms_api_details_filter($userID,$mobileNum,$days_diff,$fromDate,$toDate,$sender_name,$off_set,$limit) {
		...
		$year1 = substr($toDate,0,4);
		$month1 = substr($toDate,5,2);
		$mnth_difff = 0;//$month1 - $month;    
 		...
 		for($i = 0;$i<=$mnth_difff;$i++) {
			...
			$table_name = "sms_api_messages_".$mnth.$yr;  
	 		 ...
	 		 if($table_name) { 
				   $query = "select  * from $table_name where user_id IN ($userID) and ondate >= DATE_SUB(NOW(),INTERVAL 6 MONTH)";
				   
				if($fromDate) {
					$query .= " and date(ondate) >= '$fromDate'";
				}
				if($toDate) {
					$query .= " and date(ondate) <= '$toDate'";
				}   	
				if($mobileNum) {
					$query .= " and to_mobileno  = '".trim($mobileNum)."'";  
				}  
				...
				$query .= "  order by date(ondate) desc LIMIT $off_set,$limit";
 
				$rs = $this->db2->query($query);   
				$backupsResult = $rs->result_array();
				//echo $this->db2->last_query();
				array_push($response,$backupsResult);
			} 
	...
	public function get_sms_api_details_filter_download($userID,$mobileNum,$days_diff,$fromDate,$toDate,$sender_name) {
