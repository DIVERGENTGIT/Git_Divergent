<?php
class Free_service extends CI_Controller
{
	protected $_credits;
...
public function order_numbers()
	{	
		...
		$this->load->view('missedcall_payments/free_service/order_numbers',$this->_data);
	...
	public function getdidnos_price()
	{
		...
		$this->load->view('missedcall_payments/free_service/viewdidprice',$data);	
	}
	...
	public function SelectedNumbers()
	{
		...
		//$data['result']=$result1;
			...
			foreach($result1 as $key=>$number)
			{
				...
				if($querycount->num_rows()<=2)
				{
					$sql1="select * from order_numbers where did_number='$did_number' and user_id=$user_id and status=0";
					$query=$this->db->query($sql1);
					//echo $query->num_rows();
					if($query->num_rows()==0)
					{
...
public function getuserforms()
	{
		...
		$this->load->view('missedcall_payments/free_service/userform',$data);	
	...
	public function userregistration()
	{
		...
		/**  get order number for session variable end **/
			...
			'lastname'=>$lastname,'password'=>$password,'emailid'=>$emailid,'emailid'=>$emailid,'mobileno'=>$mobileno,'organization'=>$organization,'address1'=>$address1,'address2'=>$address2,'state_id'=>$state_id,'city_id'=>$city_id,'pincode'=>$pincode,'did_numbers'=>$_SESSION['sel_nos']);
	...
	public function sendemail()
	{
		...
		/**  get order number for session variable end **/
		      ...
		      $real_url=$this->config->item('firstring_url');
			$qry_fields =array('sendemail'=>'sendemail','user_id'=>$user_id,'firstname'=>$firstname,'lastname'=>$lastname,'username'=>$username,'password'=>$password,'emailid'=>$emailid,'mobileno'=>$mobileno);
			$did_url = $real_url."API/smsstriker/free_service/registration.php";
			$qry_fields_string = http_build_query($qry_fields);
			$did_conn= curl_init();
			//set the url, number of POST vars, POST data
	...
	public function Search_silverno()
	{
		...
		$this->load->view('missedcall_payments/free_service/service_nos/getsilver_nos',$this->_data);	 
	}
