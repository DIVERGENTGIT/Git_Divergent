...
$this->_data['shorturlCredits'] = $rs->shorturl_credits;
	...
	function addToCart(){
      ...
      {                    
		...
		$total_amount,$this->input->get('cartid'),$this->input->get('couponCode'));
	...
	//redirect(base_url().'products/shoppingCartView');
	...
	function removeCart(){
		...
		if($result){
			return $result;
			//redirect(base_url().'products/products_view');
		}
	}
...
function updateprice(){
	...
	function shoppingCartView(){
		...
		$this->session->set_userdata('noofsms','0');
	...
	function saveCartProducts(){
		...
		if($result){
			...
			redirect(base_url().'payment/confirm_order/');
			//$this->cart->destroy();
		}
		else{
			$msg="Save Failed...";
