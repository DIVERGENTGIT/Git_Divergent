...
Class mytemplate extends CI_Controller
{
	...
	$this->_dnd_check = $this->session->userdata('dnd_check');
...
$this->_data['shorturlCredits'] =  $rs->shorturl_credits;
	...
	if($this->session->userdata('from_date')) {
		...
		$this->_data['from_date'] = $from_date = date('Y-m-d', strtotime('-1 day', strtotime(date('Y-m-d'))));
	}
	if($this->session->userdata('to_date')) {
		$this->_data['to_date'] = $to_date = $this->session->userdata('to_date');
	}else{
		$this->_data['to_date']  = $to_date = date('Y-m-d');
	}
	...
	if($this->uri->segment(3) == "download"){
		if(count($templates) > 0) {
			...
			$string .= "Sl.No\tTemplate\tCreatedOn\n";
			foreach($templates as $templateData) {	
				$message = trim(preg_replace('/\s+/', ' ',$templateData->sms_text)); 
				$created_on = $templateData->created_on;
				$string .= $rowId++."\t".$message."\t".$created_on;
				$string .= "\n";
			...
			header("Content-Disposition: attachment; filename=SMSTemplates_".$this->_userId.".xls");
	...
	public function delete_campaign_template()
	{
	 ...
	 public function block_listed_numbers() {
    
 	$this->_data['title'] = "Block Listed Numbers";
        $this->load->model('user_model');
	
 
 	if($this->input->post('add_number')){   

		$this->load->library('form_validation');

		if ($this->form_validation->run('add_block_number_form') == TRUE) {
 
			$number = $this->input->post('bNumber');
			if($this->user_model->isNumberAvailable($this->_userId, $number))
			{
				$this->user_model->addBNumber($this->_userId, $number);
				redirect("mytemplate/block_listed_numbers/added");
			}else{
				redirect("mytemplate/block_listed_numbers/exist");
			}						
		}
	}
	...
	//$blocked_numbers = $this->user_model->getBlockedNames($this->_userId,$limit,$offset);
	
	//$total_count= $this->user_model->getBlockedNames_count($this->_userId);  
		
		  
 	$blocked_numbers = $this->user_model->getBlockedNames($this->_userId);
	$this->_data['blockedNumbers'] = $blocked_numbers;
 
	if($this->uri->segment(3) == "added"){
		$this->_data['deleted'] = "Number has been Added"; 
	}

	if($this->uri->segment(3) == "deleted"){
		$this->_data['deleted'] = "Number has been Deleted";
	}
	if($this->uri->segment(3) == "exist"){
		$this->_data['deleted'] = "Number already exist in your list";
	}
	/*$this->load->library('pagination');
	$config['base_url'] = site_url().'/mytemplate/block_listed_numbers';
	...
	$this->load->view('includes/header',$this->_data);
	$this->load->view('includes/leftmenu');  
	$this->load->view('mytemplate/block_listed',$this->_data);
	$this->load->view('includes/footer');
    }

     public function delete_blocked_number() {
	        $bNID = $this->uri->segment(3); 
		if(!$bNID){ 
		    redirect('mytemplate/block_listed_numbers');
		}

		$this->load->model('user_model');
		$rs = $this->user_model->getBlockNumberDetails($this->_userId, $bNID);

		if(!$rs){ 
		    redirect('mytemplate/block_listed_numbers');
		}  

		$rs = $this->user_model->delete_blockedNumber($this->_userId, $bNID);

		redirect("mytemplate/block_listed_numbers/deleted"); 
