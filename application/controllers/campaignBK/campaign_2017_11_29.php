...
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)."&to=$mobile_no&text=".urlencode($message);
}else{
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)."&to=91$mobile_no&text=".urlencode($message);
}
														//$this->Campaign_model->campaignTo($campaign_id, $sms_length, $mobile_no, $message,$sms_port);
...
$string = '';
	...
	private function _sendUnicodeSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type,$is_unicode_sms ,$is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
	{
		...
		$to_mobileno = $validNosArray = array_filter($validNosArray);
					...
					if($is_block_listed){
						...
						if(!$this->_userType){
							...
							if($isDND)
							{
								...
								// ADDED on 2017-01-23
								if($this->_International == 1) {
									$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name) ."&to=$mobile_no&text=".urlencode($sms_text);
								}else{
