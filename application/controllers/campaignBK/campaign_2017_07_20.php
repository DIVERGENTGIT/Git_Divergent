		  ...
		  $sendShorturl = ''; $smstext = $sms_text; 
			...
			if($is_schedule) {
				...
				for($i=0; $i < $offset; $i++)
				{
					...
					if($is_block_listed){
						$error_text = "Block Listed Number";
						$this->Campaign_model->campaignToNormalShorturl($campaign_id,$this->_userId, $sender_name,$sendShorturl,$sms_length, $mobile_no, $smstext, 2, $error_text);
					} else {
						if(!$this->_userType){
							//check for dnd number
							$isDND = $this->Campaign_model->checkIsDND($mobile_no);
   ...
   */
	...
	public function upload_file_data() { 
		...
		if($this->input->post('api_report_Search')) {
  		...
  		$total_rows =   $this->Campaign_model->get_sms_api_details_total($ondate,$mobile_number,$this->_userId,$days_diff); 
		$this->_data['dlr_report_type']=$this->_dlr_report_type;
	...
	{ 
		...
		{  
			...
			$sender_name = $this->session->userdata('sender_name');
		   	...
		   	if($mobileNum == '0') { $this->session->set_userdata('noRecordsFound',  1); } 
		   	if($mobileNum) {  
				redirect('index.php/campaign/apiReport');  
			}
		}     
...
but always getting this error Request header field Content-Type is not allowed by Access-Control-Allow-Headers in preflight response.";
	...
	public function calculateSMSLength() {
		...
		$apireportview = $api_data = array();
		 $total_days = 0;
		if($mobileNum != 0) {
		  $apireportview = $this->Campaign_model->get_sms_api_details_filter($this->_userId,$mobileNum,$days_diff,$fromDate,$toDate,$off_set,$limit);
 
		$total_days = $this->Campaign_model->get_sms_api_details_filter_count($this->_userId,$days_diff,$mobileNum,$fromDate,$toDate,$off_set,$limit);  
