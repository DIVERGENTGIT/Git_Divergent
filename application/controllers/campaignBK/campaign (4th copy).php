...
//$this->session->userdata('user_id') ==  4130  ||
		...
		if($campaign_id) {
			...
			$this->Campaign_model->deductSMSCredits($this->_userId, $total_numbers_ex_invalidno);
 			if($total_numbers_ex_invalidno >= 50000) {
				$this->sendAlert($campaign_id,$this->_userId); 
			}  
 ...
 $filter = $shorturl_input;
	...
	private function _sendUnicodeSMS($sms_text, $sender, $to_mobileno, $sms_type,$is_unicode_sms ,$is_schedule, $scheduled_date,$campaign_name)
	{
		...
		if($campaign_id) {
			...
			$current_balance = $this->Campaign_model->get_SMSCredits($this->_userId);
			 $ipAddress = $this->ip_address();
			$this->Campaign_model->insert_SMSCredits_logs($this->_userId, $total_no_of_sms,$current_balance,$campaign_id,$ipAddress);
			$this->Campaign_model->deductSMSCredits($this->_userId, $total_no_of_sms);
...
if($pos === false) {
	 ...
	 private function _sendNormalSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
   {
	...
	//This is added for the purpose of trans scrub users on 23/08/2014
 if($this->_userId==5082 || ($this->_userId==250 && $sender!='BALAJI')){ // sandeep users id for testing purpose
	$sms_port=47413;
	//$sms_port=47513;
}
...
but always getting this error Request header field Content-Type is not allowed by Access-Control-Allow-Headers in preflight response.";
	...
	public function sendAlert($camapign_id,$user_id) {
      		$user="support";  
		$password="Str!k3r2020"; 
		$ipAddress = getenv('HTTP_X_FORWARDED_FOR') ?: getenv('REMOTE_ADDR'); 
		$message = "SMS Campaign CampaignID - $camapign_id, UserID - $user_id with above 50000, from $ipAddress";
		...
		$to = '8886638806,8886638814,9701019800,8886638811';
 
 
	       $url = "https://www.smsstriker.com/API/sms.php?username=krishnasms&password=admin@321&from=STRIKR&to=$to&msg=$message";
