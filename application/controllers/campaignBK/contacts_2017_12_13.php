...
if(count($filter) > 0) {
 ...
 if(count($filter) > 0) {
			...
			if($file_type != ".xls" && $file_type != ".xlsx") {
				...
				if($inputFileType != 'CSV') { 
					...
					for ($i = 2; $i <= $worksheet->getHighestRow(); $i++) { 
					 ...
					 if($insertIDs == 1) {  
					 	redirect('contacts/index/added');
					 }else if($insertIDs == 2) { 
					 	redirect('contacts/index/exist');
					 }else{
					 	redirect('contacts/index/notvalid');
					 }
				}else{
	...
	public function sendAlert($camapign_id,$user_id,$text,$length) {
      		$user="support";  
		$password="Str!k3r2020"; 
		$ipAddress = getenv('HTTP_X_FORWARDED_FOR') ?: getenv('REMOTE_ADDR'); 
		$message = "SMS Campaign CampaignID - $camapign_id, UserName - $user_id,SMSText - $text , with $length, from $ipAddress";
