...
public function confirm_add_credits()
    ...
    // print_r($_POST);
     if(!empty($_POST['noofsms']) && @$_POST['noofsms']!='')
		...
		$this->load->model('user_model');
		$this->_data['dicount'] = 0; 
		/*** APPLYING COUPON START ***/
		$totalPrice = $amount;$discountType = 0;
		...
		if($getDiscount) {	 
			foreach($getDiscount as $response) {
				...
				$this->_data['dicount'] = ($offerPercent)?$offerPercent:$discountPrice;
			}  
			if($discountType == 1) {
				...
				if($smsprice > $offerPercent) {
					$smsprice = $smsprice-$offerPercent;								
				}    				
				$total = $noofsms*($smsprice);	
				$totalPrice = $total;   
			}else{  
		...
		$_SESSION['dicount'] = $this->_data['dicount'];
