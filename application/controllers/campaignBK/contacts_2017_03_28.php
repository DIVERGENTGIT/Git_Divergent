...
if($this->uri->segment(3) == "notexcel") {
			$this->_data['file'] = "Not a valid Excel File.";
		}
...
$gender = $contact_gender?$contact_gender:'';
			if($contact_name != NULL && $contcat_mobileno != NULL && $gender != NULL &&   $address != NULL )
			{
				if(is_numeric($mobile)) {
  ...
  $this->contact_model->updateContact($contact_id,$this->_userId,$group_id,$contact_name,$contact_mobileno,$gender,$dob,$address,$join_date,$relieve_date);  
	...
	public function sendSMS()
	{
		...
		$shorturl_text=$this->input->post('shorturl_text');
                ...
                if($error) {
					...
					$contacts = $this->contact_model->getContacts($this->_userId,$contacts);
                    $to_mobileno = array();
                    foreach($contacts as $row) {
                        $to_mobileno[] = trim($row->mobile_no);
                    }
	...
	private function _sendNormalSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
   {
	...
	//ADDED ON 2017-01-23
	  ...
	  $campaign_id = $this->Campaign_model->createShortUrlCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input);
		...
		if($is_schedule) {
			...
			for($i=0; $i < $offset; $i++)
			{
				...
				if($this->input->post('shorturl_input')!='')
				{
					...
					//$url = "http://ion.bz/api.php";
					
					
					// ADDED on 2017-01-23
					
					// $url = 'http://'.$_SERVER['SERVER_NAME'].'/shorturl/api.php';
					...
					//$getsendShorturl="http://ion.bz/"."$result1";
					$getsendShorturl= $this->_data['UrlGenIp']."$result1"; 
					
					//echo $getsendShorturl;exit;
