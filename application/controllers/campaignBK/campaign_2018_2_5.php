	...
	public function upload_file_data() { 
		...
		header("Content-Type: text/csv; charset=UTF-16LE");
          ...
          public function download_report1() {
        	$ondate = '2018-02-03';//$this->uri->segment(3); 
		$string = '';
		$userID = $this->_userId;  
		$abhibusUsers = array(5742,5741,5740,5739,5733);
		if(in_array($userID,$abhibusUsers)) {
		...
		foreach ($rs as $row) { 
		    ...
		    if(strlen($toMobileNum) < 10){
 			...
 			$test = 'Striker Soft Solutions Pvt Limited incepted in the year 2009, since then pioneered in serving the clients  ';
 
		 	 
		 	  $dlr = array("DateTime"=>$createdDate,"DeliveredOn" => $deliveredDate, "ToMobileNo"=>$toMobileNum,"No.ofSMS" => $numberOFSMS, "SenderName" => $senderName,"Status" => $dlrStatus ,"Message" => $test );
			$getreports[] = $dlr;
