...
if($this->uri->segment(6) == "puid")
	...
	if(@$_POST['Registration']=='Registration')
	{
		...
		foreach($result1 as $key=>$value)
		{
			if($value->count==0)
			{
			$sql="insert into users (username,password,first_name,last_name,email,mobile,address1,city_id,state_id,zipcode,reseller_id)
			values('$username','$userpassword','$firstname','$lastname','$email','$mobile','$address1',$city,$state,$pincode,$reseller_id)";
			$this->db->query($sql);
			 redirect('reseller/myUsers');
			}
			else
