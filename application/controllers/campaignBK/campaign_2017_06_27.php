	...
	public function viewReport()
	{
		...
		if($campaign_id) {
			...
			if($campaign_details_rs) {
				...
				$dlr_report_old = $this->Campaign_model->getCampaignNumbers($campaign_id,  $days_diff, $campaign_ondate, $off_set, $limit);
  ...
  public function getshortcode_reports() {
	...
	{ 
		...
		if($this->_data['selected_sms_type'] == 'ApiSms') {
			...
			$day_count = 0;
			$api_data = array();
			$campaigns_api_data = $this->Campaign_model->get_SMS_API_Reports1($this->_userId,$fd,$td);	
			if(!$off_set) { 			
