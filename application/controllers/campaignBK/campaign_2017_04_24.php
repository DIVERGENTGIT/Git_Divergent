...
$IP = $this->_data['kernalIp'];
	...
	$total_numbers_ex_invalidno = $total_numbers_ex_invalidno*$sms_length;
        ...
        //create campaign
        $campaign_id = $this->Campaign_model->createCampaign($this->_userId,$sms_type,$sms_text,$sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$total_numbers);
			...
			if ($this->form_validation->run('file_sms_form') == TRUE) {
				...
				if($error) {
					...
					if ($this->upload->do_upload()) {
						...
						$file_type = $file['file_ext'];
						$file_name = 'uploads/'.$file['file_name'];
						//$getFileName = explode('.',$file['file_name']);
						//$outfile = 'uploads/'.$getFileName[0].'.csv'; 
						//$getFileSize = $this->convertXLStoCSV($file_name,$outfile); 
						//if($getFileSize > $this->_credits) {
							//$this->_data['error'] = $this->_data['no_balance'] = "Insufficient SMS Credits. Require {$getFileSize} credits";
						//}else{	
							 ...
							 if($isValidFile == 1) 	{ 
								 ...
								 if(!$is_schedule) {    
									 ...
									 { 
										...
										if($j == 3) {
											if(!is_numeric($Row[0])) { 
												$this->_data['file'] = 'Uploaded file is not a valid formate.';
											}
										}
						...
						//checking sms credits exit
							...
							$this->_sendFileSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text,$originalFile);
							 //  redirect('campaign/viewcampaigns');
								}
							}//end of sms credits checking
						// }  
			...
			try
			{
				...
				if ($this->form_validation->run('variable_sms_form') == TRUE)
				{
					...
					if($error) {
						...
						//$getFileSize = $this->convertXLStoCSV($Filepath,$outfile); 
								 ...
								 $mobile_no = trim($uploded_data[$i][$mobile_no_column]);
  								 if($n == 3) {
									...
									if(!is_numeric($mobile_no)) {
										 $this->_data['error'] =  'Uploaded file is not a valid formate.';                                                                
									}  
								  }   
								 $mobileNoCount++;
						...
						if($total_no_of_sms > $this->_credits) {
							...
							if($shorturl_input != NULL) {
							 ...
							 $campaign_id = $this->Campaign_model->createCustomizedCampaign($this->_userId,$sms_type,$message,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$validMobileCount);  
						...
						if($shortInput) {
							...
							$newshorturl_text = str_replace("\r", "", $newshorturl_text);
							 ...
							 $this->Campaign_model->updateCampaignShortUrl($campaign_id,$shortInput,$getsendShorturl,$message);
...
$columns = array();
	...
	{ 
		...
		//$this->session->set_userdata('selected_sms_type','NormalSms');
		...
		}else { $this->_data['from_date'] = $fd = $from_date = $this->session->userdata('from_date');
			$this->_data['sender_name'] = $sn =  $sender_name = $this->session->userdata('sender_name');
			$this->_data['to_date']  =  $td =  $to_date = $this->session->userdata('to_date');
			$this->_data['selected_sms_type'] = $this->session->userdata('selected_sms_type');
		}*/
...
if(count($filter) > 0) {
 ...
 if(count($filter) > 0) {
	...
	private function _sendFileSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text,$originalFile) {
	...
	public function upload_file_data() { 
		...
		$this->_data['dlr_report_type']=$this->_dlr_report_type;
		  ...
		  $this->_data['dlr_report'] = $this->Campaign_model->get_api_campaign_count($this->_userId,$days_diff,$ondate);	
			//    print_r( $this->_data['dlr_report'] );exit;			 
