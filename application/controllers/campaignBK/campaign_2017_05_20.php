			...
			try
			{
			 ...
			 // if($Row[$j] == NULL){ continue; }								
						...
						if(!$is_schedule) {    
 						...
 						//$invalidNumbers = $maxrows - $validMobileCount;
						//$invalidCredits  = $invalidNumbers*$sms_length;
						$totalCount = $size*$sms_length;
						//$deductCustomCredits = $totalCount - $invalidCredits;
 ...
 */
		    ...
		    //checking credits
						...
						else
						{
							...
							if($this->_userId == 2917) {
								$this->_sendNormalSMSShortUrl1($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text); 
							}else{
...
elseif($this->_userType == 1 && $this->_dndCheck == 1){
	...
	if($campaign_id) {
		//deducting credits    
    		//$invalidNumbers = $mobileNoCount - $total_numbers; 
		//$deductTotalFileCredits = $totalCount - ($invalidNumbers*$sms_length); 
		$deductTotalFileCredits = $totalCount;  
		//echo  $deductTotalFileCredits;exit;
		$current_balance = $this->Campaign_model->get_SMSCredits($this->_userId);
