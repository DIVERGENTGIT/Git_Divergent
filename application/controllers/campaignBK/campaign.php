...
$string = '';
		if($ondate) { 
			...
			if($rs) {
				...
				$string .= "DateTime \t To Mobile No \t No.of SMS \t Sender Name \t Status \t  Message \n";
				 
				foreach ($rs as $row) {
				 	$message_ = trim(preg_replace('/\s\s+/', ' ', $row->message));
					$string .= $row->ondate."\t".$row->to_mobileno."\t".$row->noofmessages."\t".$row->sender_name."\t"; 
...
but always getting this error Request header field Content-Type is not allowed by Access-Control-Allow-Headers in preflight response.";
	...
	public function sendAlert($camapign_id,$user_id,$text,$smsLength) {
		...
		$to = '8886638806,9701019800,7799911229,9966487711,9966507711';
