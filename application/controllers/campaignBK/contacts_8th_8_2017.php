                            ...
                            if($isDND){
                                ...
                                $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)
                                    ."&to=91$mobile_no&text=".urlencode($sms_text);
                                          $URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode("https://www.smsstriker.com/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
	...
	public function index()
	{
		...
		if($this->input->post('editsubmit')) 
		{
		 $contact_id = $this->uri->segment(5);
        	...
        	if(!$contact_id){
            		redirect('index.php/contacts/index');
       		}
...
$join_date = $this->input->post('join_date');
 ...
 //if client type is transactional, check for template if not dynamic
		...
		if($this->uri->segment(3) == "added") {
			$this->_data['added'] = "Changes to the group has been saved"; 
		}

		if($this->uri->segment(3) == "exist") {
			$this->_data['edited'] = "Contact already exist"; 
		}
		...
		$this->load->model('contact_model');
 		if($this->input->post('mobileSearch')) {
			$this->_data['groupS'] = $groupName = $this->input->post('groupS');
			$this->_data['mobileNum'] = $mobile_no = $this->input->post('mobileNum');
			$groups = $this->contact_model->getGroupsMobile($this->_userId, $mobile_no,$groupName);
	...
	public function addGroup()
	{
		...
		if ($this->input->post('add_group')) {
			//Array ( [group_name] => fgf [group_multple_file] => groupAdd.xls [add_group] => Submit )
			...
			if ($this->form_validation->run('add_group_form') == TRUE) {
				...
				if ($this->upload->do_upload()) {     
					...
					if($file_type != ".xls" && $file_type != ".xlsx") {
						...
						if(!$group_name){redirect('index.php/contacts/index');}
						$group_id = $this->contact_model->checkGroupName($this->_userId,$group_name); 
						if($group_id == 'changeName') {
							redirect('index.php/contacts/index/changeName');
						}  
...
$ContactsAdded = 0;
											...
											if($name != NULL && $mobile != NULL)
											{
													 ...
													 if(!$group_id) {
							  						 ...
							  						 if(!$group_id) {
														...
														$result =  $this->contact_model->addContact($this->_userId, $group_id, $name, $mobile, $gender, $dob, $address, $city,$joindate,$relievedate);  
						...
						if($ContactsAdded) {
							redirect('index.php/contacts/index/group_id/'.$group_id.'/added');
						}else{
							redirect('index.php/contacts/index/notvalid');
						}	  
	...
	public function multiContactsToGroup() {
		...
		if ($this->upload->do_upload()) {     
			...
			if($file_type != ".xls" && $file_type != ".xlsx") {
				...
				$group_name = $this->input->post('group_multple');
				if(!$group_name){redirect('index.php/contacts/index');}
				$group_id = $this->contact_model->checkGroupName($this->_userId,$group_name); 
...
if($inputFileType != 'CSV') {
				...
				if($ContactsAdded) {
					redirect('index.php/contacts/index/group_id/'.$group_id.'/addContact');
				}else{
					redirect('index.php/contacts/index/notvalid');
				}	  
...
$gender = $contact_gender?$contact_gender:'';
			if($contact_name != NULL && $contcat_mobileno != NULL )
			{  
		...
		$this->_data['groups'] = $groups;
			 if($this->uri->segment(5) == "addContact"){
         redirect('index.php/contacts/index/addContact'); 
        }
		$this->load->view('includes/header',$this->_data);
	//	$this->load->view('contacts/add_contact');
		$this->load->view('includes/leftmenu');
...
echo $contents;
		...
		if($group_name) {
			...
			if ($this->input->post('edit_group')) {
				...
				$groupName = $this->input->post('group_name');
					...
					redirect('index.php/contacts/index/group_id/'.$group_id.'/edited');
				}
			}
	...
	public function deleteGroup()
	{
		...
		if($group_name) {
			...
			redirect('index.php/contacts/index/deleted');
		} else {
			redirect('index.php/contacts/index');
		}
	}
	...
	public function viewContact()
	{
        ...
        foreach($contact_details as $contact) {
            ...
            foreach($groups_rows as $grp) {
                $groups .= "<a href='".site_url('index.php/contacts/viewGroup/group/'.$grp->group_id)."'>$grp->group_name</a>,";
            }
        }
    ...
    public function deleteContact()
    {
      $contact_id = $this->uri->segment(4);
       ...
       redirect("index.php/contacts/index/group_id/$group_id/contactdeleted");
		 //redirect("contacts/index");
	...
	public function sendGroupSMS()
	{
		...
		if($this->_userType != 1 || $this->_dndCheck == 1 )
		{
 			...
 			if($this->input->post('submitMultpleGroupContacts')) {
 				$groups = $this->input->post('groups'); 
 				$this->multiGroupContacts($groups,$_FILES);
 			}else{
	...
	public function contact_list_ajax() 
	{
		...
		$mobileNo = $this->input->post('mobileNum');
 		$this->_data['contactdetails'] = array();
 		if(is_numeric($mobileNo) && $mobileNo != NULL) {   
			  $this->_data['contactdetails'] = $this->Contact_model->getMobileContacts($this->_userId,$group_id,$mobileNo);   
		}else{
	...
	public function sendSMS()
	{
		...
		if($this->_userType != 1 || $this->_dndCheck == 1 )
			...
			if($contact_select_count==0) {
				redirect('index.php/contacts/viewGroup/group/'.$group_id.'/selectContacts');
			} else{
 ...
 if($this->_userId==5082 || ($this->_userId==250 && $sender!='BALAJI')){ // sandeep users id for testing purpose and balaji market
$sms_port=47413;
//$sms_port=47513;
}
	//create campaign
						...
						if(!$this->_userType){
							...
							$isDND = $this->Campaign_model->checkIsDND($mobile_no);
							 if($isDND)
							 {
							  ...
							  $URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode($this->_data['kernalIp']."/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&file_name=dlr.php");
								 http_send($URL, $sms_port); 
...
if(count($filter) > 0) {
 ...
 if(count($filter) > 0) {
	...
	public function multiGroupContacts($groups,$files) { 
 		$groups = $groups; 
		$config['upload_path'] = 'uploads/';
		$config['allowed_types'] = 'xls|xlsx';
		$config['overwrite'] = true; 
		...
		$this->load->library('upload', $config); 
    				//	if($this->_userId == 2917) { print_r($files);exit; };
		if($this->upload->do_upload()) { 
			...
			if($file_type != ".xls" && $file_type != ".xlsx") {
				...
				if($inputFileType != 'CSV') { 
					...
					for ($i = 2; $i <= $worksheet->getHighestRow(); $i++) { 
							...
							if($name != NULL && $mobile != NULL )
			  					{
									if(is_numeric($mobile)) {
								  	  ...
								  	  $insertIDs = $this->contact_model->addContactToMultiGroup($this->_userId, $groups, $name, $mobile, $gender, $dob, $address, $city,$joindate,$relievedate);
					 ...
					 if($insertIDs == 1) {  
					 	redirect('index.php/contacts/index/added');
					 }else if($insertIDs == 2) { 
					 	redirect('index.php/contacts/index/exist');
					 }else{
					 	redirect('index.php/contacts/index/notvalid');
					 }
				}else{
