...
Class campaign extends CI_Controller
{
	...
	public function messages()
	{
	 ...
	 $option_value= $this->input->post('option_value');
		...
		$config['base_url'] = site_url().'index.php/campaign/messages';
	...
	public function missedcall()
	{
	 ...
	 $servicenumber= $this->input->post('servicenumber');
		...
		$this->load->library('pagination');
		$config['base_url'] = site_url().'index.php/campaign/missedcall';
		$config['total_rows'] = $total_rows;
		$config['per_page'] = $limit;
		$config['use_page_numbers']  = TRUE;
		$this->pagination->initialize($config);
	...
	public function fileSMS()
	{
			...
			if ($this->form_validation->run('file_sms_form') == TRUE) {
				...
				if($error) {
					...
					if ($this->upload->do_upload()) {
						...
						$sms_text_spl2 = strtolower($sms_text);
						$special_char = array(',','.','-','!','&','’','_','#','(',')','<','>','%',':',';','.','?','*','+','-','/','&','∼','!','=',',','"','’','’');
						
						$sms_text_spl = str_replace($special_char, ' ', $sms_text_spl2);
		...
		if($this->input->post('editsubmit')){
			...
			redirect('index.php/campaign/normalSMS');
	...
	/* WITH WORKING 4 shorturls ,2017_3_05
	public function fileSMS()
	{
		$this->_data['page_title'] = "File SMS";
					...
					if ($this->upload->do_upload()) {
						...
						if($file_type == ".xls") {
							...
							for ($i = 1; $i <= $worksheet->getHighestRow(); $i++) {
								if(strlen(trim($worksheet->getCellByColumnAndRow(0, $i)->getValue())) > 0) {
								 	 
									
									$to_mobileno[] = trim($worksheet->getCellByColumnAndRow(0, $i)->getValue());
							...
							for ($i = 1; $i <= $worksheet->getHighestRow(); $i++) {
								 //echo strlen(trim($worksheet->getCellByColumnAndRow(0, $i)->getValue()))."</br>";
								 if(strlen(trim($worksheet->getCellByColumnAndRow(0, $i)->getValue())) > 0) {
									   $to_mobileno[] = trim($worksheet->getCellByColumnAndRow(0, $i)->getValue());
							...
							$file_value = file_get_contents($file_name);
							$val = explode("\n", $file_value);
							for($i = 0; $i < count($val) ; $i++)
							{
								if(strlen(trim($val[$i])) > 0) {
									$to_mobileno[] = trim($val[$i]);
								}
							}
						}  
						...
						if($total_no_of_sms > $this->_credits) {
							...
							//$this->_sendSMS($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name);

 
							$this->_sendFileSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text);
							   redirect('campaign/viewcampaigns');
						}//end of sms credits checking
					} else { // end of upload
 						$this->_data['error'] = $this->upload->display_errors('','');
						if($this->_data['error'] == 'The uploaded file exceeds the maximum allowed size in your PHP configuration file.') {
			$this->_data['error'] = 'The uploaded file exceeds the maximum allowed size';
		}
					}  
				}//scheduled date error
			} // end validation
       ...
       */
        ...
        /* Working with 4 shorturls ,2017_3_05
	public function newVariableSMS()
	{
		...
		{   
			...
			$original_file = $this->input->post('userfile');
			...
			try
			{
				...
				if ($this->form_validation->run('variable_sms_form') == TRUE)
				{
					...
					if($error) {
						...
						// ADDED ON 2017-02-08
 						//$file_server_path = $_SERVER['DOCUMENT_ROOT'].'/smsstriker/uploads/'.$Filepath;
								...
								for($j = 0; $j < sizeof($Row); $j++)
								{
									...
									if($maxrows<=5)
									{
										if($i == 1)
										{
											...
											$string .= '<th  style=" font-weight:200 !important;">'.$Row[$j].'</th>';
										}
			      ...
			      $uploded_data[$i-1][$j] = $Row[$j];
						...
						//code to send sms
						//$mobile_no_column = PHPExcel_Cell::columnIndexFromString($this->input->post('mobile_column'));
  							...
  							$tmp_number = trim($mobile_no);
								...
								for($j=0;$j<$varpositionscount;$j++) {
									if($j%2 == 1) {
										...
										//$colIndex = PHPExcel_Cell::columnIndexFromString($colstringValue);
										//$message .= trim($worksheet->getCellByColumnAndRow($colIndex-1, $i)->getValue());
						...
						if($total_no_of_sms > $this->_credits) {
							...
							if($shorturl_input[0] != NULL) {
							 ...
							 $campaign_id = $this->Campaign_model->createCustomizedCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorttext,$shorturl,$mobileNoCount);  
							...
							{  
								if($campaign_id) {
									...
									$count = 1;
									//need to write job
									$largecampaignactivity_id = $this->Campaign_model->createLargeCampaignActivity_New($this->_userId,$campaign_id,$original_file,$total_no_of_sms,$this->input->post('mobile_column'),$sms_text,$is_schedule,$from_row,$to_row);
									 
									//$current_balance = $this->Campaign_model->get_SMSCredits($this->_userId);
									//$this->Campaign_model->insert_SMSCredits_logs($this->_userId, $total_no_of_sms,$current_balance,$campaign_id);
								//	$this->Campaign_model->deductSMSCredits($this->_userId, $total_no_of_sms);


									   redirect('campaign/viewcampaigns'); //changed on 2016-06-16
								}
							}
							else if($campaign_id) {
								//echo "test inside of campaign";exit;
								$count = 1;
								 $sendShorturl = '';  $sendShorturl = '';$newShortText = '';$newShortInput = '';
								 $filter = $shorturl_input;
								 $filter = array_filter($filter);
								foreach($data_array as $mobile_arr_var) {

									$mobile_no = $mobile_arr_var[0];
									$message = $mobile_arr_var[1];
									...
									if($shorturl_input != NULL)  
									{
										...
										$this->Campaign_model->updateSMSText($campaign_id,$message);
									...
									if($is_schedule) {
										...
										if($count <= 100) {
											...
											if($is_block_listed){
												...
												$this->Campaign_model->campaignToNormalShorturl($campaign_id,$sendShorturl, $sms_length, $mobile_no, $message,$sms_port, 2, $error_text);
											} else {
												...
												if(!$this->_userType){
													...
													if($isDND){
													 ...
													 $this->Campaign_model->campaignToNormalShorturl($campaign_id, $sendShorturl, $sms_length, $mobile_no, $message,$sms_port, 3, $error_text);
													} else {

														//$this->Campaign_model->campaignTo($campaign_id, $sms_length, $mobile_no, $message,$sms_port);
													 	$this->Campaign_model->campaignToNormalShorturl($campaign_id, $sendShorturl, $sms_length, $mobile_no, $message,$sms_port);  
														
														$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)."&to=91$mobile_no&text=".urlencode($message);
														$URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode("$IP/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
									...
									$current_balance = $this->Campaign_model->get_SMSCredits($this->_userId);
									$this->Campaign_model->insert_SMSCredits_logs($this->_userId, $total_no_of_sms,$current_balance,$campaign_id);
									$this->Campaign_model->deductSMSCredits($this->_userId, $sms_length);
									$count++;
								}
								 ...
								 if(strlen($mobile_no)>0 && $mobile_no!=0) {
									...
									for($j=0;$j<$varpositionscount;$j++) {
										...
										$message .= trim($uploded_data[$i][$colIndex]);	
											...
											//$message .= trim($var_positions_msg[$j]);
											$message .= $var_positions_msg[$j];
										} 
									}
									// calculate SMS length
							 ...
							 $campaign_id = $this->Campaign_model->createCustomizedCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$validMobileCount,$sms_length);  
							  ...
							  if($campaign_id) {
						  		...
						  		if($is_schedule) {
									...
									$this->Campaign_model->insert_SMSCredits_logs($this->_userId, $deductCustomCredits,$current_balance,$campaign_id);
								...
								if($shortInput) {
									...
									//} else {
										...
										if($is_duplicate){
											...
											$this->Campaign_model->campaignToNormalShorturl($campaign_id, $this->_userId,$sendShorturl, $sms_length, $mobile_no, $message,$sms_port, 16, $error_text);
										}else if($is_block_listed){
												$error_text = "Block Listed Number";
												// ADDED ON 2017-01-30 
												//$this->Campaign_model->campaignTo($campaign_id, $sms_length, $mobile_no, $message,$sms_port, 2, $error_text);
												$this->Campaign_model->campaignToNormalShorturl($campaign_id,$this->_userId,$sendShorturl, $sms_length, $mobile_no, $message,$sms_port, 2, $error_text);
											} else {
					 							...
					 							if(!$this->_userType){
													...
													if($isDND){
													 ...
													 $this->Campaign_model->campaignToNormalShorturl($campaign_id, $this->_userId,$sendShorturl, $sms_length, $mobile_no, $message,$sms_port, 3, $error_text);
													} else {

														//$this->Campaign_model->campaignTo($campaign_id, $sms_length, $mobile_no, $message,$sms_port);
													 	$this->Campaign_model->campaignToNormalShorturl($campaign_id, $this->_userId,$sendShorturl, $sms_length, $mobile_no, $message,$sms_port);  
...
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)."&to=91$mobile_no&text=".urlencode($message);
							...
							if($maxrows >= 100) {  
								...
								if($creditsCount > 0) {
									$this->Campaign_model->insert_SMSCredits_logs($this->_userId, $creditsCount,$current_balance,$campaign_id);
	...
	/* CREATE LARGE CAMPAIGN FOR CUSTOMIZED CAMPAIGN, by Saisandeepthi,2017-04-11 
	public function newVariableSMS()
	{
		$ViewTemplete="";$original_file = "";
		$file_type = "";$Filepath="";$offset = 0;$message = '';
				...
				if ($this->form_validation->run('variable_sms_form') == TRUE)
				{
					...
					if($error) {
						...
						$_sender = $sender;
							...
							$sms_type = 4;$total_no_of_sms = 0;$mobileNoCount = 0;

						  $campaign_id = $this->Campaign_model->createCustomizedCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$mobileNoCount);  
						if($campaign_id) { 
						$count = 1; 
						$largecampaignactivity_id = $this->Campaign_model->createLargeCampaignActivity_New($this->_userId,$campaign_id,$original_file,$total_no_of_sms,$mobileColumn,$sms_text,$is_schedule,$from_row,$to_row);
   						redirect('campaign/viewcampaigns');
						}	  
					}			  
...
$columns = array();
	...
	// changes done on 2016-06-09 19:00 by rakesh and krishna
	public function newVariableSMS_old()
	{
		if($this->input->post('file_upload')) {
			...
			if ($this->upload->do_upload()) {
				...
				$original_file = $file_name.$file_type;
				$objPHPExcel = $objReader->load($original_file);
				$worksheet = $objPHPExcel->getActiveSheet();
				$max_rows = $worksheet->getHighestRow();

				$string = '<table class="table table-bordered table-striped  no-footer" width="100%" style=" border: 1px solid #E5E5E5;" cellpadding=0 cellspacing=0>';

				$maxcols = PHPExcel_Cell::columnIndexFromString($worksheet->getHighestColumn());

				if($max_rows > 5) {
					$mx_rows_offset = 5;
				} else {
					$mx_rows_offset = $max_rows;
				}

				for ($i = 1; $i <= $mx_rows_offset; $i++) {
					if($i == 1){
						$string .= '<thead>';
					}
					$string .= "<tr>" ;
					for($j = 0; $j < $maxcols; $j++) {
						if($i == 1) {
							$columStringValue = PHPExcel_Cell::stringFromColumnIndex($j);
							$string .= '<th class="sendee-color" style=" font-weight:200 !important;">'.$columStringValue . ' - ' . $worksheet->getCellByColumnAndRow($j, $i)->getValue().'</th>';
						} else {
							$string .= '<td style="border: 1px solid #E5E5E5;">'.$worksheet->getCellByColumnAndRow($j, $i)->getValue().'</td>';
						}
						$uploded_data[$i-1][$j] = $worksheet->getCellByColumnAndRow($j, $i)->getValue();
					}
					$string .= '</tr>';
					if($i == 1){
						$string .='</thead>';
				...
				$columns = array('' => '--select--');
				for($k = 0; $k < $maxcols; $k++) {
					$columnValue = PHPExcel_Cell::stringFromColumnIndex($k);
					$columns[$columnValue] = $columnValue . '-'. $worksheet->getCellByColumnAndRow($k, 1)->getValue();
				}
			...
			if ($this->form_validation->run('variable_sms_form') == TRUE) {
				...
				if($error) {
					...
					for ($i = $from_row; $i <= $to_row; $i++) {
						$mobile_no = trim($worksheet->getCellByColumnAndRow($mobile_no_column-1, $i)->getValue());
						if(strlen($mobile_no)>0 && $mobile_no!=0) {
							$message = "";
							for($j=0;$j<$varpositionscount;$j++) {
								if($j%2 == 1) {
					...
					if($total_no_of_sms > $this->_credits) {
						$this->_data['no_balance'] = "Insufficient SMS Credits. Require {$total_no_of_sms} credits";
					} else {
						 $configFile = $this->config->item('configFile');
		$functionsFile = $this->config->item('functionsFile');
       include($configFile);
	   include($functionsFile);
						...
						if($campaign_id) {
							...
							foreach($data_array as $mobile_arr_var) {
								...
								if($is_schedule) {
									...
									if($count <= 100) {
										//check is block listed number?
										//check for duplicate numbers? on 05-01-201
										//$is_duplicate=$this->Campaign_model->checkduplicate($username,$s//ms_text,$sender,$mobile_no);
										$is_block_listed = $this->Campaign_model->isBlockListed($mobile_no);
										if($is_block_listed){
											$error_text = "Block Listed Number";
											$this->Campaign_model->campaignTo($campaign_id, $sms_length, $mobile_no, $message,$sms_port, 2, $error_text);
											...
											if(!$this->_userType){
												...
												if($isDND){
													...
													."&to=91$mobile_no&text=".urlencode($message);
													$URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode("$IP/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
													http_send($URL, $sms_port);
												}
											} else {
								...
								$current_balance = $this->Campaign_model->get_SMSCredits($this->_userId);
								$this->Campaign_model->insert_SMSCredits_logs($this->_userId, $total_numbers_ex_invalidno,$current_balance,$campaign_id);
								$this->Campaign_model->deductSMSCredits($this->_userId, $sms_length);
								$count++;
							}
							...
							redirect('index.php/campaign/viewcampaigns');
						}
					}
				}
			}
			...
			$string = '<table width="100%" style="border: 1px solid #E5E5E5;" cellpadding=0 cellspacing=0>';
			...
			for ($i = 1; $i <= $mx_rows_offset; $i++) {
				...
				for($j = 0; $j < $maxcols; $j++) {
					...
					if($i == 1) {
						$columStringValue = PHPExcel_Cell::stringFromColumnIndex($j);
						$string .= '<th style="background-color:#E5E5E5; border: 1px solid #E5E5E5;">'.$columStringValue . ' - ' . $worksheet->getCellByColumnAndRow($j, $i)->getValue().'</th>';
					} else {
						$string .= '<td style="border: 1px solid #E5E5E5;">'.$worksheet->getCellByColumnAndRow($j, $i)->getValue().'</td>';
					}
					$uploded_data[$i-1][$j] = $worksheet->getCellByColumnAndRow($j, $i)->getValue();
	...
	public function viewcampaignssearch()
	{
		...
		$config["base_url"] = base_url() . "index.php/campaign/viewcampaignssearch";
		//$config["per_page"] = 10;
	...
	public function viewcampaigns1()
	{
		...
		$this->load->library('pagination');
		$config['base_url'] = site_url().'index.php/campaign/viewcampaigns';
		$config['total_rows'] = $total_campaigns;
		//$config['per_page'] = $limit;
		$this->pagination->initialize($config);
	...
	/*public function scheduledcampaigns()
	{
		...
		$this->load->library('pagination');
		$config['base_url'] = 'http://www.rkadvertisings.com/index.php/campaign/viewcampaigns/';
		$config['total_rows'] = $total_campaigns;
		$config['per_page'] = $limit;
		$this->pagination->initialize($config);
	...
	{
		$sender_name = $this->input->get('sender');
		$rangeA = $this->input->get('rangeA');
		$mobile_no_ = $this->input->get('mobile_no_');
		$status_ = $this->input->post('status_'); // not required
		if(strlen($rangeA)>0)
		{
			$split = explode("/",$rangeA);
			if(sizeof($split)==1)
		...
		endforeach;
		$campaign_ids =substr($campaigns_ids,0,strlen($campaigns_ids)-1);;
		$search_result_rs=$this->Campaign_model->get_search_results_download($this->_userId,$from_date,$to_date,$campaign_ids,$status_,$mobile_no_);
		...
		if($search_result_rs)
		{
			...
			$string .= "Sl.No\tCampaignID\tSender Name\tMobile No\tSent Time\tLast Updated\tAcknowledgment\tMessage Text\tSMS Length\tCredits Charged\tProvider\tLocation\n";
			$rowId=1;
			foreach($search_result_rs as $row)
			{
					


				$campaign_id = $row->campaign_id;
				//$campaign_name_= $senderNamesArray[$row->campaign_id];
				$sender_name= $senderNamesArray[$row->campaign_id];
				$campaign_type=0;
				$msglength=strlen( $row->sms_text);
				...
				$message_ = trim(preg_replace('/\s+/', ' ', $message_)); // remove all new lines from string.
 				$string .= $rowId++."\t".$campaign_id."\t".$sender_name."\t".$to_m."\t".$row->sent_on."\t".$row->delivered_on."\t".$error_text."\t".$message_."\t".$msglength."\t".$chargedcredits."\t".$operator_name."\t".$service_area;
				$string .= "\n";
			}
...
$string = '';
	...
	public function viewReport()
	{
		...
		if($campaign_id) {
			$this->load->model('Campaign_model');
			$campaign_details_rs = $this->Campaign_model->get_campaign_details($campaign_id,$this->_userId);
			if($campaign_details_rs) {
				...
				$this->load->library('pagination'); 
				$config['base_url'] = site_url().'index.php/campaign/viewReport/campaign/'.$campaign_id.'/';
...
if($shorturl_input != NULL) {  
		...
		$campaign_id = $this->Campaign_model->createUnicodeShortUrlCampaign($this->_userId,$sms_type,$is_unicode_sms,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$total_numbers);
		  ...
		  $sendShorturl = ''; $smstext = $sms_text; 
			...
			if($is_schedule) {
				...
				for($i=0; $i < $offset; $i++)
				{
					...
					if($is_block_listed){
						$error_text = "Block Listed Number";
						$this->Campaign_model->campaignToNormalShorturl($campaign_id,$this->_userId, $sendShorturl,$sms_length, $mobile_no, $smstext, 2, $error_text);
					} else {
						if(!$this->_userType){
							//check for dnd number
							$isDND = $this->Campaign_model->checkIsDND($mobile_no);
							...
							{
								$error_text = "DND Number";
								$this->Campaign_model->campaignToNormalShorturl($campaign_id,$this->_userId,$sendShorturl, $sms_length, $mobile_no, $smstext, $sms_port,3,$error_text );
							} else {
								...
								$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("$IP/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
								
								
								http_send($URL, $sms_port);
								
								$this->Campaign_model->campaignToNormalShorturl($campaign_id,$this->_userId,$sendShorturl, $sms_length, $mobile_no, $smstext,$sms_port);
							}
						} else {
							if($this->_dndCheck)
							{
	...
	/* With working 4 short ursl, 2017_3_05
	private function _sendUnicodeSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type,$is_unicode_sms ,$is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
	{
...
$invalidNosCount=$total_numbers-$validNosCount;
	...
	$total_numbers_ex_invalidno = $total_numbers_ex_invalidno*$sms_length;
		...
		$campaign_id = $this->Campaign_model->createUnicodeShortUrlCampaign($this->_userId,$sms_type,$is_unicode_sms,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorttext,$shorturl,$total_numbers);
 ...
 $filter = $shorturl_input;
		 $filter = array_filter($filter);
			if($is_schedule) {
				...
				for($i=0; $i < $offset; $i++)
				{
					...
					if($is_block_listed){
						$error_text = "Block Listed Number";
						$this->Campaign_model->campaignToNormalShorturl($campaign_id, $sendShorturl,$sms_length, $mobile_no, $smstext, 2, $error_text);
					} else {
						if(!$this->_userType){
							//check for dnd number
							$isDND = $this->Campaign_model->checkIsDND($mobile_no);
							if($isDND)
							{
								...
								$this->Campaign_model->campaignToNormalShorturl($campaign_id,$sendShorturl, $sms_length, $mobile_no, $smstext,$sms_port);
							}
						} else {
							if($this->_dndCheck)
							{
	...
	private function _sendUnicodeSMS($sms_text, $sender, $to_mobileno, $sms_type,$is_unicode_sms ,$is_schedule, $scheduled_date,$campaign_name)
	{
				...
				for($i=0; $i < $offset; $i++)
				{
					...
					if($is_block_listed){
						...
						if(!$this->_userType){
							...
							{
								$error_text = "DND Number";
								$this->Campaign_model->campaignTo($campaign_id, $sms_length, $mobile_no, $sms_text, $sms_port,3,$error_text );
							} else {
								$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)."&to=91$mobile_no&text=".urlencode($sms_text);
								$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("$IP/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
								http_send($URL, $sms_port);
	...
	public function misreports()
	{
		...
		{
			$data['result'] = $this->Campaign_model->get_all_smsreports($this->_userId,$from_date,$to_date);
		}
	...
	public function api_misreports()
	{
		...
		{
			$data['result'] = $this->Campaign_model->get_all_api_smsreports($this->_userId,$from_date,$to_date);
		}
	...
	public function credits_logs()
	{
		...
		$config['base_url'] = site_url().'index.php/campaign/credits_logs';
 ...
 */
				...
				if($error) {
					...
					if($totalMobileCount > $normalSMSLimit) { 
						...
						$sms_text_spl = str_replace($special_char, ' ', $sms_text); 
  	...
  	/*private function _sendFileSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text) {
	...
	//ADDED ON 2017-01-23
	  ...
	  $campaign_id = $this->Campaign_model->createFileShortUrlCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorttext,$shorturl,$total_numbers);
...
if(count($filter) > 0) {
		            ...
		            $IP = $this->_data['kernalIp'];
						if($is_block_listed){
							...
							echo $error_text = "Block Listed Number";
							...
							echo $error_text = "Invalid Number";
							...
							if(!$this->_userType){
								...
								$isDND = $this->Campaign_model->checkIsDND($mobile_no);
								 if($isDND)
								 {
									...
									$URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode("$IP/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&file_name=dlr.php");
									 http_send($URL, $sms_port); 
 ...
 if(count($filter) > 0) {
	...
	$CountryRoute = array("971" => "34013","91" => "34013","972" => "34013","971" => "34013","968" => "34013","966" => "34013","974" => "34013","90" => "34013","973" => "34013","962" => "34013","965" => "34013","60" => "34013","95" => "34013","63" => "34013","65" => "34013","84" => "34013","62" => "34013");
  	...
  	$sms_text = trim($sms_text);
	// calculate SMS length  
	$sms_text_spl2 = $sms_text1 = strtolower($sms_text);
	//remove special characters 
	//changed on 4thmay2015 by bharath for characters count.
	$special_char = array(',','.','-','!','&','’','_','#','(',')','<','>','%',':',';','.','?','*','+','-','/','&','∼','!','=',',','"','’','’');
	  ...
	  include($functionsFile);   
		...
		if($is_schedule) {
			...
			$this->Campaign_model->update_campaign_status($campaign_id, '1');
 				...
 				$this->Campaign_model->insert_SMSCredits_logs($this->_userId, $deductTotalFileCredits,$current_balance,$campaign_id);
...
if($pos === false) {
						...
						if(strlen($smstext)>160)
							$sms_length_tmp = ceil(strlen($smstext)/153);
						else
							$sms_length_tmp = ceil(strlen($smstext)/160);
		            ...
		            $IP = $this->_data['kernalIp'];
						if($is_block_listed){
							  ...
							  $error_text = "Block Listed Number";
							$this->Campaign_model->campaignToNormalShorturl($campaign_id, $this->_userId,$sendShorturl, $sms_length, $mobile_no, $smstext, $sms_port,2, $error_text);
						}elseif($is_invalid_no){


							  $error_text = "Invalid Number";
							$this->Campaign_model->campaignToNormalShorturl($campaign_id, $this->_userId,$sendShorturl, $sms_length, $mobile_no, $smstext, $sms_port,16, $error_text);
								...
								$isDND = $this->Campaign_model->checkIsDND($mobile_no);
								 if($isDND)
								 {
									...
									$URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode("$IP/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&file_name=dlr.php");
									 ...
									 $this->Campaign_model->campaignToNormalShorturl($campaign_id, $this->_userId,$sendShorturl, $sms_length, $mobile_no, $smstext, $sms_port,$dlr = null, $error_text= null);
								}
...
if($pos === false) {
	 ...
	 private function _sendNormalSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
   {
	...
	$sms_port = 0;
	//get port number based on port type
	  $sms_port = $this->Campaign_model->getFirstPriorityPort($portType);    //42113
	//This is added for the purpose of trans scrub users on 23/08/2014
 if($this->_userId==5082){ // sandeep users id for testing purpose
	$sms_port=47413;
}
...
if($pos === false) {
	...
	private function _sendNormalSMSShortUrl1($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
       {
...
if($pos === false) {
	...
	/* With working 4 short urls, 2017_3_05
   private function _sendNormalSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
   {
   ...
   */
	...
	public function shorturl_allreports()
	{
		...
		$total_rows1= $this->Campaign_model->shorturl_reportssearch_newcount($this->_userId,$from_date,$to_date);
 		...
 		$data['shorturlreports']=$this->Campaign_model->shorturl_reportssearch_new($this->_userId,$limit,$off_set,$from_date,$to_date);	  
		      ...
		      $this->load->library('pagination');
			$config['base_url'] = base_url().'index.php/campaign/shorturl_allreports';
	...
	public function shortcode_reports()
	 ...
	 //  $data['shorturlreports']=$this->Campaign_model->shortcode_all_reports($this->_userId,$shortcode,$campaign_id,$limit,$offset);
		...
		$this->load->library('pagination');
		$config['base_url'] = base_url().'index.php/campaign/shortcode_reports/'.$campaign_id.'/'.$shortcode.'/';
		$config['total_rows'] = @$total_rows;
		$config['per_page'] = $limit; 
		$config['full_tag_open'] = "<ul class='pagination'>";
		$config['full_tag_close'] ="</ul>";
	...
	public function missedcall_allreports()
	{
		...
		$this->_data['page_title'] = "All Missed Call Reports";
		   ...
		   if($this->uri->segment(3)=='sessout')
		 {
			 ...
			 redirect('index.php/campaign/missedcall_allreports');
		...
		// ADDED ON 2017-01-31
		$config['base_url'] = site_url().'index.php/campaign/missedcall_allreports/';
	...
	public function upload_file_data() { 
		...
		if($this->input->post('api_report_Search')) {
			...
			$mobile_number = $this->input->post('mobile_number');
 			$apireportview =  $this->Campaign_model->get_sms_api_details_by_filter($ondate,$this->_userId,$days_diff,$sender,$mobile_number);  
      		}else{
      			$apireportview =  $this->Campaign_model->get_sms_api_details($ondate,$this->_userId,$days_diff,$limit,$offset);

      		} 

  		$total_rows =   $this->Campaign_model->get_sms_api_details_total($ondate,$this->_userId,$days_diff); 
		$this->_data['dlr_report_type']=$this->_dlr_report_type;

		 $this->_data['dlr_report'] =  $this->Campaign_model->get_api_campaign_count($this->_userId,$days_diff,$ondate);

 		//$this->_data['dlr_report'] =  $this->Campaign_model->get_api_campaign_count($this->_userId,$days_diff,$messageId,$ondate);	
   
		$this->_data['apireportview'] = $apireportview; 

		$this->load->library('pagination');
		$config['base_url'] = site_url().'index.php/campaign/apiViewReport/campaign/'.$ondate.'/';
		$config['full_tag_open'] = "<div><ul class='pagination'>";
...
$string = '';
		if($ondate) { 
			...
			if($rs) {
				...
				$string .= "DateTime \t To Mobile No \t No.of SMS \t Sender Name \t Message \t Status \n";
				 
				foreach ($rs as $row) {
				 					$message_ = preg_replace('/\s+/', ' ', $row->message);
					$string .= $row->ondate."\t".$row->to_mobileno."\t".$row->noofmessages."\t".$row->sender_name."\t".$message_."\t";
				...
				if($urlCount > 0 ) {	
					...
					$totalCredits = $urlCount;
	 				if($this->_shorturlCredits >= $totalCredits) {
						...
						redirect('index.php/campaign/bulkShorturl/noCredits');
					}
				}else{   	 
					redirect('index.php/campaign/bulkShorturl/invalid');
				}
			}else{
				redirect('index.php/campaign/bulkShorturl/noCredits');
			} 		    
				...
				if($urlCount > 0 ) {	
					$totalCredits = $urlCount;
	 				if($this->_shorturlCredits >= $totalCredits) {
						...
						redirect('index.php/campaign/bulkshortUrlimg/noCredits');
					}
				}else{   	 
					redirect('index.php/campaign/bulkshortUrlimg/invalid');
				}
			}else{
				redirect('index.php/campaign/bulkshortUrlimg/noCredits');
			}    
				...
				if($urlCount > 0 ) {
					$totalCredits = $urlCount;
	 				if($this->_shorturlCredits >= $totalCredits) {
						...
						redirect('index.php/campaign/bulkshortUrlvideo/noCredits');
					}
				}else{   	 
					redirect('index.php/campaign/bulkshortUrlvideo/invalid');
				}
			}else{
				redirect('index.php/campaign/bulkshortUrlvideo/noCredits');
			}    
				...
				if($urlCount > 0 ) {
					$totalCredits = $urlCount;
	 				if($this->_shorturlCredits >= $totalCredits) {
						...
						redirect('index.php/campaign/bulkshortUrlaudio/noCredits');
					}
				}else{   	   
					redirect('index.php/campaign/bulkshortUrlaudio/invalid');
				}
			}else{
				redirect('index.php/campaign/bulkshortUrlaudio/noCredits');
			}    
	 	...
	 	if($shorturl_id) 
		{
			...
			if($data['shorturl_number'] != '' && $data['shorturlenter'] != '')
			{
 			 ...
 			 $this->Campaign_model->createUserBulShorturl($data_array,$shortUrl_array); 
				 ...
				 redirect('index.php/campaign/shorturlReports');
	...
	public function shorturlReports() {   
		...
		$this->_data['userurl'] = $this->Campaign_model->getUserShortUrls($this->_userId,$from_date,$to_date,$limit,$offset);
		 ...
		 $total_rows = $this->Campaign_model->getUserShortUrlsCount($this->_userId,$from_date,$to_date);
		      ...
		      $this->load->library('pagination');
			$config['base_url'] = base_url().'index.php/campaign/shorturlReports';
		      $config['total_rows'] = @$total_rows;
			$config['per_page'] = $limit; 
			$config['full_tag_open'] = "<ul class='pagination'>";
			$config['full_tag_close'] ="</ul>";
	...
	public function bulkurl() { 
		...
		if($urlId) {		  
			...
			$total_rows = $this->Campaign_model->Campaign_model->getUserUrlsCount($this->_userId,$urlId);
		      ...
		      $this->load->library('pagination');
			$config['base_url'] = site_url().'index.php/campaign/bulkurl/'.$urlId.'/';
...
header("Content-type: application/octet-stream");
	...
	public function confirm_order() {  
		...
		if($this->input->post('confirm_order')) {
			...
			$this->_data['total_amount'] = $service_tax+$amount;
			$user_id = $this->_userId;    
		...
		if($this->input->post('place_order')) {  
			...
			if($product_id) {   
				...
				/*
				$val = 'amount='.base64_encode($amount).'&tax_amount='.base64_encode($tax_amount).'&total_amount='.base64_encode($gettotal_amount).'&name='.base64_encode($name).'&sms_price='.base64_encode($sms_price).'&trnsale='.base64_encode($urlcount).'&customerid='.base64_encode($user_id).'&address1='.base64_encode($address).'&address2='.base64_encode($address).'&state='.base64_encode($getstate_id).'&city='.base64_encode($getcity_id).'&zip='.base64_encode($zipcode).'&mobile='.base64_encode($mobile).'&email='.base64_encode($email).'&shorturl_credits='.base64_encode($shorturl_credits).
				...
				redirect('index.php/campaign/addShorturlCredits');
			}  
		...
		if($this->input->post('renewal_package')) { 
  		...
  		if($this->input->post('place_order')) {
			...
			if($product_id) {  
				...
				$testssl_url = base_url()."index.php/payment/TestSsl.php?ids=".base64_encode($product_id)."&".$val;
	  
				  redirect($testssl_url);    
			}else{
				redirect('index.php/campaign/myLongcodePackage'); 
			}   
 		}  
...
//1 21 25500
  ...
  public function getshortcode_reports() {
		...
		$this->load->library('pagination');
		$config['base_url'] = base_url().'index.php/campaign/getshortcode_reports/'.$campaign_id."/".$shortcode."/";
		$config['total_rows'] = @$total_rows;  
		$config['uri_segment'] =5;
		$config['per_page'] = $limit; 
		$config['full_tag_open'] = "<ul class='pagination'>";
	...
	{ 
		...
		if($this->_data['selected_sms_type'] == 'ApiSms') {
			...
			$total_days = $day_count = 0;
			$api_data = array();  
			$campaigns_api_data = $this->Campaign_model->get_SMS_API_Reports1($this->_userId,$mobileNum,$fd,$td);	
			if(!$off_set) { 			
			...
			if(!$mobileNum) {
				$total_days = $this->Campaign_model->get_total_sms_api_days($this->_userId,$fd,$td);			
										
			
	  			$days_data =  $this->Campaign_model->get_sms_api_days($this->_userId,$fd,$td,$off_set,$limit);
				...
				$total_days = 0;//$this->Campaign_model->get_total_sms_api_days1($this->_userId,$mobileNum,$days_diff,$fd,$td);			  
 
  				$days_data =  $this->Campaign_model->get_sms_api_days1($this->_userId,$mobileNum,$days_diff,$fd,$td,$off_set,$limit);
