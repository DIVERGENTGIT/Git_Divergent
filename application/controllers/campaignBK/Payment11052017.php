...
$url="https://eazypay.icicibank.com/EazyPG?merchantid=129258&mandatory fields=$Encrypt_MandatoryFields&optional fields=&returnurl=$Encrypt_ReturnUrl&Reference No=$Encrypt_ReferenceNo&submerchantid=$Encrypt_SubMerchantId&transaction amount=$Encrypt_TransactionAmount&paymode=$Encrypt_PaymentMode";
redirect($url);
}
public function aes128Encrypt($str,$key)
{
	...
	public function confirm_order()
	{
		...
		$six_digit_random_number = mt_rand(100000, 999999);
			 $sql="update price_enquery set transaction_id='$six_digit_random_number'  where id in ($ids)";
				$this->db->query($sql);
				

				$query = "insert into transaction_history (user_id,noofsms,sms_price,amount,tax_amount,total_amount,transaction_id)
 values ($userid,$qty,$sms_price,$amount,$tax_amount,$gettotal_amount,'$six_digit_random_number')";
 				$this->db->query($query);

		$this->encryptPaynow($gettotal_amount,$six_digit_random_number);
