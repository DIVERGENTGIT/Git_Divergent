	...
	{ 	
		...
		if($this->input->post('username')!='' && $this->input->post('userpass'))
		{
			...
			if ($rs)
			{
				...
				if(!$is_blocked)
				{
					$this->user_model->updateLoginTime($result->user_id);
					...
					{
					redirect('index.php/missedcall/messages');
					}
					else
					{
						...
						if($result->mverify==0)
						{
						redirect('index.php/myaccount/index');
						}
						else
						{
							...
							if(count($userexist)>0)
							{
								...
								//redirect('Payment/paynow');
								 redirect('index.php/products/index');
								}
								else
								{
...
$data['not_exist'] = "Your Password not sent!. Please Contact Administrator";
	...
	public function logout()
	{
		...
		/**** remove order number end***/
		$this->session->unset_userdata('user_id'); // this new added
		//$this->session->sess_destroy(); // reason this remove all session values (In other appllications)
		$redirect = base_url().'login.html';
		redirect($redirect);  
		//redirect('http://localhost/smsstriker/login.html');
