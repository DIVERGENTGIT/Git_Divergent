...
$columns = array();
				...
				//echo $string;  
			  		...
			  		$data['error'] = 'Uploaded file is not a valid MIME Type.'; 
				 } 
			}
			catch (Exception $E)
			{
...
$selected_sms_type = $this->input->post('selected_sms_type');
 			$this->session->set_userdata('selected_sms_type', $selected_sms_type);
...
$total_rows1=$this->Campaign_model->shorturl_reportssearch_newcount($this->_userId);

 $data['shorturlreports']=$this->Campaign_model->shorturl_reportssearch_new($this->_userId,$limit,$off_set);	  
	...
	public function bulkurl() { 
		...
		if($urlId) {		
			...
			$total_rows = $this->Campaign_model->Campaign_model->getUserUrlsCount($this->_userId,$urlId);
		      $this->load->library('pagination');
			$config['base_url'] = base_url().'/campaign/bulkurl/'.$urlId;
		      $config['total_rows'] = @$total_rows;
			$config['per_page'] = $limit; 
			$config['full_tag_open'] = "<ul class='pagination'>";
			$config['full_tag_close'] ="</ul>";
