		...
		if($campaign_id) {
			...
			if($rs) {
				...
				foreach ($campaign_numbers as $row) {
					...
					$slNo = $rowId++;
    					$values = array('Sl.No'=> $slNo,'MSG ID'=>$campaign_id,'Campaign Name'=>$campaign_name_,'Sender Name'=>$sender_name ,'Mobile No'=>$to_m,'Sent Time' => $row->sent_on,"Last Updated" => $row->delivered_on ,"Acknowledgment" => $error_text,"Message Text" => $message_,"SMS Length" => $msglength,"Credits Charged" => $chargedcredits,"Provider" => $operator_name,"Location" => $service_area  );
...
//$objPHPExcel->getActiveSheet()->getColumnDimension('G'.$rowCount)->setWidth(50);
		...
		foreach ($rs as $row) { 
		    ...
		    if(strlen($toMobileNum) < 10){
 				...
 				$values = array('DateTime'=> $createdDate,'DeliveredOn'=>$deliveredDate,'ToMobileNo'=>$toMobileNum,'No.ofSMS'=>$numberOFSMS ,'SenderName'=>$senderName,'Status' => $dlrStatus,"Message" => $message_ );
				array_push($reports,$values);     
