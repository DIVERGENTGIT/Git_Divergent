							  ...
							  if($campaign_id) {
						  		...
						  		if($is_schedule) {
									...
									$this->Campaign_model->deductSMSCredits($this->_userId, $deductCustomCredits);   							  										$this->Campaign_model->update_campaign_status($campaign_id, '1');
 									$largecampaignactivity_id = $this->Campaign_model->createLargeCampaignActivity_New($this->_userId,$campaign_id,$original_file,$total_no_of_sms,$mobile_no_column,$sms_text,$is_schedule,$from_row,$to_row,$this->_International);
 					  				//redirect('campaign/viewcampaigns');
								}else{
...
$URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)."&to=91$mobile_no&text=".urlencode($message);
							 ...
							 if($is_schedule) {
								if($this->_International == 1){
									$this->Campaign_model->update_campaign_status($campaign_id, '9');
								}else{
							 		$this->Campaign_model->update_campaign_status($campaign_id, '1');
								}
							 } else {
...
error_log($log."\r\n",3,"/var/www/html/logs/strikerCampReturnCredits_log/file_first_process".$date.".log");      
 ...
 $ipAddress = $this->ip_address();
				...
				$largecampaignactivity_id = $this->Campaign_model->createLargeCampaignActivity_New($this->_userId,$campaign_id,$originalFile,$total_no_of_sms,$mobileColumn,$sms_text,$is_schedule,$from_row,$to_row,$this->_International);
				// redirect('campaign/viewcampaigns');  
	...
	{ 
		...
		if($this->_data['selected_sms_type'] == 'ApiSms') {
			...
			if(!$this->session->userdata('noRecordsFound')) {  
				$campaigns_api_data = $this->Campaign_model->get_SMS_API_Reports1($userID,$mobileNum,$fd,$td);	
				if(!$off_set) { 			
...
but always getting this error Request header field Content-Type is not allowed by Access-Control-Allow-Headers in preflight response.";
	...
	public function calculateSMSLength() {
		...
		$apireportview = $api_data = array();
		 $total_days = 0;
		if($mobileNum != 0) {
		  $apireportview = $this->Campaign_model->get_sms_api_details_filter($userID,$mobileNum,$days_diff,$fromDate,$toDate,$off_set,$limit);
 
		$total_days = $this->Campaign_model->get_sms_api_details_filter_count($userID,$days_diff,$mobileNum,$fromDate,$toDate,$off_set,$limit);  
