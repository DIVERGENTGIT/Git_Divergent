...
Class campaign extends CI_Controller
{
		...
		if ($this->input->post('sendsms')) {
			...
			if ($this->form_validation->run('single_sms_form') == TRUE) {
				...
				if($is_schedule) {
					if(!$scheduled_date) {
						$error = true;
						$error_msg .= "Please enter schedule date<br />";
					}
					...
					{
						$error = true;
						$error_msg .= "Please enter valid schedule date<br />";
					}
				}
				...
				if($this->_userType != 1 )
				{
					...
					{
						$error = true;
						$error_msg .= "Non Promotion Timings i.e., Before 9 am and after 9 Pm.";
	...
	public function normalSMS_Tabs()
	{
		$this->_data['page_title'] = "Normal SMS";
		...
		if ($this->input->post('sendfilesms')) {
			$this->load->library('form_validation');
		...
		if ($this->input->post('sendunicodesms')) {
			$this->load->library('form_validation');
			if ($this->form_validation->run('single_sms_form') == TRUE) {
				//input variables
				$campaign_name = $this->input->post('campaign_name');
		...
		} else if($this->input->post('sendcustomsms')) {
...
$IP = $this->_data['kernalIp'];
		...
		$this->load->view('includes/header',$this->_data);
		$this->load->view('includes/leftmenu');
		//$this->load->view('campaign/normal-sms');

		$this->load->view('campaign/smssevices');
		$this->load->view('includes/footer');
	}

	 private function _sendSMS($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name)
    {
	...
	$total_numbers_ex_invalidno = $total_numbers_ex_invalidno*$sms_length;
        ...
        if($campaign_id) {
            //deducting credits
             ...
             include("/var/www/vhosts/www.smsstriker.com/htdocs/smslib/config.inc");
	   include("/var/www/vhosts/www.smsstriker.com/htdocs/smslib/functions.inc");
		...
		$sendShorturl='';
                ...
                for($i=0; $i < $offset; $i++)
                {
                   ...
                   $smstext=$sms_text;
                      ...
                      if($this->input->post('shorturl_input')!='')
                      {
...
$smstext = str_replace($newshorturl_text, "$getsendShorturl", $newsms_text); 
                  ...
                  //$is_duplicate=$this->campaign_model->checkduplicate($username,$sms_text,$sender,$mobile_no);
                    ...
                    if($is_block_listed){
                         ...
                         $error_text = "Block Listed Number";
                        $this->Campaign_model->campaignTo($campaign_id,$sms_length, $mobile_no, $smstext, 2, $error_text);
                    } 
                    ...
                    elseif($is_invalid_no){
                            ...
                            $this->Campaign_model->campaignTo($campaign_id, $sms_length, $mobile_no, $smstext, 16, $error_text,$sms_port);
                   ...
                   if(!$this->_userType){
							...
							{
		                        $error_text = "DND Number";	
                                $this->Campaign_model->campaignTo($campaign_id,$sms_length, $mobile_no, $smstext, 3,$error_text );
                            } else {

		                if($this->_International==1)
				{
	                                $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)
                                    ."&to=$mobile_no&text=".urlencode($smstext); 

				}else{
				
			
	                                $URL = "/cgi-bin/sendsms?username=".USERNAME."&password=".PASSWORD."&from=".urlencode($sender_name)
                                    ."&to=91$mobile_no&text=".urlencode($smstext);
				}
                               ...
                               $URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode("$IP/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&file_name=dlr.php");
                                ...
                                $this->Campaign_model->campaignTo($campaign_id,$sms_length, $mobile_no, $smstext, $dlr = null, $error_text= null);
                            }
                        } else {
							if($this->_dndCheck)
							{
...
//exit;
		 ...
		 if ($this->input->post('sendsms')) {
			...
			if ($this->form_validation->run('file_sms_form') == TRUE) {
				...
				if($is_schedule) {
					if(!$scheduled_date) {
						$error = true;
						$this->_data['error'] = $error_msg .= "Please enter schedule date<br />";
					}
					...
					{
						$error = true;
						$this->_data['error'] = $error_msg .= "Please enter valid schedule date<br />";
					}
				}
	...
	public function newVariableSMS()
	{
		...
		{   
			...
			/** ADDED ON 2017-01-30
			require('/var/www/html/smsstriker/spreadsheet-reader-master/php-excel-reader/excel_reader2.php');
			require('/var/www/html/smsstriker/spreadsheet-reader-master/SpreadsheetReader.php');**/
			      ...
			      $uploded_data[$i-1][$j] = $Row[$j];
						...
						if($total_no_of_sms > $this->_credits) {
							...
							if($shorturl_input[0] != NULL) {
							 ...
							 $campaign_id = $this->Campaign_model->createCustomizedCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorttext,$shorturl);
							...
							{  
								if($campaign_id) {
									...
									$count = 1;
									//need to write job
									$largecampaignactivity_id = $this->Campaign_model->createLargeCampaignActivity($this->_userId,$campaign_id,$original_file,$total_no_of_sms,$this->input->post('mobile_column'),$sms_text,$is_schedule,$from_row,$to_row);
												...
												if(!$this->_userType){
													...
													if($isDND){
													 ...
													 $this->Campaign_model->campaignToNormalShorturl($campaign_id, $sendShorturl, $sms_length, $mobile_no, $message,$sms_port, 3, $error_text);
														...
														$URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode("$IP:81/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
														http_send($URL, $sms_port);
													}
												} else {
													//$this->Campaign_model->campaignTo($campaign_id, $sms_length, $mobile_no, $message,$sms_port);
					...
					$this->_data['error'] = 'Customize your file data.';
				}
	...
	public function uploadCustomSmsFile() {  
		...
		$this->_data['error'] = '';
			...
			if ($this->upload->do_upload()) {
				...
				/** ADDED ON 2017-01-30	require('/var/www/html/smsstriker/spreadsheet-reader-master/php-excel-reader/excel_reader2.php');
					require('/var/www/html/smsstriker/spreadsheet-reader-master/SpreadsheetReader.php');	**/
				...
				date_default_timezone_set('UTC');
				$StartMem = memory_get_usage();
			}else {$this->_data['error'] = 'The uploaded file exceeds the maximum allowed size';} 
		...
		if($ViewTemplete=="Y")
		{
			...
			try
			{
				...
				$this->_data['file_name'] = $original_file;
				$this->_data['file_type'] = $file_type[1];
				$this->_data['maxcols'] = $maxcols;
				$this->_data['max_rows'] = $max_rows;
				$this->_data['uploaded_data'] = $string;
		...
		if(empty($this->_data['uploaded_data']) ) {
	...
	public function viewcampaigns()
	{
		...
		if($this->input->post('selected_sms_type') == 'ApiSms') {
			$limit = 25;
			$day_count = 0;
			$api_data = array();	
			$off_set = $this->uri->segment(3);
			...
			$campaigns_api_data = $this->Campaign_model->get_SMS_API_Reports($this->_userId,$fd,$td);
 		...
 		}else if($this->input->post('selected_sms_type') == 'NormalSms') {
	...
	public function editcampaign()
	{
		...
		$data['campaign_id'] = $this->_campaign_id;
		$data['page_title'] = "Edit Schedule sms";
		 $data['isftpuser'] = $this->_data['isftpuser'];
		$this->load->view('includes/header',$data);
	...
	public function updateschedulecampaign()
	{
		...
		$data['edited'] = "Scheduled sms has been updated";
		$data['page_title'] = "Edit Schedule sms";
		 $data['isftpuser'] = $this->_data['isftpuser'];
		$this->load->view('includes/header',$data);
	...
	public function cancelcampaign()
	{
		...
		if($campaign_id) {
			...
			if($rs) {
				...
				redirect('campaign/viewcampaigns/deleted');
			} else {
				$data['available_credits'] = $this->_credits;
	...
	public function downloadDlrReport_new()
	{
		...
		if($campaign_id) {
			...
			if($rs) {
				...
				$string .= "Sl.No\tMSGID\tCampaignName\tSenderName\tMobileNo\tSentTime\tLastUpdated\tAcknowledgment\tMessageText\tSMSLength\tCreditsCharged\tProvider\tLocation\n";
	...
	public function viewReport()
	{
		...
		if($campaign_id) {
			...
			if($campaign_details_rs) {
				...
				$total_rows = $this->Campaign_model->getTotalNumbersCount($campaign_id, $days_diff, $campaign_ondate);

				$off_set = $this->uri->segment(5);
				$limit = 25;

				$dlr_report_old = $this->Campaign_model->getCampaignNumbers($campaign_id, $mobile_no, $days_diff, $campaign_ondate, $off_set, $limit);
	...
	private function _sendUnicodeSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type,$is_unicode_sms ,$is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
	{
		...
		$campaign_id = $this->Campaign_model->createUnicodeShortUrlCampaign($this->_userId,$sms_type,$is_unicode_sms,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorttext,$shorturl);
					...
					if($is_block_listed){
						...
						if(!$this->_userType){
							...
							if($isDND)
							{
								...
								$URL .= "$unicode_sms$mclass&dlr-mask=19&dlr-url=".urlencode("$IP:81/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T");
	...
	//ADDED ON 2017-01-23
	  ...
	  $campaign_id = $this->Campaign_model->createFileShortUrlCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorttext,$shorturl);
   ...
   private function _sendNormalSMSShortUrl($sms_text, $sender, $to_mobileno, $sms_type, $is_schedule, $scheduled_date,$campaign_name,$shorturl_input,$shorturl_text)
   {
	...
	//ADDED ON 2017-01-23
	  ...
	  $campaign_id = $this->Campaign_model->createShortUrlCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorttext,$shorturl);
	...
	public function shorturl_allreports()
	{
		...
		if($this->input->post('shorturl_report_search') != '')
		{
		 ...
		 if($this->input->post('shorturl_report_reset') != '')
		{
		      $this->session->unset_userdata('from_date');
		        $this->session->unset_userdata('to_date');
		...
		//$data['shorturlreports']=$this->Campaign_model->shorturl_all_reports($this->_userId,$off_set,$limit);
 

		$data['shorturlreportsdropdown']=$this->Campaign_model->shorturl_dropdown_reports($this->_userId);
		
 
 
		  $count_rows = $this->Campaign_model->shorturl_total_rows($this->_userId,$from_date,$to_date,$longurl);
			 

		  $data['shorturlreports']=$this->Campaign_model->shorturl_reportssearch($this->_userId,$from_date,$to_date,$longurl,$off_set,$limit);
	    
		//print_r($data['shorturlreports']);exit;
		...
		if($this->input->post('shorturl_download'))
		{
			...
			//echo count($download_shorturl_result);exit;
			$reports = array();
		...
		$config['base_url'] = site_url().'/campaign/shorturl_allreports';
	...
	public function shortcode_reports()
		...
		$data['shorturlreports']=$this->Campaign_model->shortcode_all_reports($this->_userId,$shortcode);
		...
		if($this->input->post('shorturl_download'))
		{
			...
			{
			$download_shorturl_result=$this->Campaign_model->shortcode_all_reports($this->_userId,$shortcode);
			}
	...
	/* public function campaignallleadsFeedback()
	{
	...
	$userlist= $this->Campaign_model->get_reseller_users($user_id);
	$r_id=$userlist[0]->rid;

	$usersfeedback_result = $this->Campaign_model->put_userscampaignFeedback($r_id,$users_reason,$users_comment,$users_phoneno);
	    $result1=$usersfeedback_result;
	...
	/** ADDED ON 2017-02-2 **/	   
   	public function upload_encoded_image() {   
		...
		//$file_full_path = $_SERVER['DOCUMENT_ROOT'].'/smsstriker/upload_shorturl_image/'.$file_name;
		$file_full_path ='/var/www/vhosts/www.smsstriker.com/htdocs/strikerapp/upload_shorturl_image/'.$file_name;
	...
	public function upload_file_data() { 
  		...
  		if($_FILES != NULL) {
			...
			$file_name = $_FILES['file_input']['name'];
 			...
 			$path_name = "/var/www/vhosts/www.smsstriker.com/htdocs/strikerapp/upload_shorturl_image/$year";
			...
			$path_name = "/var/www/vhosts/www.smsstriker.com/htdocs/strikerapp/upload_shorturl_image/$year/$month";
	 		if(!file_exists($path_name)) {
				mkdir($path_name, 0777, true);
				chown($path_name,'striker:root');  
			}
			$path_name = "/var/www/vhosts/www.smsstriker.com/htdocs/strikerapp/upload_shorturl_image/$year/$month/$date";
			if(!file_exists($path_name)) {  
				mkdir($path_name, 0777, true);
				chown($path_name,'striker:root');    
			}	 
	 	  	move_uploaded_file($_FILES['file_input']['tmp_name'],"$path_name/$file_name");
			echo base_url()."upload_shorturl_image/$year/$month/$date/$file_name";  
		}  
   	}  
		...
		// Added on 2017-02-5
		$this->load->view('campaign/api_dlr_report');
		$this->load->view('includes/footer'); 
			...
			if($data['shorturl_number'] != '' && $data['shorturlenter'] != '')
			{
				...
				for($i=1;$i < $bulk_url_count;$i++ ) {
					...
					$getsendShorturl= $this->_data['UrlGenIp'].$result1;
					  ...
					  $bulk_url_response = $this->Campaign_model->createUserBulShorturl($getsendShorturl,$shorturl_id,$result1);  
	...
	public function shorturlReports() {   
		...
		$this->_data['userurl'] = $this->Campaign_model->getUserShortUrls($this->_userId,$from_date,$to_date);
	...
	public function bulkurl() { 
		...
		if($urlId) {		
			$this->load->model('Campaign_model'); 
			  $this->_data['userurl'] = $this->Campaign_model->getUserUrls($this->_userId,$urlId);  
		}  
		...
		if($this->input->post('renewal_package')) { 
  		...
  		if($this->input->post('place_order')) {
			...
			if($product_id) {  
				 ...
				 $this->session->set_userdata('productId', $product_id); 
				 $userid = $this->_userId;
				 $val = 'amount='.base64_encode($amount).'&tax_amount='.base64_encode($tax_amount).'&total_amount='.base64_encode($total_amount).'&name='.base64_encode($name).'&sms_price='.base64_encode($sms_price).'&trnsale='.base64_encode($sms_count).'&customerid='.base64_encode($userid).'&address1='.base64_encode($address).'&address2='.base64_encode($address).'&state='.base64_encode($getstate_id).'&city='.base64_encode($getcity_id).'&zip='.base64_encode($zipcode).'&mobile='.base64_encode($mobile).'&email='.base64_encode($email);
				$testssl_url = base_url()."payment/TestSsl.php?ids=".base64_encode($product_id)."&".$val;
...
//1 21 25500
 ...
 
	{
 		...
 		//$campaign_id = 13;
		//$shortUrl = array ( "http://www.smsstriker.com","http://10.10.10.30/smsstriker/upload_shorturl_image/1_UXMP6952.png");
		//$shortText = array("http://10.10.10.30/shorturl/wvnxklj","http://10.10.10.30/shorturl/vcmljdt");
 		$this->load->model('Campaign_model');  
		$newShortText = array();	 
		if($shortUrl != '')
		{
			...
			{ 
				if($shortUrl[$i] != NULL ) {
					...
					$getSmsText = $this->Campaign_model->updateCampaignSmsText($campaign_id,$this->_userId,$smstext,$campaign_long_url,$campaign_shorturl_text);   
				}  
			} 
