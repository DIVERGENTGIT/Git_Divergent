...
//exit;
				...
				if($error) {
					...
					if ($this->upload->do_upload()) {
						...
						if($file_type == ".xls") {
							...
							$worksheet = $objPHPExcel->getActiveSheet();
							$activeRows = $worksheet->getHighestRow();
							for ($i = 1; $i <= $worksheet->getHighestRow(); $i++) {
								if(strlen(trim($worksheet->getCellByColumnAndRow(0, $i)->getValue())) > 0) {
								 	 
									$to_mobileno=array_unique($to_mobileno);
									$to_mobileno = array_values($to_mobileno);
									 if(count($to_mobileno) == 1000) { break; }
									$to_mobileno[] = trim($worksheet->getCellByColumnAndRow(0, $i)->getValue());
						...
						if($total_numbers == 1000) {
							if($activeRows > $this->_credits) {
								$this->_data['error'] = $this->_data['no_balance'] = "Insufficient SMS Credits.";
							} else { 
	...
	*/
	...
	/*Existed Code Of Customized SMS With File Read, by saisandeepthi, 2017-03-10 	
	public function newVariableSMS()
	{
		$ViewTemplete="";$original_file = "";
		$file_type = "";$Filepath="";$offset = 0;$message = '';
			      ...
			      $uploded_data[$i-1][$j] = $Row[$j];
						...
						if($total_no_of_sms > $this->_credits) {
							...
							$sms_type = 4;
							 ...
							 $campaign_id = $this->Campaign_model->createCustomizedCampaign($this->_userId,$sms_type,$message,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorturl_text,$shorturl_input,$mobileNoCount);  
...
$columns = array();
	...
	{ 
 		...
 		$limit =10;  $from_date  = date('Y-m-d'); $to_date  = date('Y-m-d');
...
elseif($this->_userType == 1 && $this->_dndCheck == 1){
	...
	//  $campaign_id = $this->Campaign_model->createFileShortUrlCampaign($this->_userId,$sms_type,$sms_text,$sender,$_sender,$total_no_of_sms,$is_schedule,$scheduled_date,$sms_port,$campaign_name,$shorttext,$shorturl,$total_numbers);
	  if($mobileNoCount  == 1000) { $total_no_of_sms = 0; }
