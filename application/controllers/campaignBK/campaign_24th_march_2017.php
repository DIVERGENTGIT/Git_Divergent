	...
	public function uploadCustomSmsFile() {  
		...
		if($ViewTemplete=="Y")
		{
			...
			catch (Exception $E)
			{
			  ...
			  $this->_data['error'] = 'Uploaded file is not in readable format.';//$this->upload->display_errors('','');
		}
...
header("Content-Type: text/plain");
 ...
 $data['shorturlreportsdropdown'] = array();
		...
		//$data['shorturlreportsdropdown']=$this->Campaign_model->shorturl_dropdown_reports($this->_userId);
		  ...
		  $data['shorturlreports']=$this->Campaign_model->shorturl_reportssearch_new($this->_userId);
    ...
    $campaign_id = $this->uri->segment(3);
		$data['shorturlreports']=$this->Campaign_model->shortcode_all_reports($this->_userId,$shortcode,$campaign_id);
    // print_r($data['shorturlreports']);
...
//1 21 25500
  ...
  public function getshortcode_reports() {
		...
		$this->load->model('Campaign_model');     
		$this->_data['urlReports'] = $this->Campaign_model->getShortCodeUrls($campaign_id,$shortcode);  
 	 //print_r($this->_data['urlReports']);
		$this->load->view('includes/header',$this->_data);
        	$this->load->view('includes/leftmenu'); 
 ...
 
	{
		...
		//$shortText = array("http://10.10.10.30/shorturl/wvnxklj","http://10.10.10.30/shorturl/vcmljdt");
 		$this->load->model('Campaign_model');  
	//	$newShortText = array();
		//$smstext = '';	$sendShorturl = ''; $newShortText='';$newShortInput='';	 
		if($shortUrl != '')
		{
		 	$shortUrl = 	array_filter($shortUrl);
			if(count($shortUrl) > 0) {
