						...
						if(!$this->_userType){
							...
							$isDND = $this->Campaign_model->checkIsDND($mobile_no);
							 if($isDND)
							 {
							  ...
							  $URL .= "$mclass&dlr-mask=19&dlr-url=".urlencode($this->_data['kernalIp']."/dlr.php?campaign_id=$campaign_id&smscID=%i&dlr=%d&answer=%A&to=%p&from=%P&ts=%T&sms_port=$sms_port"); 
