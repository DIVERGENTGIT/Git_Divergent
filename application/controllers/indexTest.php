...
class indexTest extends CI_Controller
{
	...
	public function __construct()
	{
		...
		if($this->session->userdata('user_id')) {            
			...
			$credits_rs = $this->usert_model->getAvailableCredits($this->_userId);
	...
	{     
		...
		if ($this->input->post('register')) {
			...
			if ($this->form_validation->run('register_form') == FALSE) {
				...
				if(!$this->usert_model->usernameExist()) {
...
<p><b>Mobile: +91 9966507711| Ph : 040&#45;64547711|</b></p> 
		...
		$this->load->model('usert_model');
		$statesAll = $this->usert_model->getNew_StatesList();
...
redirect('http://www.smsstriker.com');
    ...
    public function cities_ajax()
    {
        ...
        $citiesAll = $this->usert_model->getNew_CitiesList($state_id_ajax);
    ...
    public function cities_ajaxreg()
    {
		...
		$this->load->model('usert_model');
        $state_id_ajax=$this->input->post('state_id');
        $citiesAll = $this->usert_model->getNew_CitiesListReg($state_id_ajax);
        $city_all = array('' => '--select--');
        foreach($citiesAll as $row){
             $city_all[$row->city_id] = ucwords(strtolower($row->city_name));
        }
	 ...
	 public function userchecker_forget()
	{
		$this->load->model('usert_model');
		 $username=$this->input->post('username');
		 $checker=$this->usert_model->usernameExist($username);
		 $status ="false";	
		  if($checker)
		  {
			$status = "true";
	 ...
	 public function userchecker()
	{
		$this->load->model('usert_model');
		 $username=$this->input->post('username');
		 $checker=$this->usert_model->getUser_Checker($username);
		 $status ="true";	
		  if($checker)
		  {
			$status = "false";
...
public function login()
	{ 	
		...
		if($this->input->post('username')!='' && $this->input->post('userpass'))
		{
			...
			if ($rs)
			{
				...
				if(!$is_blocked)
				{
					$this->usert_model->updateLoginTime($result->user_id);
					...
					else
					{
						...
						else
						{
							...
							$userexist=$this->usert_model->getPriceTableuser($result->user_id);
							if(count($userexist)>0)
							{
								  $paymentstatus=$this->usert_model->getpaymentstatus($result->user_id);
	...
	public function otpVerify() {
		...
		$this->load->model('usert_model');
		$rs = $this->usert_model->checkUserAuth($otpCode); 
		if($rs && $otpCode)
			{
				foreach($rs as $result)
				{
				...
				{ 
					...
					$this->usert_model->updateLoginTime($user_id);
							 ...
							 $tmp_number = trim($mobileNumber);						
								if(strlen($tmp_number) == 10) {
									if($tmp_number[0] =='7' or $tmp_number[0] =='8' or $tmp_number[0] =='9' )	 {		  	  													
										...
										$this->usert_model->sendAuthCode($sessionData['user_id'],$OTPCODE,$mobileNumber); 
										$this->sendOTP($mobileNumber,$OTPCODE,$username);    
					 					//$this->session->set_userdata('USERID',$sessionData['user_id']);					
										$otpSend = TRUE;
...
//if($result->user_id==3907 ||$result->user_id==3958||$result->user_id==4065||$result->user_id==4066)
	...
	public function forgot()
	{
		...
		if ($this->input->post('forgot_password')) {
			...
			if ($this->form_validation->run('forgot_form') == TRUE) {
				$this->load->model('usert_model');
	$rs = $this->usert_model->usernameExist();
...
$data['not_exist'] = "Your Password not sent!. Please Contact Administrator";
					...
					$this->usert_model->setNewPassword($user_id, $new_password);
...
public function newsletter()
	...
	$data['canonical'] = "http://www.smsstriker.com/newsletter.html";
		if ($this->input->post('news_letter')) {
				$this->load->model('usert_model');
				if(!$this->usert_model->usernameExist_newsletter()) {
					$this->usert_model->newsletter_register();
					//redirect('login');
	...
	public function users_feedback()
	{
	  ...
	  //date_default_timezone_set('asia/calcutta');
		...
		if ($this->input->post('feedback')) {
				...
				//$this->load->model('usert_model');
				//$this->usert_model->feedback($contact_number,$name,$contact_email,$contact_subject,$comment);
	...
	public function college_enquiryform()
	{
		...
		if ($this->input->post('college_enquireform')) {
				...
				$comment = $this->input->post('comment');
				$this->load->model('usert_model');
				$this->usert_model->college_enquireform($contact_number,$name,$address,$courses,$comment);
...
*/
	...
	public function checkCouponValidity() {  
		...
		$couponCode = $this->input->post('coupon');  
	 	$response = $this->usert_model->checkCoupon($couponCode,$user_id);  
	...
	function generatePassword($l = 8, $c = 0, $n = 8, $s = 0) {
	 ...
	 public function userActivityLog($userID,$userName,$otpCode,$status) {
		...
		if($query && $query['status'] == 'success')
 		{
			  $this->usert_model->addUserActivity($query,$userID,$userName,$otpCode,$status);
		}
	}		
