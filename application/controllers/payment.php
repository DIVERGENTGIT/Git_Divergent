...
$jsonRes = json_encode($_REQUEST);$date = date('Y-m-d');
error_log("\n".date('Y-m-d H:i:s')."|Striker ::: Response|".$jsonRes."\r\n",3,"/var/www/html/logs/payments_log/response_".$date.".log"); 
   
$Message = FALSE;
$getStatusMsg = $this->db->query("SELECT * FROM paymentStatusCodes WHERE errorCode = '".$Response_Code."'");
foreach($getStatusMsg->result_array() as $statusMsg)
{
...
//error_log($data."\r\n",3,"/var/www/html/logs/payments_log/".$date.".log");
...
window.self.location='<?php print $web_url;?>campaign/confirm_order/?tn=<?php print $ePGTxnID; ?>&rm=<?php print $Message ?>';
	...
	?>
	<script language="javascript" type="text/javascript">
window.self.location='<?php print $web_url;?>index.php/longcode/renwaldedicated/?tn=<?php print $ePGTxnID; ?>&rm=<?php print $Message ?>';
	</script>  
	<?php     
	} 
	...
	?>
	<script language="javascript" type="text/javascript">
window.self.location='<?php print $web_url;?>index.php/longcode_shared/renwalshared/?tn=<?php print $ePGTxnID; ?>&rm=<?php print $Message ?>';
	</script>
	<?php     
...
public function aes128Encrypt($str,$key)
	...
	if(isset($_REQUEST['tn']))  
		...
		$_SESSION['total_amount'] = $total_amount1=$totalPrice+$tax_amount;
 		...
 		if(@$_POST['confirm_order']!='')
			...
			$getUserType = $this->db->query("SELECT user_id from users WHERE user_id ='".$userid."' AND reseller_id = 0 ");
			if($getUserType->num_rows() > 0) {
				$getUserType = $this->db->query("SELECT payment_id from user_payments WHERE user_id ='".$userid."' ");
				if($getUserType->num_rows() == 0) {
					$pagetype = 'inside-new';
				}    
			}  
			...
			$sql="update price_enquery set transaction_id='$transaction_id' ,pagetype = '$pagetype' where id in ($ids)";
			$this->db->query($sql);

			$qty = $_SESSION['noofsms'];$sms_price = $_SESSION['sms_price'];$amount = $_SESSION['totalPrice'];
			$tax_amount=round(($amount *$this->_servicetax)/100);$total_amount = $tax_amount+$amount;
	...
	session_start();
	if($this->input->post('updateNOOfSMS')) {
		$_SESSION['noofsms'] = $this->input->post('noofsms');
	}
	$user_id=$this->session->userdata('user_id'); 
	...
	$this->load->view('/includes/footer'); 
     ...
     public function getSMSPriceOnCredits($credits) {
     	$user_id = $this->session->userdata('user_id');
	...
	$sql = "select price,note from user_payments where user_id=$user_id and price > 1000000 and  service_type ='' order by payment_id desc limit 1";
	...
	$user_id=$this->session->userdata('user_id');
 	...
 	if($_SESSION['noofsms'] <= 0) {  
     	...
     	if(!empty($_POST['noofsms']) && @$_POST['noofsms']!='')
        {
        	...
        	$smsprice = $this->getSMSPriceOnCredits($noofsms);
		...
		$amount = $smsprice*$noofsms;
		
		//$tax_amount=round(($amount *$this->_servicetax)/100);
...
redirect($testssl_url);*/
	...
	$this->db->query($sql);
		//if($this->_userId == 2917) {
	 		//echo $gettotal_amount= ceil($_SESSION['totalPrice']); echo " - Total Amount</br>";
			//echo $qty = $_SESSION['noofsms'];echo " - Qty</br>";
			//echo $sms_price = $_SESSION['sms_price'];echo " - sms_price</br>";
			//echo $amount = $_SESSION['totalPrice']; echo " - amount </br>";
	 		//echo $tax_amount=round(($amount *$this->_servicetax)/100);echo " - tax_amount</br>";
			//echo $total_amount = $tax_amount+$amount;echo " - Total Amount</br>"; exit;
